window.DE_CONCEPTS = [
  {
    "name": "Cardinality",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Number of distinct values in a column (or relation size: number of rows).",
    "why": "Optimizer uses cardinality to choose joins, indexes, and broadcast vs shuffle.",
    "example": "`country` has ~200 distinct values (low cardinality). `user_id` may have millions (high cardinality).",
    "remember": [
      "Low cardinality \u2192 bitmap indexes / dictionaries often help",
      "Wrong cardinality estimates \u2192 bad plans"
    ],
    "mustKnow": 1,
    "snippet": "SELECT COUNT(DISTINCT user_id) AS cardinality FROM events;",
    "related": [
      "Selectivity",
      "Histograms",
      "Data Skew"
    ],
    "symptom": "Bad join strategy because NDV was underestimated.",
    "tags": [
      "interview",
      "sql"
    ]
  },
  {
    "name": "Granularity (Grain)",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "The level of detail one row represents (what one row means).",
    "why": "Wrong grain causes double-counting, bad joins, and incorrect metrics.",
    "example": "Orders table grain = one row per `order_id`. Order-items grain = one row per `order_id + product_id`.",
    "remember": [
      "Always state grain before modeling",
      "Never mix grains in one fact without care"
    ],
    "mustKnow": null,
    "snippet": "-- State grain before modeling:\n-- one row per order_id + product_id (line item)",
    "related": [
      "Grain",
      "Additive Facts",
      "Bridge Table"
    ],
    "symptom": "Revenue doubles after joining order headers to line items.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Selectivity",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Fraction of rows a predicate returns. High selectivity = few rows (selective filter).",
    "why": "Selective predicates favor index seeks; low selectivity favors scans.",
    "example": "`WHERE user_id = 42` is highly selective. `WHERE status = 'active'` on 90% active rows is not.",
    "remember": [
      "Selectivity \u2248 matching_rows / total_rows",
      "Indexes help most on selective filters"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Data Distribution",
      "Histograms",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Data Distribution",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "How values are spread across a column (uniform, skewed, sparse).",
    "why": "Skew causes hot partitions, slow joins, and uneven executor load.",
    "example": "80% of orders from one country \u2192 that partition/key dominates shuffle.",
    "remember": [
      "Check histograms / approx_percentile for skew",
      "Salting and AQE help with skew"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Histograms",
      "Cost-Based Optimizer (CBO)",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Histograms",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Statistics summarizing value frequency distribution in a column.",
    "why": "Help CBO estimate selectivity when values are uneven.",
    "example": "Optimizer sees `status='cancelled'` is 2% of rows via histogram, not assuming uniform 25%.",
    "remember": [
      "Stale histograms \u2192 bad plans",
      "Refresh stats after large loads"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Cost-Based Optimizer (CBO)",
      "Rule-Based Optimizer (RBO)",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Cost-Based Optimizer (CBO)",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Chooses execution plan by estimating cost using table/column statistics.",
    "why": "Modern warehouses and Spark SQL rely on CBO for join order and strategies.",
    "example": "With stats, CBO picks broadcast hash join for a small dimension; without stats, may shuffle both sides.",
    "remember": [
      "Collect/analyze stats regularly",
      "No stats \u2248 guessing"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Rule-Based Optimizer (RBO)",
      "Execution Plan",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Rule-Based Optimizer (RBO)",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Rewrites queries using fixed heuristic rules (e.g., push filters early), not cost estimates.",
    "why": "Fast and predictable; weak when data sizes vary.",
    "example": "Always push `WHERE` before `JOIN` regardless of table size.",
    "remember": [
      "Spark Catalyst uses both rules and cost",
      "RBO alone can miss better join strategies"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Execution Plan",
      "Explain Plan",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Execution Plan",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "The concrete operators the engine will run (scans, joins, aggregates, sorts).",
    "why": "Primary tool to debug slow queries.",
    "example": "Plan shows `SortMergeJoin` + large `Exchange` (shuffle) \u2192 expensive.",
    "remember": [
      "Read bottom-up or follow arrows depending on engine",
      "Look for scans, shuffles, spills"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Explain Plan",
      "Composite Index",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Explain Plan",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Command/API that prints the logical and/or physical plan without always running the full query.",
    "why": "Inspect strategy before/while tuning.",
    "example": "```sql\nEXPLAIN ANALYZE SELECT * FROM orders WHERE order_date = '2024-01-01';\n-- Spark: df.explain(True)  or  EXPLAIN EXTENDED / COST\n```",
    "remember": [
      "`ANALYZE` / actual metrics beat estimated-only plans",
      "Compare before vs after rewrite"
    ],
    "mustKnow": null,
    "snippet": "EXPLAIN ANALYZE SELECT ...;\n-- Spark: df.explain(\"cost\")",
    "related": [
      "Execution Plan",
      "Cost-Based Optimizer (CBO)",
      "Statistics"
    ],
    "symptom": "Tuning blindly without reading the plan.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Composite Index",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Index on multiple columns in a defined order.",
    "why": "Speeds queries filtering/sorting on the leading columns.",
    "example": "Index `(customer_id, order_date)` helps `WHERE customer_id = ?` and `WHERE customer_id = ? AND order_date > ?`, not `WHERE order_date > ?` alone (usually).",
    "remember": [
      "Column order matters (leftmost prefix)",
      "Don't over-index; writes get slower"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Covering Index",
      "Clustered vs Non-Clustered Index",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Covering Index",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Index that contains all columns a query needs, so the table heap/base isn't touched.",
    "why": "Index-only scans are much faster for narrow queries.",
    "example": "Query selects `id, email` with index on `(id) INCLUDE (email)` \u2192 covering.",
    "remember": [
      "Great for hot read paths",
      "Wider indexes cost more storage/write amp"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Clustered vs Non-Clustered Index",
      "Bitmap Index",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Clustered vs Non-Clustered Index",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "- **Clustered:** table rows stored in index key order (often primary key); typically one per table. - **Non-clustered:** separate structure pointing to rows; many allowed.",
    "why": "Clustered key choice affects range scans and insert patterns.",
    "example": "Clustered on `order_date` makes date-range queries fast; random UUIDs as clustered keys can fragment inserts.",
    "remember": [
      "InnoDB PK is clustered; SQL Server similar",
      "Choose clustered key for common access pattern"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Bitmap Index",
      "B-Tree Index",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Bitmap Index",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Index storing bitmaps per distinct value; good for low-cardinality columns and AND/OR of predicates.",
    "why": "Common in analytic DBs; poor for high-churn OLTP updates.",
    "example": "Bitmap on `gender`, `region`, `status` for warehouse filters.",
    "remember": [
      "Low cardinality + analytics",
      "Avoid on frequently updated columns"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "B-Tree Index",
      "Predicate Pushdown",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "B-Tree Index",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Balanced tree index supporting equality and range lookups; default in most RDBMS.",
    "why": "Workhorse for OLTP and many warehouse lookups.",
    "example": "B-tree on `email` \u2192 fast `WHERE email = 'a@b.com'`.",
    "remember": [
      "Equality + ranges",
      "Not ideal alone for very low cardinality"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Predicate Pushdown",
      "Partition Pruning",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Predicate Pushdown",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Move filters as close as possible to the data source so less data is read/shuffled.",
    "why": "Huge I/O and network savings on parquet/ORC and remote sources.",
    "example": "`spark.read.parquet(...).filter(\"year=2024\")` pushes year filter into file/footer reading when partitioned.",
    "remember": [
      "Prefer partition columns and columnar formats",
      "UDFs often block pushdown"
    ],
    "mustKnow": 3,
    "snippet": "df = spark.read.parquet(\"s3://lake/events\").filter(\"dt = '2024-01-01'\")\n# filter pushed into scan / partition prune",
    "related": [
      "Partition Pruning",
      "Catalyst Optimizer",
      "Statistics"
    ],
    "symptom": "Full scan of years of data for a one-day query.",
    "tags": [
      "interview",
      "spark",
      "sql"
    ]
  },
  {
    "name": "Partition Pruning",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Skip reading partitions that cannot match the query filter.",
    "why": "Turns full scans into reading a fraction of data.",
    "example": "Table partitioned by `dt`; `WHERE dt='2024-06-01'` reads only that partition.",
    "remember": [
      "Filter on partition columns explicitly",
      "Dynamic partition pruning helps joins"
    ],
    "mustKnow": 4,
    "snippet": "SELECT * FROM sales WHERE dt BETWEEN '2024-01-01' AND '2024-01-07';\n-- only those partitions are read",
    "related": [
      "Predicate Pushdown",
      "Dynamic Partition Pruning",
      "Small File Problem"
    ],
    "symptom": "Query reads every partition folder despite a date filter.",
    "tags": [
      "interview",
      "spark",
      "sql"
    ]
  },
  {
    "name": "Statistics",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "Metadata about tables/columns: row counts, NDV, min/max, histograms, size.",
    "why": "Fuel for CBO and good join strategies.",
    "example": "```sql\nANALYZE TABLE orders COMPUTE STATISTICS FOR COLUMNS customer_id, amount;\n```",
    "remember": [
      "Refresh after bulk loads/deletes",
      "Missing stats \u2192 conservative/bad plans"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Table Scan vs Index Scan",
      "Seek vs Scan",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Table Scan vs Index Scan",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "- **Table (full) scan:** read all rows/pages. - **Index scan:** walk an index (may still touch many rows).",
    "why": "Scans aren't always bad \u2014 for large fractions of the table they can win.",
    "example": "Selecting 2% of rows by PK \u2192 index. Selecting 70% \u2192 full scan often better.",
    "remember": [
      "Optimizer picks based on selectivity + cost",
      "\"Index scan\" \u2260 cheap if it returns most of the table"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Seek vs Scan",
      "Cardinality"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "Seek vs Scan",
    "category": "Database & SQL Concepts",
    "categorySlug": "01-database-sql",
    "file": "01-database-sql/concepts.md",
    "definition": "- **Seek:** jump to a specific key/range in the index. - **Scan:** read a contiguous slice (or all) of index/table.",
    "why": "Seeks are for pinpoint lookups; scans for broader access.",
    "example": "`WHERE id = 10` \u2192 seek. `WHERE id BETWEEN 1 AND 1000000` \u2192 range scan.",
    "remember": [
      "Seek + few rows = ideal",
      "Large range scan can rival table scan"
    ],
    "mustKnow": null,
    "snippet": "-- inspect plan\nEXPLAIN ANALYZE SELECT ...;",
    "related": [
      "Cardinality",
      "Granularity (Grain)"
    ],
    "symptom": "Slow query with no clear plan insight.",
    "tags": [
      "sql"
    ]
  },
  {
    "name": "DAG Scheduler",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Builds a DAG of stages from the RDD/DataFrame lineage and submits stage tasks when parents finish.",
    "why": "Explains why shuffles create stage boundaries and how failures recompute.",
    "example": "`map \u2192 filter \u2192 reduceByKey` \u2192 narrow stage then shuffle stage.",
    "remember": [
      "Stages split at wide dependencies (shuffles)",
      "Failed tasks recompute from lineage"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Task Scheduler",
      "Driver"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Task Scheduler",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Assigns individual tasks to executors (resource offers from cluster manager).",
    "why": "Locality, speculation, and stragglers live here.",
    "example": "Prefer `PROCESS_LOCAL` / `NODE_LOCAL` tasks when data is cached on that executor.",
    "remember": [
      "One task \u2248 one partition",
      "Speculation helps slow stragglers"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Driver",
      "Executor",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Driver",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "The process that runs your main program, builds the DAG, and coordinates executors.",
    "why": "Driver OOM or GC pauses stall the whole job.",
    "example": "Collecting a huge DataFrame to the driver (`collect()`) crashes the driver.",
    "remember": [
      "Keep driver light",
      "Avoid large `collect` / broadcast of huge objects"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Executor",
      "Worker",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Executor",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "JVM worker process that runs tasks and stores shuffle/cache data.",
    "why": "Memory, cores, and disk per executor dominate performance.",
    "example": "10 executors \u00d7 4 cores \u2192 up to 40 concurrent tasks.",
    "remember": [
      "Tune `executor-memory` and cores together",
      "Watch spill and GC"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Worker",
      "Cluster Manager",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Worker",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Cluster node process (e.g., Spark standalone worker) that hosts executors.",
    "why": "Distinguishes machine vs JVM process.",
    "example": "One worker machine runs multiple executors.",
    "remember": [
      "Worker \u2260 executor",
      "YARN/K8s abstract workers differently"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Cluster Manager",
      "Job \u2192 Stage \u2192 Task",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Cluster Manager",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Allocates resources: Standalone, YARN, Kubernetes, Mesos (legacy).",
    "why": "How cores/memory are requested and released.",
    "example": "`spark.master=yarn` or `k8s://...` with dynamic allocation.",
    "remember": [
      "Dynamic allocation needs external shuffle service (classic YARN)",
      "K8s common in modern platforms"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Job \u2192 Stage \u2192 Task",
      "Lineage",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Job \u2192 Stage \u2192 Task",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "- **Job:** triggered by an action (`count`, `write`, `collect`) - **Stage:** set of tasks with narrow deps between shuffles - **Task:** unit of work on one partition",
    "why": "UI debugging language.",
    "example": "One `write` action \u2192 1 job \u2192 3 stages \u2192 N tasks per stage.",
    "remember": [
      "Actions create jobs",
      "Shuffles create stages",
      "Partitions create tasks"
    ],
    "mustKnow": 6,
    "snippet": "# action \u2192 job; shuffle boundary \u2192 stage; partition \u2192 task\ndf.write.parquet(path)  # triggers a job",
    "related": [
      "DAG Scheduler",
      "Shuffle",
      "Lazy Evaluation"
    ],
    "symptom": "Confused debugging \u2014 looking at tasks without finding the slow stage.",
    "tags": [
      "interview",
      "spark"
    ]
  },
  {
    "name": "Lineage",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Graph of transformations needed to recompute an RDD/DataFrame partition.",
    "why": "Fault tolerance without replicating all data.",
    "example": "Lose executor \u2192 recompute lost partitions from source + lineage.",
    "remember": [
      "Checkpoint breaks long lineages",
      "Lineage is logical, not always stored data"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Lazy Evaluation",
      "Shuffle",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Lazy Evaluation",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Transformations build a plan; work runs only on actions.",
    "why": "Enables whole-stage optimization and predicate pushdown.",
    "example": "Many `filter`/`select` calls do nothing until `show()` or `write`.",
    "remember": [
      "Chain transforms freely",
      "Actions trigger compute (and cost)"
    ],
    "mustKnow": 7,
    "snippet": "f = df.filter(...).select(...)  # builds plan only\nf.count()                     # action runs the job",
    "related": [
      "Catalyst Optimizer",
      "Lineage",
      "Job \u2192 Stage \u2192 Task"
    ],
    "symptom": "Thinking transforms already ran; surprise cost on first action.",
    "tags": [
      "interview",
      "spark"
    ]
  },
  {
    "name": "Shuffle",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Redistribute data across partitions by key (network + sort/hash).",
    "why": "Usually the most expensive part of a job.",
    "example": "`groupBy`, `join`, `repartition`, `distinct` often shuffle.",
    "remember": [
      "Minimize shuffles",
      "Watch shuffle read/write in Spark UI"
    ],
    "mustKnow": 5,
    "snippet": "df.groupBy(\"user_id\").count()  # wide transform \u2192 shuffle\n# Spark UI: Shuffle Read/Write bytes",
    "related": [
      "Wide vs Narrow Transformations",
      "Job \u2192 Stage \u2192 Task",
      "Data Skew"
    ],
    "symptom": "Stage stuck; network/disk dominated by shuffle.",
    "tags": [
      "interview",
      "performance",
      "spark"
    ]
  },
  {
    "name": "Spill to Disk",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "When memory is insufficient, intermediate data is written to local disk.",
    "why": "Correctness preserved; speed drops.",
    "example": "Large sort-merge join spills during sort.",
    "remember": [
      "Spill \u2260 failure; it's a warning sign",
      "Increase memory, reduce partition size, or fix skew"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Memory Management",
      "Serialization (Java/Kryo)",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Memory Management",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Spark splits executor memory among execution (shuffles/joins), storage (cache), and user/overhead.",
    "why": "Contention between cache and shuffle causes eviction/spill.",
    "example": "Unified memory region shared by execution and storage (post Spark 1.6).",
    "remember": [
      "Don't cache everything",
      "Tune fraction only after measuring"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Serialization (Java/Kryo)",
      "Broadcast Variables",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Serialization (Java/Kryo)",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Convert objects to bytes for shuffle/cache/network. Kryo is faster/smaller than Java serializer when registered.",
    "why": "Slow serialization burns CPU on every shuffle.",
    "example": "`spark.serializer=org.apache.spark.serializer.KryoSerializer` + register classes.",
    "remember": [
      "Prefer DataFrames (Catalyst/Tungsten) over RDDs of custom objects",
      "Kryo helps RDD/custom types"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Broadcast Variables",
      "Accumulators",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Broadcast Variables",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Read-only copy of data sent once to each executor.",
    "why": "Avoids shipping large lookup tables per task.",
    "example": "Broadcast a small dimension for map-side join / `broadcast(df)`.",
    "remember": [
      "Size must fit executor memory",
      "Default auto-broadcast threshold exists in Spark SQL"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Accumulators",
      "Closure",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Accumulators",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Write-only aggregate variables (counters) updated by executors, read on driver.",
    "why": "Lightweight metrics without collecting data.",
    "example": "Count malformed records while mapping.",
    "remember": [
      "Updates inside transforms may be recomputed on retry \u2014 use carefully",
      "Prefer built-in Spark metrics when possible"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Closure",
      "Wide vs Narrow Transformations",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Closure",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Function + captured outer variables sent to executors.",
    "why": "Accidental capture of huge objects (DB connections, full DataFrames) causes OOM or slow shipping.",
    "example": "Capturing a large Python list in a UDF serializes it to every task.",
    "remember": [
      "Don't capture huge objects",
      "Prefer broadcast for shared lookups"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Wide vs Narrow Transformations",
      "Adaptive Query Execution (AQE)",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Wide vs Narrow Transformations",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "- **Narrow:** each input partition maps to \u22641 output partition (`map`, `filter`) \u2014 no shuffle - **Wide:** input partitions contribute to many outputs (`groupByKey`, `join`) \u2014 shuffle",
    "why": "Wide = stage boundary + cost.",
    "example": "`filter` narrow; `reduceByKey` wide.",
    "remember": [
      "Pipeline narrow transforms in one stage",
      "Wide transforms need justification"
    ],
    "mustKnow": 8,
    "snippet": "# narrow: map/filter (no shuffle)\n# wide: groupBy/join/repartition (shuffle)",
    "related": [
      "Shuffle",
      "DAG Scheduler",
      "Broadcast Join"
    ],
    "symptom": "Extra wide transforms inflate runtime for little gain.",
    "tags": [
      "interview",
      "spark"
    ]
  },
  {
    "name": "Adaptive Query Execution (AQE)",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Spark SQL re-optimizes the plan at runtime using size stats after shuffles (Spark 3+).",
    "why": "Fixes bad static estimates: coalesces partitions, switches join strategy, handles skew.",
    "example": "AQE converts sort-merge to broadcast when post-shuffle side is small.",
    "remember": [
      "Enable AQE in modern Spark (`spark.sql.adaptive.enabled`)",
      "Helps skew join and partition coalescing"
    ],
    "mustKnow": 9,
    "snippet": "spark.conf.set(\"spark.sql.adaptive.enabled\", \"true\")\nspark.conf.set(\"spark.sql.adaptive.skewJoin.enabled\", \"true\")",
    "related": [
      "Skew Join Optimization",
      "Broadcast Join",
      "Data Skew"
    ],
    "symptom": "Static plan picks sort-merge when broadcast would win.",
    "tags": [
      "interview",
      "performance",
      "spark"
    ]
  },
  {
    "name": "Catalyst Optimizer",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Spark SQL's rule + cost optimizer: analysis \u2192 logical optimization \u2192 physical planning.",
    "why": "Why DataFrames beat handwritten RDDs for SQL-like work.",
    "example": "Constant folding, predicate pushdown, column pruning done automatically.",
    "remember": [
      "Prefer DataFrame/Dataset API",
      "UDFs limit Catalyst optimizations"
    ],
    "mustKnow": 10,
    "snippet": "df.explain(True)  # parsed/analyzed/optimized/physical plans",
    "related": [
      "Tungsten Engine",
      "Predicate Pushdown",
      "Cost-Based Optimizer (CBO)"
    ],
    "symptom": "Python UDF blocks optimizations \u2192 slower than SQL expr.",
    "tags": [
      "interview",
      "spark"
    ]
  },
  {
    "name": "Tungsten Engine",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Execution layer: off-heap/cache-aware memory, whole-stage codegen, binary processing.",
    "why": "Faster CPU and less GC than row-object RDDs.",
    "example": "Whole-stage codegen fuses operators into Java bytecode.",
    "remember": [
      "Works best with native expressions (not Python UDFs)",
      "Columnar formats amplify gains"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Dynamic Partition Pruning",
      "Skew Join Optimization",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Dynamic Partition Pruning",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "At runtime, prune partitions of a large fact using values from a filtered dimension join.",
    "why": "Star-schema queries read far less data.",
    "example": "Join `sales` (partitioned by `date`) to `dim_date` filtered to one month \u2192 only those fact partitions scanned.",
    "remember": [
      "Needs partitioned fact + filter on dimension",
      "Related to AQE / DPP features"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "Skew Join Optimization",
      "DAG Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Skew Join Optimization",
    "category": "Spark Internals",
    "categorySlug": "02-spark-internals",
    "file": "02-spark-internals/concepts.md",
    "definition": "Detect hot keys and split/handle them separately (AQE skew join, salting).",
    "why": "One key can stall an entire stage.",
    "example": "One `customer_id` with 40% of rows \u2192 AQE splits that partition.",
    "remember": [
      "Check Spark UI for straggler tasks",
      "Salting is the manual fallback"
    ],
    "mustKnow": null,
    "snippet": "df.explain(True)  # check stages/shuffles",
    "related": [
      "DAG Scheduler",
      "Task Scheduler"
    ],
    "symptom": "Stage time dominated by unexpected work.",
    "tags": [
      "spark"
    ]
  },
  {
    "name": "Grain",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Exact meaning of one fact row (business level of detail).",
    "why": "Foundation of dimensional modeling; wrong grain breaks metrics.",
    "example": "\"One row per order line per day\" vs \"one row per order\".",
    "remember": [
      "Declare grain in one sentence before designing",
      "All measures must match that grain"
    ],
    "mustKnow": 2,
    "snippet": "-- Fact grain: 1 row = 1 completed checkout\n-- Always declare grain before designing the fact",
    "related": [
      "Granularity (Grain)",
      "Snapshot Fact",
      "Additive Facts"
    ],
    "symptom": "KPI doesn't match finance because grains differ.",
    "tags": [
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "Granularity",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Same idea as grain \u2014 how fine or coarse the data is. Often used interchangeably with grain in DW.",
    "why": "Aggregation level must match reporting needs.",
    "example": "Daily grain vs monthly rollup tables.",
    "remember": [
      "Finer grain \u2192 more rows, more flexibility",
      "Coarser \u2192 faster reports, less detail"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Additive Facts",
      "Semi-Additive Facts",
      "Grain"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Additive Facts",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Measures that can be summed across any dimension.",
    "why": "Safest metrics for rollups.",
    "example": "`sales_amount`, `quantity_sold` \u2014 sum by day, store, product all valid.",
    "remember": [
      "Prefer additive measures when possible",
      "Document additivity of each measure"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Semi-Additive Facts",
      "Non-Additive Facts",
      "Grain"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Semi-Additive Facts",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Summable across some dimensions, not others (often not across time).",
    "why": "Easy to misuse in BI (double-count balances).",
    "example": "Account balance \u2014 sum across customers OK; sum across days wrong (use last/average).",
    "remember": [
      "Inventory and balances are classic semi-additive",
      "Use snapshot patterns carefully"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Non-Additive Facts",
      "Degenerate Dimension",
      "Grain"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Non-Additive Facts",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Cannot be summed meaningfully; ratios, percentages, temperatures.",
    "why": "Must recalculate from components, not average averages blindly.",
    "example": "`profit_margin` \u2014 store `profit` and `revenue`, compute margin at query time.",
    "remember": [
      "Store additive components",
      "Aggregate then compute ratio"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Degenerate Dimension",
      "Junk Dimension",
      "Grain"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Degenerate Dimension",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Dimension-like identifier stored in the fact table with no separate dimension table.",
    "why": "Avoids empty dimension tables for transactional IDs.",
    "example": "`order_id`, `invoice_number` living on the fact row.",
    "remember": [
      "Common for transaction numbers",
      "Still useful for drill-to-detail"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Junk Dimension",
      "Role-Playing Dimension",
      "Grain"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Junk Dimension",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Single dimension table combining low-cardinality flags/indicators.",
    "why": "Avoids many tiny dimensions cluttering the model.",
    "example": "`is_gift`, `payment_type`, `channel` packed into `dim_order_flags`.",
    "remember": [
      "Keep cardinality manageable",
      "Good for yes/no and small enums"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Role-Playing Dimension",
      "Conformed Dimension",
      "Grain"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Role-Playing Dimension",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Same dimension reused for multiple roles via different foreign keys.",
    "why": "One `dim_date` serves order date, ship date, delivery date.",
    "example": "`order_date_key`, `ship_date_key` both point to `dim_date`.",
    "remember": [
      "Views/aliases per role in BI tools",
      "Don't duplicate date tables"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Conformed Dimension",
      "Mini Dimension",
      "Grain"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Conformed Dimension",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Shared dimension with consistent keys and attributes across facts/marts.",
    "why": "Enables cross-process drill-across reporting.",
    "example": "Same `customer_key` and customer attributes in sales and support marts.",
    "remember": [
      "Enterprise consistency",
      "Owned by a shared data team/process"
    ],
    "mustKnow": 15,
    "snippet": "-- same customer_key + attributes shared by sales & support facts",
    "related": [
      "Role-Playing Dimension",
      "Grain",
      "Data Lineage"
    ],
    "symptom": "Two marts define \"customer\" differently; dashboards disagree.",
    "tags": [
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "Mini Dimension",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Split frequently changing attributes into a smaller dimension to limit type-2 explosion on the main dim.",
    "why": "Keeps large customer dims manageable when demographics change often.",
    "example": "`dim_customer` stable attrs + `dim_customer_profile` for segment/score that changes monthly.",
    "remember": [
      "Use when type-2 history would explode",
      "Fact points to both keys when needed"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Bridge Table",
      "Factless Fact Table",
      "Grain"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Bridge Table",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Helper table resolving many-to-many between fact and dimension (or between dims).",
    "why": "Correct weighting and multi-valued attributes.",
    "example": "Order with multiple sales reps \u2192 `bridge_order_rep` with allocation weights.",
    "remember": [
      "Watch double-counting; use weights",
      "Common for multi-valued dims (diagnoses, tags)"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Factless Fact Table",
      "Snapshot Fact",
      "Grain"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Factless Fact Table",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Fact table with keys only (no numeric measures) \u2014 records events or coverage.",
    "why": "Answers \"did it happen?\" and \"what was assigned?\"",
    "example": "Student attendance: `student_key`, `class_key`, `date_key` \u2014 count rows = attendance events.",
    "remember": [
      "Event tracking and coverage/eligibility",
      "Measures are often counts of rows"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Snapshot Fact",
      "Accumulating Snapshot",
      "Grain"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Snapshot Fact",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "Fact capturing state at a point in time (generic term; often periodic).",
    "why": "Measures levels (balances, inventory) over time.",
    "example": "Daily inventory on-hand per SKU per warehouse.",
    "remember": [
      "Semi-additive measures common",
      "Choose periodic vs accumulating pattern"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Accumulating Snapshot",
      "Periodic Snapshot",
      "Grain"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Accumulating Snapshot",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "One row per process instance; columns updated as milestones complete (pipeline pipeline).",
    "why": "Tracks lifecycle duration and conversion.",
    "example": "Order fulfillment: `ordered_at`, `packed_at`, `shipped_at`, `delivered_at` on one row, updated in place.",
    "remember": [
      "Fixed known milestones",
      "Lag metrics between dates"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Periodic Snapshot",
      "Grain"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Periodic Snapshot",
    "category": "Data Warehouse Concepts",
    "categorySlug": "03-data-warehouse",
    "file": "03-data-warehouse/concepts.md",
    "definition": "New fact rows taken at regular intervals for all entities (day/week/month).",
    "why": "Time-series of levels for reporting.",
    "example": "End-of-day account balance for every account every day.",
    "remember": [
      "Dense over time",
      "Storage heavy but query-friendly for trends"
    ],
    "mustKnow": null,
    "snippet": "-- declare grain, then model facts/dims",
    "related": [
      "Grain",
      "Granularity"
    ],
    "symptom": "Metrics disagree across marts.",
    "tags": [
      "warehouse"
    ]
  },
  {
    "name": "Data Lake",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Central storage of raw and processed data in files (often object storage), schema-on-read.",
    "why": "Cheap scale for diverse data; weak governance if unmanaged.",
    "example": "S3/ADLS/GCS buckets with landing, bronze, silver zones as folders.",
    "remember": [
      "Storage \u2260 warehouse",
      "Needs catalog, quality, and access control"
    ],
    "mustKnow": null,
    "snippet": "-- prefer ACID table format ops (MERGE/OPTIMIZE)",
    "related": [
      "Data Lakehouse",
      "Delta Lake"
    ],
    "symptom": "Readers see partial or inconsistent files.",
    "tags": [
      "lakehouse"
    ]
  },
  {
    "name": "Data Lakehouse",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Lake storage + warehouse capabilities: ACID tables, governance, BI-friendly SQL.",
    "why": "One copy of data for batch, streaming, and analytics.",
    "example": "Delta/Iceberg tables on S3 queried by Spark, Trino, and BI tools.",
    "remember": [
      "Open table formats are the core",
      "Combines lake cost with warehouse reliability"
    ],
    "mustKnow": null,
    "snippet": "-- prefer ACID table format ops (MERGE/OPTIMIZE)",
    "related": [
      "Delta Lake",
      "Apache Iceberg",
      "Data Lake"
    ],
    "symptom": "Readers see partial or inconsistent files.",
    "tags": [
      "lakehouse"
    ]
  },
  {
    "name": "Delta Lake",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Open table format (Databricks-origin) with transaction log (`_delta_log`) for ACID on files.",
    "why": "Upserts, time travel, schema enforcement on object storage.",
    "example": "`MERGE INTO target USING source ON ... WHEN MATCHED THEN UPDATE ...`",
    "remember": [
      "Transaction log is source of truth",
      "`OPTIMIZE` + `VACUUM` for maintenance"
    ],
    "mustKnow": 16,
    "snippet": "MERGE INTO target t USING source s ON t.id = s.id\nWHEN MATCHED THEN UPDATE SET *\nWHEN NOT MATCHED THEN INSERT *;",
    "related": [
      "Time Travel",
      "ACID Tables",
      "Apache Iceberg"
    ],
    "symptom": "Concurrent jobs overwrite parquet folders; readers see partial data.",
    "tags": [
      "interview",
      "lakehouse"
    ]
  },
  {
    "name": "Apache Iceberg",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Open table format with snapshot metadata, hidden partitioning, strong engine interoperability.",
    "why": "Multi-engine lakehouse (Spark, Flink, Trino, Athena).",
    "example": "Partition evolution without rewriting all data paths visible to users.",
    "remember": [
      "Snapshots + manifests",
      "Hidden partitioning is a key feature"
    ],
    "mustKnow": null,
    "snippet": "-- prefer ACID table format ops (MERGE/OPTIMIZE)",
    "related": [
      "Apache Hudi",
      "Time Travel",
      "Data Lake"
    ],
    "symptom": "Readers see partial or inconsistent files.",
    "tags": [
      "lakehouse"
    ]
  },
  {
    "name": "Apache Hudi",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Open table format focused on incremental processing, upserts, and streaming ingestion.",
    "why": "Near-real-time lakes with copy-on-write or merge-on-read.",
    "example": "Upsert CDC into a Hudi MOR table; readers see compacted view.",
    "remember": [
      "COW vs MOR tradeoffs (write vs read cost)",
      "Strong for CDC / incremental pipelines"
    ],
    "mustKnow": null,
    "snippet": "-- prefer ACID table format ops (MERGE/OPTIMIZE)",
    "related": [
      "Time Travel",
      "Schema Evolution",
      "Data Lake"
    ],
    "symptom": "Readers see partial or inconsistent files.",
    "tags": [
      "lakehouse",
      "streaming"
    ]
  },
  {
    "name": "Time Travel",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Query table as of a past version/timestamp via snapshots/logs.",
    "why": "Reproduce reports, debug bad writes, audit.",
    "example": "```sql\nSELECT * FROM orders VERSION AS OF 120;\n-- or TIMESTAMP AS OF '2024-06-01'\n```",
    "remember": [
      "Retention limited by vacuum/expire settings",
      "Great for \"what did we have yesterday?\""
    ],
    "mustKnow": 17,
    "snippet": "SELECT * FROM orders VERSION AS OF 120;\n-- or TIMESTAMP AS OF '2024-06-01'",
    "related": [
      "Delta Lake",
      "Vacuum",
      "Checkpointing"
    ],
    "symptom": "Can't reproduce yesterday's report after a bad overwrite.",
    "tags": [
      "interview",
      "lakehouse"
    ]
  },
  {
    "name": "Schema Evolution",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Safely change table schema (add/rename/drop columns) without full reload when supported.",
    "why": "Pipelines and producers change over time.",
    "example": "Adding `discount_code` column to a Delta/Iceberg table.",
    "remember": [
      "Additive changes are safest",
      "Breaking changes need contracts + migration"
    ],
    "mustKnow": 18,
    "snippet": "df.write.format(\"delta\").option(\"mergeSchema\", \"true\").mode(\"append\").save(path)",
    "related": [
      "Schema Enforcement",
      "Data Contracts",
      "Delta Lake"
    ],
    "symptom": "Producer adds a column; downstream jobs break or ignore it silently.",
    "tags": [
      "interview",
      "lakehouse",
      "pipeline"
    ]
  },
  {
    "name": "Schema Enforcement",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Reject writes that don't match the table schema (or strict evolution rules).",
    "why": "Stops corrupt/partial schemas from poisoning the lake.",
    "example": "Write with extra unexpected type fails instead of silently creating junk files.",
    "remember": [
      "Enforce at write boundary",
      "Pair with data contracts"
    ],
    "mustKnow": null,
    "snippet": "-- prefer ACID table format ops (MERGE/OPTIMIZE)",
    "related": [
      "ACID Tables",
      "Compaction",
      "Data Lake"
    ],
    "symptom": "Readers see partial or inconsistent files.",
    "tags": [
      "lakehouse"
    ]
  },
  {
    "name": "ACID Tables",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Atomicity, Consistency, Isolation, Durability for table operations on the lake.",
    "why": "Readers never see partial commits; concurrent writers coordinated.",
    "example": "A failed job leaves no half-visible partition after transactional commit.",
    "remember": [
      "File formats alone aren't ACID \u2014 table formats add it",
      "Isolation level still matters for readers"
    ],
    "mustKnow": null,
    "snippet": "-- prefer ACID table format ops (MERGE/OPTIMIZE)",
    "related": [
      "Compaction",
      "Vacuum",
      "Data Lake"
    ],
    "symptom": "Readers see partial or inconsistent files.",
    "tags": [
      "lakehouse"
    ]
  },
  {
    "name": "Compaction",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Rewrite many small files into larger optimized files.",
    "why": "Fixes small-file problem; improves scan speed.",
    "example": "Iceberg rewrite / Hudi compact / Delta OPTIMIZE.",
    "remember": [
      "Schedule regularly on hot tables",
      "Trade compute now for faster reads later"
    ],
    "mustKnow": null,
    "snippet": "OPTIMIZE delta.`/path/table`;  -- Delta\n-- Iceberg: rewrite_data_files",
    "related": [
      "Small File Problem",
      "Optimize",
      "Z-Ordering"
    ],
    "symptom": "Reads degrade daily as streaming adds tiny files.",
    "tags": [
      "lakehouse"
    ]
  },
  {
    "name": "Vacuum",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Delete obsolete data files no longer referenced by the table (past retention).",
    "why": "Reclaims storage; ends time-travel past retention.",
    "example": "`VACUUM table_name RETAIN 168 HOURS;` (Delta).",
    "remember": [
      "Too aggressive vacuum breaks time travel / long readers",
      "Coordinate retention with consumers"
    ],
    "mustKnow": null,
    "snippet": "-- prefer ACID table format ops (MERGE/OPTIMIZE)",
    "related": [
      "Optimize",
      "Z-Ordering",
      "Data Lake"
    ],
    "symptom": "Readers see partial or inconsistent files.",
    "tags": [
      "lakehouse"
    ]
  },
  {
    "name": "Optimize",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Maintenance command to compact/layout data for faster reads (Delta `OPTIMIZE`).",
    "why": "Combines small files; optional Z-Order clustering.",
    "example": "`OPTIMIZE sales ZORDER BY (customer_id, event_date);`",
    "remember": [
      "Run on tables with heavy small-file ingest",
      "Not free \u2014 schedule off-peak"
    ],
    "mustKnow": null,
    "snippet": "-- prefer ACID table format ops (MERGE/OPTIMIZE)",
    "related": [
      "Z-Ordering",
      "Data Lake"
    ],
    "symptom": "Readers see partial or inconsistent files.",
    "tags": [
      "lakehouse"
    ]
  },
  {
    "name": "Z-Ordering",
    "category": "Data Lake & Lakehouse",
    "categorySlug": "04-data-lake-lakehouse",
    "file": "04-data-lake-lakehouse/concepts.md",
    "definition": "Multi-dimensional clustering of data files so related values co-locate (minmax skipping improves).",
    "why": "Faster filters on high-cardinality columns that aren't partition keys.",
    "example": "Z-Order by `user_id` so point lookups skip most files.",
    "remember": [
      "Complement to partitioning, not a replacement",
      "Best for selective multi-column filters"
    ],
    "mustKnow": null,
    "snippet": "OPTIMIZE sales ZORDER BY (customer_id, event_date);",
    "related": [
      "Optimize",
      "Partition Pruning",
      "Delta Lake"
    ],
    "symptom": "Selective filters on non-partition columns still scan everything.",
    "tags": [
      "lakehouse"
    ]
  },
  {
    "name": "CAP Theorem",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "In a network partition, a system can provide Consistency or Availability, not both (Partition tolerance assumed in distributed DBs).",
    "why": "Forces explicit tradeoffs in data platforms.",
    "example": "During partition, CP system rejects writes; AP system accepts writes that may conflict.",
    "remember": [
      "Real systems tune the C/A tradeoff",
      "\"CA without P\" isn't realistic for multi-node"
    ],
    "mustKnow": 19,
    "snippet": "# In a partition: choose Consistency or Availability\n# Document the tradeoff for your store/queue",
    "related": [
      "Eventual Consistency",
      "Strong Consistency",
      "High Availability (HA)"
    ],
    "symptom": "Expecting strong reads from an AP system during a region blip.",
    "tags": [
      "distributed",
      "interview"
    ]
  },
  {
    "name": "BASE",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "Basically Available, Soft state, Eventually consistent \u2014 alternative mindset to strict ACID across distributed services.",
    "why": "Many large-scale stores and caches lean BASE.",
    "example": "Shopping cart eventually syncs across regions; brief stale reads OK.",
    "remember": [
      "Soft state may change without input (anti-entropy)",
      "Pair with application-level conflict handling"
    ],
    "mustKnow": null,
    "snippet": "# Basically Available, Soft state, Eventually consistent",
    "related": [
      "CAP Theorem",
      "Eventual Consistency",
      "Strong Consistency"
    ],
    "symptom": "Applying strict ACID expectations to a geo-replicated cache.",
    "tags": [
      "distributed"
    ]
  },
  {
    "name": "Eventual Consistency",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "If no new updates, all replicas converge to the same value eventually.",
    "why": "Default for many geo-replicated and highly available systems.",
    "example": "Read-after-write may fail briefly across regions; later reads agree.",
    "remember": [
      "Document staleness windows for consumers",
      "Use stronger reads when needed (quorum/linearizable APIs)"
    ],
    "mustKnow": null,
    "snippet": "# document C/A tradeoff for this store",
    "related": [
      "Strong Consistency",
      "Replication",
      "CAP Theorem"
    ],
    "symptom": "Surprising behavior under node/network failure.",
    "tags": [
      "distributed"
    ]
  },
  {
    "name": "Strong Consistency",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "After a write commits, all subsequent reads see it (linearizability / similar guarantees vary by system).",
    "why": "Simpler app logic; higher latency/cost.",
    "example": "Financial ledger balance must be strongly consistent.",
    "remember": [
      "Costs latency and availability under partitions",
      "Use where correctness > availability"
    ],
    "mustKnow": null,
    "snippet": "# document C/A tradeoff for this store",
    "related": [
      "Replication",
      "Leader Election",
      "CAP Theorem"
    ],
    "symptom": "Surprising behavior under node/network failure.",
    "tags": [
      "distributed"
    ]
  },
  {
    "name": "Replication",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "Keep multiple copies of data for durability and/or read scale.",
    "why": "Failure domains and read throughput.",
    "example": "Kafka topic RF=3; Postgres primary + replicas; S3 cross-region copy.",
    "remember": [
      "Sync vs async replication tradeoffs",
      "Replica lag affects read-your-writes"
    ],
    "mustKnow": null,
    "snippet": "# document C/A tradeoff for this store",
    "related": [
      "Leader Election",
      "Consensus",
      "CAP Theorem"
    ],
    "symptom": "Surprising behavior under node/network failure.",
    "tags": [
      "distributed"
    ]
  },
  {
    "name": "Leader Election",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "Cluster chooses a primary coordinator for writes or decisions.",
    "why": "Avoids split-brain conflicting writes.",
    "example": "ZooKeeper/etcd/Raft elect Kafka controller or DB primary.",
    "remember": [
      "Fencing tokens prevent old leaders writing",
      "Elections add brief unavailability"
    ],
    "mustKnow": null,
    "snippet": "# document C/A tradeoff for this store",
    "related": [
      "Consensus",
      "Raft",
      "CAP Theorem"
    ],
    "symptom": "Surprising behavior under node/network failure.",
    "tags": [
      "distributed"
    ]
  },
  {
    "name": "Consensus",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "Nodes agree on a value/order of operations despite failures.",
    "why": "Metadata, configs, and transaction logs need agreement.",
    "example": "Agreeing on the next committed Kafka controller epoch or Raft log entry.",
    "remember": [
      "Quorum majority required",
      "Expensive; keep consensus sets small"
    ],
    "mustKnow": null,
    "snippet": "# document C/A tradeoff for this store",
    "related": [
      "Raft",
      "Paxos",
      "CAP Theorem"
    ],
    "symptom": "Surprising behavior under node/network failure.",
    "tags": [
      "distributed"
    ]
  },
  {
    "name": "Raft",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "Consensus algorithm emphasizing understandability: leader, log replication, terms.",
    "why": "Used by etcd, Consul, many modern systems.",
    "example": "etcd stores K8s state via Raft.",
    "remember": [
      "Strong leader model",
      "Majority of voters must ack"
    ],
    "mustKnow": null,
    "snippet": "# document C/A tradeoff for this store",
    "related": [
      "Paxos",
      "Fault Tolerance",
      "CAP Theorem"
    ],
    "symptom": "Surprising behavior under node/network failure.",
    "tags": [
      "distributed"
    ]
  },
  {
    "name": "Paxos",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "Classic family of consensus protocols (Multi-Paxos, etc.).",
    "why": "Theoretical foundation; still in older systems (Chubby-like).",
    "example": "Google Chubby historically influenced by Paxos.",
    "remember": [
      "Harder to implement correctly than Raft",
      "Same goal: agree under failures"
    ],
    "mustKnow": null,
    "snippet": "# document C/A tradeoff for this store",
    "related": [
      "Fault Tolerance",
      "High Availability (HA)",
      "CAP Theorem"
    ],
    "symptom": "Surprising behavior under node/network failure.",
    "tags": [
      "distributed"
    ]
  },
  {
    "name": "Fault Tolerance",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "System continues correctly despite component failures.",
    "why": "Commodity hardware fails; jobs and stores must survive.",
    "example": "Spark recomputes lost partitions; Kafka replicates partitions.",
    "remember": [
      "Detect, isolate, recover",
      "Design for failure, not perfection"
    ],
    "mustKnow": null,
    "snippet": "# document C/A tradeoff for this store",
    "related": [
      "High Availability (HA)",
      "Horizontal Scaling",
      "CAP Theorem"
    ],
    "symptom": "Surprising behavior under node/network failure.",
    "tags": [
      "distributed"
    ]
  },
  {
    "name": "High Availability (HA)",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "Minimize downtime via redundancy and fast failover.",
    "why": "SLAs for pipelines and serving layers.",
    "example": "Multi-AZ brokers; standby scheduler; health checks + auto restart.",
    "remember": [
      "HA \u2260 zero data loss (depends on durability config)",
      "Measure MTTR and availability %"
    ],
    "mustKnow": null,
    "snippet": "# document C/A tradeoff for this store",
    "related": [
      "Horizontal Scaling",
      "Vertical Scaling",
      "CAP Theorem"
    ],
    "symptom": "Surprising behavior under node/network failure.",
    "tags": [
      "distributed"
    ]
  },
  {
    "name": "Horizontal Scaling",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "Add more nodes to increase capacity.",
    "why": "Default for big data systems.",
    "example": "Add Spark executors or Kafka brokers.",
    "remember": [
      "Needs partitionable workloads",
      "Shuffle/network can become bottleneck"
    ],
    "mustKnow": null,
    "snippet": "# document C/A tradeoff for this store",
    "related": [
      "Vertical Scaling",
      "CAP Theorem"
    ],
    "symptom": "Surprising behavior under node/network failure.",
    "tags": [
      "distributed"
    ]
  },
  {
    "name": "Vertical Scaling",
    "category": "Distributed Systems",
    "categorySlug": "05-distributed-systems",
    "file": "05-distributed-systems/concepts.md",
    "definition": "Give one machine more CPU/RAM/disk.",
    "why": "Simple until limits/cost/HA constraints hit.",
    "example": "Upsize a warehouse warehouse node or driver machine.",
    "remember": [
      "Ceiling exists; single point of failure risk",
      "Mix: scale out data plane, scale up hot coordinators carefully"
    ],
    "mustKnow": null,
    "snippet": "# document C/A tradeoff for this store",
    "related": [
      "CAP Theorem",
      "BASE"
    ],
    "symptom": "Surprising behavior under node/network failure.",
    "tags": [
      "distributed"
    ]
  },
  {
    "name": "Event Time",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Timestamp when the event actually occurred in the real world.",
    "why": "Correct windows despite late/out-of-order arrival.",
    "example": "Click happened at 10:00; arrived at 10:07 \u2192 event time = 10:00.",
    "remember": [
      "Prefer event time for analytics windows",
      "Requires watermarks for progress"
    ],
    "mustKnow": 20,
    "snippet": "df.withWatermark(\"event_time\", \"10 minutes\") \\\n  .groupBy(window(\"event_time\", \"1 hour\")).count()",
    "related": [
      "Watermarks",
      "Processing Time",
      "Late Data"
    ],
    "symptom": "Hourly metrics shift when late mobile events arrive.",
    "tags": [
      "interview",
      "streaming"
    ]
  },
  {
    "name": "Processing Time",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Time on the machine when the event is processed.",
    "why": "Simple but wrong for delayed data.",
    "example": "Same click processed at 10:07 \u2192 processing-time window uses 10:07.",
    "remember": [
      "OK for monitoring ops latency",
      "Bad for business \"hourly sales\""
    ],
    "mustKnow": null,
    "snippet": ".withWatermark('event_time', '10 minutes')",
    "related": [
      "Ingestion Time",
      "Watermarks",
      "Event Time"
    ],
    "symptom": "Results change as late events arrive.",
    "tags": [
      "streaming"
    ]
  },
  {
    "name": "Ingestion Time",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Time when the event enters the streaming system.",
    "why": "Middle ground; stable clock at entry, not true event time.",
    "example": "Kafka timestamp at produce/consume (depending on config).",
    "remember": [
      "Better than processing time for some cases",
      "Still not true business event time"
    ],
    "mustKnow": null,
    "snippet": ".withWatermark('event_time', '10 minutes')",
    "related": [
      "Watermarks",
      "Windowing",
      "Event Time"
    ],
    "symptom": "Results change as late events arrive.",
    "tags": [
      "streaming"
    ]
  },
  {
    "name": "Watermarks",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Engine's notion of how far event time has advanced; used to close windows.",
    "why": "Trade completeness vs latency for late data.",
    "example": "Watermark = max_event_time - 10 minutes \u2192 close hour window when watermark passes end + grace.",
    "remember": [
      "Too tight \u2192 drop late events",
      "Too loose \u2192 high latency/state"
    ],
    "mustKnow": 21,
    "snippet": ".withWatermark(\"event_time\", \"15 minutes\")\n# closes windows; late data policy after this",
    "related": [
      "Event Time",
      "Late Data",
      "Windowing"
    ],
    "symptom": "State grows forever, or late events are dropped too aggressively.",
    "tags": [
      "interview",
      "streaming"
    ]
  },
  {
    "name": "Windowing",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Group stream events into finite buckets: tumbling, sliding, session.",
    "why": "Aggregations need bounded scopes.",
    "example": "Tumbling 5-min count of orders; session window by user inactivity gap.",
    "remember": [
      "Tumbling: fixed, non-overlap",
      "Sliding: overlap; session: activity-based"
    ],
    "mustKnow": null,
    "snippet": ".withWatermark('event_time', '10 minutes')",
    "related": [
      "Late Data",
      "Stateful Processing",
      "Event Time"
    ],
    "symptom": "Results change as late events arrive.",
    "tags": [
      "streaming"
    ]
  },
  {
    "name": "Late Data",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Events arriving after the watermark/window allowed them.",
    "why": "Reality of mobile/offline producers.",
    "example": "Event time 10:01 arrives at 10:25 with 5-min watermark \u2014 late.",
    "remember": [
      "Side outputs / allowed lateness / update previous results",
      "Decide drop vs correct"
    ],
    "mustKnow": null,
    "snippet": ".withWatermark('event_time', '10 minutes')",
    "related": [
      "Stateful Processing",
      "Stateless Processing",
      "Event Time"
    ],
    "symptom": "Results change as late events arrive.",
    "tags": [
      "streaming"
    ]
  },
  {
    "name": "Stateful Processing",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Operators keep state across events (windows, joins, dedupe).",
    "why": "Enables real aggregations; needs checkpointing and TTL.",
    "example": "Running count per user; stream-stream join.",
    "remember": [
      "State grows \u2014 set TTL",
      "Checkpoint state for exactly-once"
    ],
    "mustKnow": null,
    "snippet": ".withWatermark('event_time', '10 minutes')",
    "related": [
      "Stateless Processing",
      "Exactly Once",
      "Event Time"
    ],
    "symptom": "Results change as late events arrive.",
    "tags": [
      "streaming"
    ]
  },
  {
    "name": "Stateless Processing",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Each event handled independently; no cross-event state.",
    "why": "Easier scale and recovery.",
    "example": "Map parse JSON \u2192 enrich from broadcast config \u2192 filter.",
    "remember": [
      "Prefer when possible",
      "Filter/map/flatMap style ops"
    ],
    "mustKnow": null,
    "snippet": ".withWatermark('event_time', '10 minutes')",
    "related": [
      "Exactly Once",
      "At Least Once",
      "Event Time"
    ],
    "symptom": "Results change as late events arrive.",
    "tags": [
      "streaming"
    ]
  },
  {
    "name": "Exactly Once",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Effect of each record applied once end-to-end (despite retries).",
    "why": "No double-counting in sinks.",
    "example": "Kafka \u2192 Flink \u2192 transactional sink / idempotent sink with checkpoints.",
    "remember": [
      "Needs idempotent or transactional sinks",
      "\"Exactly once\" is end-to-end, not just processing"
    ],
    "mustKnow": 22,
    "snippet": "# Need checkpointing + idempotent/transactional sink\nquery = df.writeStream.option(\"checkpointLocation\", ckpt).foreachBatch(...)",
    "related": [
      "At Least Once",
      "Idempotency",
      "Checkpointing"
    ],
    "symptom": "Duplicates after restart even though processing \"felt\" safe.",
    "tags": [
      "interview",
      "pipeline",
      "streaming"
    ]
  },
  {
    "name": "At Least Once",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Every record processed \u22651 time; duplicates possible on retry.",
    "why": "Common default; simpler than exactly-once.",
    "example": "Restart after crash reprocesses last uncommitted batch.",
    "remember": [
      "Pair with idempotent writes",
      "Often \"good enough\" with dedupe keys"
    ],
    "mustKnow": 23,
    "snippet": "# retries may re-send \u2014 make sink upsert/idempotent\n# ON CONFLICT (id) DO UPDATE ...",
    "related": [
      "Exactly Once",
      "Idempotency",
      "Dead Letter Queue (DLQ)"
    ],
    "symptom": "Counts inflate after every pipeline retry.",
    "tags": [
      "interview",
      "pipeline",
      "streaming"
    ]
  },
  {
    "name": "At Most Once",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Process \u22641 time; may lose data on failure.",
    "why": "Lowest latency/complexity; rare for critical pipelines.",
    "example": "Fire metrics where loss is acceptable.",
    "remember": [
      "Don't use for money/inventory",
      "Explicit tradeoff: loss over duplicates"
    ],
    "mustKnow": null,
    "snippet": ".withWatermark('event_time', '10 minutes')",
    "related": [
      "Event Sourcing",
      "CQRS",
      "Event Time"
    ],
    "symptom": "Results change as late events arrive.",
    "tags": [
      "streaming"
    ]
  },
  {
    "name": "Event Sourcing",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Persist state as an append-only sequence of events; rebuild state by replay.",
    "why": "Auditability and temporal queries.",
    "example": "`OrderPlaced`, `ItemAdded`, `OrderShipped` rebuild current order.",
    "remember": [
      "Source of truth = event log",
      "Snapshots speed replay"
    ],
    "mustKnow": null,
    "snippet": "# append OrderPlaced, ItemAdded... rebuild state by replay",
    "related": [
      "CQRS",
      "Exactly Once",
      "Idempotency"
    ],
    "symptom": "Overwriting current state and losing audit history.",
    "tags": [
      "pipeline",
      "streaming"
    ]
  },
  {
    "name": "CQRS",
    "category": "Streaming",
    "categorySlug": "06-streaming",
    "file": "06-streaming/concepts.md",
    "definition": "Command Query Responsibility Segregation \u2014 separate write model from read model(s).",
    "why": "Scale reads/writes independently; tailor projections.",
    "example": "Write orders to OLTP; project read-optimized order search index.",
    "remember": [
      "Often paired with event sourcing",
      "Read models can be eventually consistent"
    ],
    "mustKnow": null,
    "snippet": ".withWatermark('event_time', '10 minutes')",
    "related": [
      "Event Time",
      "Processing Time"
    ],
    "symptom": "Results change as late events arrive.",
    "tags": [
      "pipeline",
      "streaming"
    ]
  },
  {
    "name": "Idempotency",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Running the same operation multiple times yields the same result as running once.",
    "why": "Retries and backfills won't duplicate or corrupt data.",
    "example": "Upsert by primary key; overwrite partition `dt=2024-01-01` instead of append-only blindly.",
    "remember": [
      "Design sinks for safe replay",
      "Natural keys + MERGE / partition overwrite"
    ],
    "mustKnow": 24,
    "snippet": "# overwrite a partition instead of blind append\ndf.write.mode(\"overwrite\").partitionBy(\"dt\").parquet(path)",
    "related": [
      "Checkpointing",
      "Exactly Once",
      "Data Contracts"
    ],
    "symptom": "Re-running a failed day duplicates rows in the target.",
    "tags": [
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "Checkpointing",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Persist progress (offsets/state) so recovery resumes cleanly.",
    "why": "Fault-tolerant streaming and long batch stages.",
    "example": "Spark Structured Streaming checkpoint location stores offsets + state.",
    "remember": [
      "Checkpoint path is critical state \u2014 back it up/protect it",
      "Changing query identity may invalidate checkpoints"
    ],
    "mustKnow": 25,
    "snippet": "writeStream.option(\"checkpointLocation\", \"s3://ops/checkpoints/job_a\")",
    "related": [
      "Idempotency",
      "Exactly Once",
      "Fault Tolerance"
    ],
    "symptom": "Streaming job reprocesses from earliest offsets after path loss.",
    "tags": [
      "interview",
      "pipeline",
      "streaming"
    ]
  },
  {
    "name": "Retry Strategy",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Policy for re-attempting failed work (how many, when, which errors).",
    "why": "Transient cloud/network faults are normal.",
    "example": "Retry S3/API 429/5xx up to 5 times; fail fast on 400 auth errors.",
    "remember": [
      "Distinguish transient vs permanent errors",
      "Cap retries to avoid thundering herds"
    ],
    "mustKnow": null,
    "snippet": "# make reruns safe: upsert / partition overwrite",
    "related": [
      "Backoff Strategy",
      "Circuit Breaker",
      "Idempotency"
    ],
    "symptom": "Reruns corrupt or duplicate outputs.",
    "tags": [
      "pipeline"
    ]
  },
  {
    "name": "Backoff Strategy",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Increase wait between retries (often exponential + jitter).",
    "why": "Prevents stampeding a recovering dependency.",
    "example": "Wait 1s, 2s, 4s, 8s with random jitter.",
    "remember": [
      "Always add jitter",
      "Set max backoff"
    ],
    "mustKnow": null,
    "snippet": "# make reruns safe: upsert / partition overwrite",
    "related": [
      "Circuit Breaker",
      "Dead Letter Queue (DLQ)",
      "Idempotency"
    ],
    "symptom": "Reruns corrupt or duplicate outputs.",
    "tags": [
      "pipeline"
    ]
  },
  {
    "name": "Circuit Breaker",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Stop calling a failing dependency temporarily after error threshold; probe later.",
    "why": "Fails fast; protects both sides from cascade.",
    "example": "After 50 warehouse connection failures, open circuit for 60s, then half-open trial.",
    "remember": [
      "Use for shared DBs/APIs",
      "Pair with timeouts"
    ],
    "mustKnow": null,
    "snippet": "# after N failures \u2192 open circuit \u2192 fail fast \u2192 half-open probe",
    "related": [
      "Retry Strategy",
      "Backoff Strategy",
      "Dead Letter Queue (DLQ)"
    ],
    "symptom": "Retries hammer a down warehouse and cascade timeouts.",
    "tags": [
      "pipeline"
    ]
  },
  {
    "name": "Dead Letter Queue (DLQ)",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Side channel for records that fail processing after retries.",
    "why": "Unblocks the main pipeline; enables quarantine and replay.",
    "example": "Malformed JSON \u2192 DLQ topic/table with error reason; alert and fix.",
    "remember": [
      "Monitor DLQ depth",
      "Have a replay runbook"
    ],
    "mustKnow": 26,
    "snippet": "try:\n    process(record)\nexcept BadRecord as e:\n    dlq.send(record, error=str(e))",
    "related": [
      "Retry Strategy",
      "Data Validation",
      "Data Contracts"
    ],
    "symptom": "One poison message blocks the whole consumer group.",
    "tags": [
      "interview",
      "pipeline",
      "streaming"
    ]
  },
  {
    "name": "Data Contracts",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Explicit agreement on schema, semantics, SLAs, and ownership between producers and consumers.",
    "why": "Prevents silent breaking changes.",
    "example": "Producer guarantees `orders.v2` fields + daily freshness < 2h; CI validates schema.",
    "remember": [
      "Version schemas",
      "Enforce in CI/CD and at write time"
    ],
    "mustKnow": 27,
    "snippet": "# CI: fail PR if producer schema breaks consumer contract\n# enforce required fields + types at write boundary",
    "related": [
      "Schema Enforcement",
      "Schema Evolution",
      "Data Validation"
    ],
    "symptom": "Silent column rename breaks 12 downstream jobs overnight.",
    "tags": [
      "interview",
      "pipeline",
      "quality"
    ]
  },
  {
    "name": "Cron Expression",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Compact schedule string (usually 5 fields: minute hour day-of-month month day-of-week) that says when a job should fire.",
    "why": "Still the lingua franca for schedules \u2014 OS crontab, Airflow, K8s CronJob, cloud schedulers all speak cron-ish dialects.",
    "example": "`0 6 * * 1-5` = 06:00 every weekday; `*/15 * * * *` = every 15 minutes.",
    "remember": [
      "Field order: minute \u2192 hour \u2192 day \u2192 month \u2192 weekday (seconds field is a non-standard 6th)",
      "`*` = every; `*/n` = every n; lists and ranges vary slightly by tool"
    ],
    "mustKnow": null,
    "snippet": "# min hour dom month dow\n0 6 * * 1-5   # 06:00 weekdays\n*/15 * * * * # every 15 min",
    "related": [
      "Crontab / Cron Job",
      "Schedule / Timetable",
      "Cron Timezone Pitfalls"
    ],
    "symptom": "Job fires at the wrong time or dialect (5 vs 6 fields) is misread.",
    "tags": [
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "Crontab / Cron Job",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "OS or host-level job launcher that runs a command when a cron expression matches (classic `/etc/crontab` or user crontab).",
    "why": "Fine for one machine / one script; weak for dependencies, retries, observability, and multi-step pipelines.",
    "example": "`0 2 * * * /opt/etl/load_orders.sh >> /var/log/orders.log 2>&1`",
    "remember": [
      "No DAG: jobs don\u2019t know about upstream readiness",
      "Overlaps, silent failures, and timezone bugs are common",
      "Prefer an orchestrator once you have dependencies or SLAs"
    ],
    "mustKnow": null,
    "snippet": "0 2 * * * /opt/etl/load_orders.sh >> /var/log/orders.log 2>&1",
    "related": [
      "Cron Expression",
      "Pipeline Orchestration",
      "Cron vs Orchestrator"
    ],
    "symptom": "Scripts race with no dependency graph or silent log failures.",
    "tags": [
      "devops",
      "pipeline"
    ]
  },
  {
    "name": "Cron Timezone Pitfalls",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Cron fires in the scheduler\u2019s timezone (often UTC on servers); DST and \u201clocal business midnight\u201d confuse partition dates.",
    "why": "Wrong tz \u2192 wrong `dt=` partition, missed business-day runs, or double runs around DST.",
    "example": "Job meant for \u201cUS Eastern midnight\u201d scheduled as `0 0 * * *` on a UTC host runs at 19:00 ET previous day.",
    "remember": [
      "Prefer UTC everywhere and convert for business calendars explicitly",
      "Document which clock the schedule uses",
      "Airflow: set DAG timezone deliberately"
    ],
    "mustKnow": null,
    "snippet": "# Prefer UTC on servers; map to business calendar in code\n# 0 0 * * * on UTC \u2260 Eastern midnight",
    "related": [
      "Cron Expression",
      "Execution Date / Data Interval",
      "Freshness"
    ],
    "symptom": "Wrong partition date or missed weekday runs around DST.",
    "tags": [
      "pipeline"
    ]
  },
  {
    "name": "Overlapping Cron Runs",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Next scheduled start begins while the previous run is still executing (slow job + frequent cron).",
    "why": "Duplicate writes, lock fights, or corrupted partial loads unless the job is idempotent and concurrency-safe.",
    "example": "Hourly job still running at :00 next hour \u2192 two loaders append the same partition.",
    "remember": [
      "Use locks (`flock`), `max_active_runs=1`, or skip-if-running",
      "Make writes idempotent even when overlaps happen",
      "Lengthen the interval or speed up the job"
    ],
    "mustKnow": null,
    "snippet": "# flock -n /tmp/load.lock -c '/opt/etl/load.sh'\n# or orchestrator max_active_runs=1",
    "related": [
      "Idempotency",
      "Crontab / Cron Job",
      "Retry Strategy"
    ],
    "symptom": "Two copies of the same hourly job write at once.",
    "tags": [
      "pipeline"
    ]
  },
  {
    "name": "Cron vs Orchestrator",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Cron starts a command on a clock; an orchestrator (Airflow/Dagster/etc.) manages DAGs, sensors, retries, backfill, and lineage of runs.",
    "why": "Clock \u2260 data ready. Blind cron races upstream; orchestrators can wait on files/datasets and recover cleanly.",
    "example": "Cron: always start at 6am. Orchestrator: sensor waits for `_SUCCESS`, then transform \u2192 test \u2192 publish.",
    "remember": [
      "Cron for simple single-host tasks; orchestrator for production pipelines",
      "Dataset/sensor triggers beat fixed cron when latency varies"
    ],
    "mustKnow": null,
    "snippet": "cron = clock start\norchestrator = deps + retries + backfill + sensors",
    "related": [
      "Pipeline Orchestration",
      "Sensor",
      "Schedule / Timetable",
      "Crontab / Cron Job"
    ],
    "symptom": "Fixed 6am job runs before upstream file arrives.",
    "tags": [
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "Pipeline Orchestration",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Schedule, dependency management, retries, and observability of multi-step data jobs.",
    "why": "Production reliability beyond a single script.",
    "example": "Airflow/Dagster/Prefect/Azure DF: extract \u2192 transform \u2192 test \u2192 publish.",
    "remember": [
      "Prefer data-aware sensors over blind cron when possible",
      "Idempotent tasks + clear SLAs"
    ],
    "mustKnow": null,
    "snippet": "# make reruns safe: upsert / partition overwrite",
    "related": [
      "Metadata-Driven Pipelines",
      "Idempotency",
      "Cron vs Orchestrator",
      "Cron Expression"
    ],
    "symptom": "Reruns corrupt or duplicate outputs.",
    "tags": [
      "pipeline"
    ]
  },
  {
    "name": "Metadata-Driven Pipelines",
    "category": "Pipeline Design",
    "categorySlug": "07-pipeline-design",
    "file": "07-pipeline-design/concepts.md",
    "definition": "Behavior configured by metadata (tables, mappings, rules) rather than hard-coded per table jobs.",
    "why": "Scales to hundreds of similar pipelines.",
    "example": "Config table lists source, target, PK, SCD type \u2192 one generic loader.",
    "remember": [
      "Great for ingestion patterns",
      "Keep escape hatches for special cases"
    ],
    "mustKnow": null,
    "snippet": "# make reruns safe: upsert / partition overwrite",
    "related": [
      "Idempotency",
      "Checkpointing"
    ],
    "symptom": "Reruns corrupt or duplicate outputs.",
    "tags": [
      "pipeline"
    ]
  },
  {
    "name": "Data Skew",
    "category": "Performance Tuning",
    "categorySlug": "08-performance-tuning",
    "file": "08-performance-tuning/concepts.md",
    "definition": "Uneven key/partition sizes so a few tasks do most of the work.",
    "why": "Stage time \u2248 slowest task.",
    "example": "`null` or popular `country_code` holds 50% of join keys.",
    "remember": [
      "Detect via task duration histograms",
      "Fix with salting, AQE skew join, or pre-agg"
    ],
    "mustKnow": 13,
    "snippet": "df.groupBy(\"key\").count().orderBy(desc(\"count\")).show(20)\n# then salt hot keys or enable AQE skew join",
    "related": [
      "Salting",
      "Skew Join Optimization",
      "Shuffle"
    ],
    "symptom": "One task runs 10\u00d7 longer than the rest of the stage.",
    "tags": [
      "interview",
      "performance",
      "spark"
    ]
  },
  {
    "name": "Broadcast Join",
    "category": "Performance Tuning",
    "categorySlug": "08-performance-tuning",
    "file": "08-performance-tuning/concepts.md",
    "definition": "Replicate the small side to all executors; join locally (map-side).",
    "why": "Avoids shuffling the large side.",
    "example": "Fact \u00d7 small dimension (`broadcast(dim)`).",
    "remember": [
      "Small side must fit memory",
      "Best default for star-schema dims"
    ],
    "mustKnow": 11,
    "snippet": "from pyspark.sql.functions import broadcast\nfact.join(broadcast(dim), \"id\")",
    "related": [
      "Sort Merge Join",
      "Data Skew",
      "Adaptive Query Execution (AQE)"
    ],
    "symptom": "Shuffling a tiny dimension across the cluster.",
    "tags": [
      "interview",
      "performance",
      "spark"
    ]
  },
  {
    "name": "Shuffle Join",
    "category": "Performance Tuning",
    "categorySlug": "08-performance-tuning",
    "file": "08-performance-tuning/concepts.md",
    "definition": "Generic term for joins that redistribute both sides by join key (often sort-merge or shuffle-hash).",
    "why": "Expensive but necessary for two large tables.",
    "example": "Large `orders` \u22c8 large `payments` on `order_id`.",
    "remember": [
      "Ensure join keys have good distribution",
      "Filter early before shuffle"
    ],
    "mustKnow": null,
    "snippet": "# check Spark UI: skew, spill, shuffle",
    "related": [
      "Sort Merge Join",
      "Hash Join",
      "Data Skew"
    ],
    "symptom": "Job runtime much worse than data size suggests.",
    "tags": [
      "performance",
      "spark"
    ]
  },
  {
    "name": "Sort Merge Join",
    "category": "Performance Tuning",
    "categorySlug": "08-performance-tuning",
    "file": "08-performance-tuning/concepts.md",
    "definition": "Shuffle both sides by key, sort, then merge \u2014 Spark's common large-large join.",
    "why": "Scalable; sensitive to skew and spill.",
    "example": "Default for two big DataFrames when broadcast isn't chosen.",
    "remember": [
      "Watch sort spill",
      "AQE may switch strategy"
    ],
    "mustKnow": 12,
    "snippet": "# default large-large join after shuffle+sort\nbig.join(other_big, \"user_id\")",
    "related": [
      "Broadcast Join",
      "Shuffle",
      "Spill to Disk"
    ],
    "symptom": "Join spills to disk; one key's partition is huge.",
    "tags": [
      "interview",
      "performance",
      "spark"
    ]
  },
  {
    "name": "Hash Join",
    "category": "Performance Tuning",
    "categorySlug": "08-performance-tuning",
    "file": "08-performance-tuning/concepts.md",
    "definition": "Build hash table on one side, probe with the other (in-memory or spilled).",
    "why": "Fast when build side fits (or partially).",
    "example": "Broadcast hash join = hash join with replicated build side.",
    "remember": [
      "Build = smaller side ideally",
      "OOM if build side underestimated"
    ],
    "mustKnow": null,
    "snippet": "# check Spark UI: skew, spill, shuffle",
    "related": [
      "Bucket Join",
      "Salting",
      "Data Skew"
    ],
    "symptom": "Job runtime much worse than data size suggests.",
    "tags": [
      "performance",
      "spark"
    ]
  },
  {
    "name": "Bucket Join",
    "category": "Performance Tuning",
    "categorySlug": "08-performance-tuning",
    "file": "08-performance-tuning/concepts.md",
    "definition": "Tables pre-bucketed (and often sorted) on the same key so joins avoid full shuffle.",
    "why": "Speeds repeated joins on stable keys.",
    "example": "Both tables bucketed by `user_id` into 200 buckets \u2192 bucket join.",
    "remember": [
      "Bucket count must match (or be compatible)",
      "Worth it for heavy reuse patterns"
    ],
    "mustKnow": null,
    "snippet": "# check Spark UI: skew, spill, shuffle",
    "related": [
      "Salting",
      "Caching",
      "Data Skew"
    ],
    "symptom": "Job runtime much worse than data size suggests.",
    "tags": [
      "performance",
      "spark"
    ]
  },
  {
    "name": "Salting",
    "category": "Performance Tuning",
    "categorySlug": "08-performance-tuning",
    "file": "08-performance-tuning/concepts.md",
    "definition": "Add a random salt to hot keys to spread them across partitions, then remove/aggregate.",
    "why": "Manual skew fix when AQE isn't enough.",
    "example": "Hot `customer_id` \u2192 `customer_id + salt_0..N` on both sides for join, then drop salt.",
    "remember": [
      "Increases rows temporarily",
      "Choose salt range from skew severity"
    ],
    "mustKnow": null,
    "snippet": "hot = df.withColumn(\"salt\", (rand()*N).cast(\"int\"))\n# join on key+salt, then drop salt",
    "related": [
      "Data Skew",
      "Skew Join Optimization",
      "Broadcast Join"
    ],
    "symptom": "AQE still leaves one monstrous partition on a hot key.",
    "tags": [
      "performance",
      "spark"
    ]
  },
  {
    "name": "Caching",
    "category": "Performance Tuning",
    "categorySlug": "08-performance-tuning",
    "file": "08-performance-tuning/concepts.md",
    "definition": "Keep a dataset in memory/disk across actions to avoid recomputation.",
    "why": "Iterative algorithms and multi-action reuse.",
    "example": "`df.cache()` before multiple joins/aggregations on same base.",
    "remember": [
      "Unpersist when done",
      "Cache only reused expensive stages"
    ],
    "mustKnow": null,
    "snippet": "df.cache()\ndf.count()  # materialize\n# ... reuse df ...\ndf.unpersist()",
    "related": [
      "Persistence Levels",
      "Memory Management",
      "Lazy Evaluation"
    ],
    "symptom": "Same expensive subtree recomputed on every action.",
    "tags": [
      "performance",
      "spark"
    ]
  },
  {
    "name": "Persistence Levels",
    "category": "Performance Tuning",
    "categorySlug": "08-performance-tuning",
    "file": "08-performance-tuning/concepts.md",
    "definition": "Storage level for cached data: memory only, memory+disk, disk only, serialized, replicated.",
    "why": "Trade speed vs memory pressure vs reliability.",
    "example": "`MEMORY_AND_DISK` safer than `MEMORY_ONLY` for large frames.",
    "remember": [
      "DataFrames: `cache` \u2248 MEMORY_AND_DISK",
      "Replication costs memory"
    ],
    "mustKnow": null,
    "snippet": "# check Spark UI: skew, spill, shuffle",
    "related": [
      "Parallelism",
      "Partition Sizing",
      "Data Skew"
    ],
    "symptom": "Job runtime much worse than data size suggests.",
    "tags": [
      "performance",
      "spark"
    ]
  },
  {
    "name": "Parallelism",
    "category": "Performance Tuning",
    "categorySlug": "08-performance-tuning",
    "file": "08-performance-tuning/concepts.md",
    "definition": "Degree of concurrent tasks (related to partitions and cores).",
    "why": "Too low = idle cluster; too high = scheduling overhead.",
    "example": "`repartition(200)` to use ~200 cores effectively for a stage.",
    "remember": [
      "Target a few partitions per core",
      "Aim for reasonable partition file sizes"
    ],
    "mustKnow": null,
    "snippet": "# check Spark UI: skew, spill, shuffle",
    "related": [
      "Partition Sizing",
      "Small File Problem",
      "Data Skew"
    ],
    "symptom": "Job runtime much worse than data size suggests.",
    "tags": [
      "performance",
      "spark"
    ]
  },
  {
    "name": "Partition Sizing",
    "category": "Performance Tuning",
    "categorySlug": "08-performance-tuning",
    "file": "08-performance-tuning/concepts.md",
    "definition": "Choosing number/size of partitions (and output files) for balanced work.",
    "why": "Core lever for Spark/Hive performance.",
    "example": "Target ~128\u2013256 MB per partition file for many lake workloads (rule of thumb, not law).",
    "remember": [
      "Too small \u2192 small files",
      "Too large \u2192 spill/OOM risk"
    ],
    "mustKnow": null,
    "snippet": "# check Spark UI: skew, spill, shuffle",
    "related": [
      "Small File Problem",
      "Data Skew"
    ],
    "symptom": "Job runtime much worse than data size suggests.",
    "tags": [
      "performance",
      "spark"
    ]
  },
  {
    "name": "Small File Problem",
    "category": "Performance Tuning",
    "categorySlug": "08-performance-tuning",
    "file": "08-performance-tuning/concepts.md",
    "definition": "Too many tiny files \u2192 heavy listing, task overhead, slow queries.",
    "why": "Streaming/micro-batch ingest often creates this.",
    "example": "100k files of 1 MB instead of 400 files of 256 MB.",
    "remember": [
      "Compact regularly",
      "Coalesce/repartition before write; avoid over-partitioning"
    ],
    "mustKnow": 14,
    "snippet": "df.coalesce(64).write.mode(\"overwrite\").parquet(path)\n# or OPTIMIZE / compaction on lakehouse tables",
    "related": [
      "Compaction",
      "Optimize",
      "Partition Sizing"
    ],
    "symptom": "Driver spends ages listing; thousands of tiny tasks.",
    "tags": [
      "interview",
      "lakehouse",
      "performance",
      "spark"
    ]
  },
  {
    "name": "Data Profiling",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "Statistical survey of a dataset: nulls, distincts, distributions, patterns.",
    "why": "Baseline before modeling and for anomaly detection.",
    "example": "12% null `email`; `amount` min/max/p99; top values of `status`.",
    "remember": [
      "Profile new sources early",
      "Re-profile after major changes"
    ],
    "mustKnow": null,
    "snippet": "# assert freshness + uniqueness before publish",
    "related": [
      "Data Validation",
      "Data Lineage"
    ],
    "symptom": "Consumers distrust the numbers.",
    "tags": [
      "quality"
    ]
  },
  {
    "name": "Data Validation",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "Automated checks that data meets rules (schema, ranges, uniqueness, referential).",
    "why": "Catch bad data before it hits consumers.",
    "example": "Great Expectations / dbt tests: `order_id` unique, `amount >= 0`, FK to customers.",
    "remember": [
      "Fail or quarantine on critical rules",
      "Warn on softer anomalies"
    ],
    "mustKnow": null,
    "snippet": "# assert freshness + uniqueness before publish",
    "related": [
      "Data Lineage",
      "Metadata",
      "Data Profiling"
    ],
    "symptom": "Consumers distrust the numbers.",
    "tags": [
      "quality"
    ]
  },
  {
    "name": "Data Lineage",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "Trace where data came from and where it flows (table/column/job level).",
    "why": "Impact analysis, debugging, compliance.",
    "example": "`raw.orders` \u2192 `stg_orders` \u2192 `fct_orders` \u2192 dashboard KPI.",
    "remember": [
      "Column-level lineage is gold for audits",
      "Automate from orchestrator/SQL parsers/catalog"
    ],
    "mustKnow": 28,
    "snippet": "# raw.orders \u2192 stg_orders \u2192 fct_orders \u2192 BI KPI\n# track table + column lineage in the catalog",
    "related": [
      "Metadata",
      "Data Catalog",
      "Observability"
    ],
    "symptom": "Can't tell which dashboard breaks if a source column is dropped.",
    "tags": [
      "interview",
      "quality"
    ]
  },
  {
    "name": "Metadata",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "Data about data: schema, owners, freshness, partitions, descriptions.",
    "why": "Discoverability and safe use.",
    "example": "Table description, column glossary, last update time, PII tags.",
    "remember": [
      "Treat metadata as a product",
      "Stale metadata is worse than none if trusted blindly"
    ],
    "mustKnow": null,
    "snippet": "# assert freshness + uniqueness before publish",
    "related": [
      "Data Catalog",
      "Master Data Management (MDM)",
      "Data Profiling"
    ],
    "symptom": "Consumers distrust the numbers.",
    "tags": [
      "quality"
    ]
  },
  {
    "name": "Data Catalog",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "Searchable inventory of datasets with metadata, ownership, and often lineage.",
    "why": "\"Where is customer revenue defined?\"",
    "example": "DataHub, Collibra, Unity Catalog, Glue Data Catalog, OpenMetadata.",
    "remember": [
      "Integrate with ingestion + BI",
      "Ownership + certification badges help trust"
    ],
    "mustKnow": null,
    "snippet": "# assert freshness + uniqueness before publish",
    "related": [
      "Master Data Management (MDM)",
      "Data Governance",
      "Data Profiling"
    ],
    "symptom": "Consumers distrust the numbers.",
    "tags": [
      "quality"
    ]
  },
  {
    "name": "Master Data Management (MDM)",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "Processes/systems to create a single trusted master for core entities (customer, product).",
    "why": "Deduped golden records across systems.",
    "example": "Merge CRM + billing customers into golden `customer_id` with survivorship rules.",
    "remember": [
      "Matching + survivorship + stewardship",
      "Hard organizationally, not just technically"
    ],
    "mustKnow": null,
    "snippet": "# assert freshness + uniqueness before publish",
    "related": [
      "Data Governance",
      "Data Stewardship",
      "Data Profiling"
    ],
    "symptom": "Consumers distrust the numbers.",
    "tags": [
      "quality"
    ]
  },
  {
    "name": "Data Governance",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "Policies, roles, and controls for data quality, security, privacy, and usage.",
    "why": "Trust, compliance (GDPR etc.), consistent definitions.",
    "example": "PII classification, access reviews, approved metric definitions.",
    "remember": [
      "People + process + platform",
      "Embed in pipelines, not only documents"
    ],
    "mustKnow": null,
    "snippet": "# assert freshness + uniqueness before publish",
    "related": [
      "Data Stewardship",
      "Observability",
      "Data Profiling"
    ],
    "symptom": "Consumers distrust the numbers.",
    "tags": [
      "quality"
    ]
  },
  {
    "name": "Data Stewardship",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "Human ownership of domain data quality and definitions.",
    "why": "Governance without stewards doesn't stick.",
    "example": "Finance steward owns `revenue` definition and approves changes.",
    "remember": [
      "Assign stewards per domain",
      "Stewards \u2260 only platform engineers"
    ],
    "mustKnow": null,
    "snippet": "# assert freshness + uniqueness before publish",
    "related": [
      "Observability",
      "Freshness",
      "Data Profiling"
    ],
    "symptom": "Consumers distrust the numbers.",
    "tags": [
      "quality"
    ]
  },
  {
    "name": "Observability",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "Ability to understand pipeline health via metrics, logs, traces, and data checks.",
    "why": "Detect silent data failures, not just job crashes.",
    "example": "Freshness, row-count anomalies, schema drift alerts, job duration SLOs.",
    "remember": [
      "Observe data + compute",
      "Alert on symptoms that matter to consumers"
    ],
    "mustKnow": null,
    "snippet": "# assert freshness + uniqueness before publish",
    "related": [
      "Freshness",
      "Completeness",
      "Data Profiling"
    ],
    "symptom": "Consumers distrust the numbers.",
    "tags": [
      "quality"
    ]
  },
  {
    "name": "Freshness",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "How up-to-date the data is vs expectation (SLA/SLO).",
    "why": "Stale dashboards mislead decisions.",
    "example": "\"Orders table max `updated_at` < 1 hour ago\".",
    "remember": [
      "Define per table SLA",
      "Alert on breach, not only job fail"
    ],
    "mustKnow": 29,
    "snippet": "SELECT MAX(updated_at) FROM fct_orders;\n-- alert if now() - max_updated_at > SLA",
    "related": [
      "Observability",
      "Completeness",
      "Pipeline Orchestration"
    ],
    "symptom": "Job is green but data is 18 hours stale.",
    "tags": [
      "interview",
      "quality"
    ]
  },
  {
    "name": "Completeness",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "Required data is present (rows and fields).",
    "why": "Missing partitions/nulls break KPIs.",
    "example": "All 24 hourly partitions arrived; `customer_id` null rate < 0.1%.",
    "remember": [
      "Partition completeness checks",
      "Critical field null thresholds"
    ],
    "mustKnow": null,
    "snippet": "# assert freshness + uniqueness before publish",
    "related": [
      "Accuracy",
      "Consistency",
      "Data Profiling"
    ],
    "symptom": "Consumers distrust the numbers.",
    "tags": [
      "quality"
    ]
  },
  {
    "name": "Accuracy",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "Data correctly represents real-world values.",
    "why": "Valid schema can still be wrong.",
    "example": "Currency conversion rates match source system within tolerance.",
    "remember": [
      "Often needs business reconciliation",
      "Sample audits + source compares"
    ],
    "mustKnow": null,
    "snippet": "# assert freshness + uniqueness before publish",
    "related": [
      "Consistency",
      "Data Profiling"
    ],
    "symptom": "Consumers distrust the numbers.",
    "tags": [
      "quality"
    ]
  },
  {
    "name": "Consistency",
    "category": "Data Quality & Governance",
    "categorySlug": "09-data-quality-governance",
    "file": "09-data-quality-governance/concepts.md",
    "definition": "Same facts agree across systems/tables within defined rules.",
    "why": "Conflicting numbers destroy trust.",
    "example": "Sum of order lines equals order header total; warehouse matches finance extract.",
    "remember": [
      "Cross-system reconciliations",
      "Define which system is source of truth"
    ],
    "mustKnow": null,
    "snippet": "# assert freshness + uniqueness before publish",
    "related": [
      "Data Profiling",
      "Data Validation"
    ],
    "symptom": "Consumers distrust the numbers.",
    "tags": [
      "quality"
    ]
  },
  {
    "name": "Object Storage",
    "category": "Cloud & Storage",
    "categorySlug": "10-cloud-storage",
    "file": "10-cloud-storage/concepts.md",
    "definition": "Store immutable objects (files) addressed by key in buckets; virtually infinite scale.",
    "why": "Default substrate for data lakes.",
    "example": "S3, ADLS Gen2, GCS holding parquet/Delta tables.",
    "remember": [
      "Eventually consistent listing quirks are rarer now but design carefully",
      "Cheap capacity; watch request/API costs"
    ],
    "mustKnow": 30,
    "snippet": "s3://lake/bronze/events/dt=2024-01-01/part-000.parquet\n# lake tables live as objects + metadata",
    "related": [
      "Data Lake",
      "Delta Lake",
      "IAM"
    ],
    "symptom": "Treating S3 like a local FS; millions of tiny LIST requests.",
    "tags": [
      "cloud",
      "interview",
      "lakehouse"
    ]
  },
  {
    "name": "Block Storage",
    "category": "Cloud & Storage",
    "categorySlug": "10-cloud-storage",
    "file": "10-cloud-storage/concepts.md",
    "definition": "Disk volumes attached to VMs (like SAN/local disks).",
    "why": "Databases, Kafka brokers, Spark local shuffle disks.",
    "example": "EBS/Azure Disks for Postgres data directory.",
    "remember": [
      "Higher IOPS for stateful services",
      "Not the primary lake storage"
    ],
    "mustKnow": null,
    "snippet": "# least-privilege IAM on lake prefixes",
    "related": [
      "Lifecycle Policies",
      "IAM",
      "Object Storage"
    ],
    "symptom": "Access or cost issues around lake storage.",
    "tags": [
      "cloud"
    ]
  },
  {
    "name": "Lifecycle Policies",
    "category": "Cloud & Storage",
    "categorySlug": "10-cloud-storage",
    "file": "10-cloud-storage/concepts.md",
    "definition": "Automatic transition/expire of objects by age/prefix (hot \u2192 cool \u2192 archive \u2192 delete).",
    "why": "Cost control for lakes and logs.",
    "example": "Raw logs to Glacier after 90 days; delete after 365.",
    "remember": [
      "Align with compliance retention",
      "Archive retrieval can be slow/expensive"
    ],
    "mustKnow": null,
    "snippet": "# least-privilege IAM on lake prefixes",
    "related": [
      "IAM",
      "Secrets Management",
      "Object Storage"
    ],
    "symptom": "Access or cost issues around lake storage.",
    "tags": [
      "cloud"
    ]
  },
  {
    "name": "IAM",
    "category": "Cloud & Storage",
    "categorySlug": "10-cloud-storage",
    "file": "10-cloud-storage/concepts.md",
    "definition": "Identity and Access Management \u2014 who can do what on which resources.",
    "why": "Security boundary for every cloud data platform.",
    "example": "Role for Spark job: read `s3://lake/bronze/*`, write `s3://lake/silver/*` only.",
    "remember": [
      "Least privilege",
      "Prefer roles over long-lived keys"
    ],
    "mustKnow": null,
    "snippet": "# job role: s3:GetObject on bronze/*, PutObject on silver/* only",
    "related": [
      "Secrets Management",
      "KMS",
      "Private Endpoints"
    ],
    "symptom": "Long-lived access keys leaked from a notebook.",
    "tags": [
      "cloud"
    ]
  },
  {
    "name": "Secrets Management",
    "category": "Cloud & Storage",
    "categorySlug": "10-cloud-storage",
    "file": "10-cloud-storage/concepts.md",
    "definition": "Store/rotate credentials and API keys outside code.",
    "why": "Prevent leaks in repos and logs.",
    "example": "AWS Secrets Manager / Azure Key Vault / HashiCorp Vault for DB passwords.",
    "remember": [
      "Never commit secrets",
      "Rotate and audit access"
    ],
    "mustKnow": null,
    "snippet": "# least-privilege IAM on lake prefixes",
    "related": [
      "KMS",
      "Encryption at Rest",
      "Object Storage"
    ],
    "symptom": "Access or cost issues around lake storage.",
    "tags": [
      "cloud"
    ]
  },
  {
    "name": "KMS",
    "category": "Cloud & Storage",
    "categorySlug": "10-cloud-storage",
    "file": "10-cloud-storage/concepts.md",
    "definition": "Key Management Service \u2014 create and control encryption keys.",
    "why": "Separate permission to use keys from permission to see data.",
    "example": "S3 SSE-KMS; revoke key usage to lock data access.",
    "remember": [
      "Key policies are critical",
      "Audit key usage"
    ],
    "mustKnow": null,
    "snippet": "# least-privilege IAM on lake prefixes",
    "related": [
      "Encryption at Rest",
      "Encryption in Transit",
      "Object Storage"
    ],
    "symptom": "Access or cost issues around lake storage.",
    "tags": [
      "cloud"
    ]
  },
  {
    "name": "Encryption at Rest",
    "category": "Cloud & Storage",
    "categorySlug": "10-cloud-storage",
    "file": "10-cloud-storage/concepts.md",
    "definition": "Data encrypted on disk/storage media.",
    "why": "Protects against disk theft and some breach scenarios.",
    "example": "S3 default encryption; encrypted EBS volumes.",
    "remember": [
      "Usually mandatory in enterprises",
      "Combine with IAM + KMS"
    ],
    "mustKnow": null,
    "snippet": "# least-privilege IAM on lake prefixes",
    "related": [
      "Encryption in Transit",
      "VPC",
      "Object Storage"
    ],
    "symptom": "Access or cost issues around lake storage.",
    "tags": [
      "cloud"
    ]
  },
  {
    "name": "Encryption in Transit",
    "category": "Cloud & Storage",
    "categorySlug": "10-cloud-storage",
    "file": "10-cloud-storage/concepts.md",
    "definition": "Data encrypted on the network (TLS/HTTPS).",
    "why": "Protects against eavesdropping on the wire.",
    "example": "TLS to warehouses, HTTPS to APIs, SSL to Postgres.",
    "remember": [
      "Enforce TLS everywhere practical",
      "Mind internal service mesh policies"
    ],
    "mustKnow": null,
    "snippet": "# least-privilege IAM on lake prefixes",
    "related": [
      "VPC",
      "Private Endpoints",
      "Object Storage"
    ],
    "symptom": "Access or cost issues around lake storage.",
    "tags": [
      "cloud"
    ]
  },
  {
    "name": "VPC",
    "category": "Cloud & Storage",
    "categorySlug": "10-cloud-storage",
    "file": "10-cloud-storage/concepts.md",
    "definition": "Virtual Private Cloud \u2014 isolated network for your cloud resources.",
    "why": "Network security and private connectivity.",
    "example": "Data platform subnets with no public IPs; NAT for egress.",
    "remember": [
      "Segment public/private subnets",
      "Control egress for cost and security"
    ],
    "mustKnow": null,
    "snippet": "# least-privilege IAM on lake prefixes",
    "related": [
      "Private Endpoints",
      "Object Storage"
    ],
    "symptom": "Access or cost issues around lake storage.",
    "tags": [
      "cloud"
    ]
  },
  {
    "name": "Private Endpoints",
    "category": "Cloud & Storage",
    "categorySlug": "10-cloud-storage",
    "file": "10-cloud-storage/concepts.md",
    "definition": "Private network interfaces to cloud services without public internet.",
    "why": "Keep data plane traffic off public network.",
    "example": "S3/GCP Private Service Connect / Azure Private Link to storage and warehouses.",
    "remember": [
      "Prefer for production lake + warehouse access",
      "DNS and firewall rules matter"
    ],
    "mustKnow": null,
    "snippet": "# least-privilege IAM on lake prefixes",
    "related": [
      "Object Storage",
      "Block Storage"
    ],
    "symptom": "Access or cost issues around lake storage.",
    "tags": [
      "cloud"
    ]
  },
  {
    "name": "Docker",
    "category": "DevOps for Data",
    "categorySlug": "11-devops",
    "file": "11-devops/concepts.md",
    "definition": "Package app + dependencies into portable container images.",
    "why": "Same Spark/Python job runs locally, CI, and prod.",
    "example": "Image with `spark-submit` entrypoint and pinned library versions.",
    "remember": [
      "Pin base image digests for reproducibility",
      "Keep images lean; separate build vs runtime if needed"
    ],
    "mustKnow": null,
    "snippet": "# infra change via PR + plan/apply",
    "related": [
      "Kubernetes",
      "CI/CD"
    ],
    "symptom": "Environment drift between staging and prod.",
    "tags": [
      "cloud",
      "devops"
    ]
  },
  {
    "name": "Kubernetes",
    "category": "DevOps for Data",
    "categorySlug": "11-devops",
    "file": "11-devops/concepts.md",
    "definition": "Orchestrates containers: scheduling, scaling, restarts, service discovery.",
    "why": "Common runtime for Spark-on-K8s, Airflow, Flink, microservices.",
    "example": "SparkApplication CRD submits driver/executors as pods.",
    "remember": [
      "Requests/limits matter for stability",
      "Persistent volumes for stateful components"
    ],
    "mustKnow": null,
    "snippet": "# infra change via PR + plan/apply",
    "related": [
      "CI/CD",
      "GitOps",
      "Docker"
    ],
    "symptom": "Environment drift between staging and prod.",
    "tags": [
      "cloud",
      "devops"
    ]
  },
  {
    "name": "CI/CD",
    "category": "DevOps for Data",
    "categorySlug": "11-devops",
    "file": "11-devops/concepts.md",
    "definition": "Continuous Integration / Continuous Delivery \u2014 automate test and deploy of code/config.",
    "why": "Safe, repeatable pipeline and dbt/SQL changes.",
    "example": "PR runs unit tests + SQL lint \u2192 merge deploys DAG to Airflow.",
    "remember": [
      "Test data contracts in CI",
      "Separate deploy of code vs heavy data backfills"
    ],
    "mustKnow": null,
    "snippet": "# infra change via PR + plan/apply",
    "related": [
      "GitOps",
      "Terraform",
      "Docker"
    ],
    "symptom": "Environment drift between staging and prod.",
    "tags": [
      "cloud",
      "devops"
    ]
  },
  {
    "name": "GitOps",
    "category": "DevOps for Data",
    "categorySlug": "11-devops",
    "file": "11-devops/concepts.md",
    "definition": "Desired system state lives in Git; controllers sync cluster to match.",
    "why": "Auditable infra and app deploys.",
    "example": "Argo CD applies Helm charts from `infra` repo on merge to main.",
    "remember": [
      "Git is source of truth",
      "PRs become change control"
    ],
    "mustKnow": null,
    "snippet": "# infra change via PR + plan/apply",
    "related": [
      "Terraform",
      "Helm",
      "Docker"
    ],
    "symptom": "Environment drift between staging and prod.",
    "tags": [
      "cloud",
      "devops"
    ]
  },
  {
    "name": "Terraform",
    "category": "DevOps for Data",
    "categorySlug": "11-devops",
    "file": "11-devops/concepts.md",
    "definition": "Declarative Infrastructure as Code tool (HCL) for cloud resources.",
    "why": "Reproducible buckets, IAM, warehouses, networks.",
    "example": "Terraform creates S3 lake buckets, Glue catalog DB, IAM roles.",
    "remember": [
      "Remote state + locks",
      "Plan before apply; module wisely"
    ],
    "mustKnow": null,
    "snippet": "# infra change via PR + plan/apply",
    "related": [
      "Helm",
      "Infrastructure as Code (IaC)",
      "Docker"
    ],
    "symptom": "Environment drift between staging and prod.",
    "tags": [
      "cloud",
      "devops"
    ]
  },
  {
    "name": "Helm",
    "category": "DevOps for Data",
    "categorySlug": "11-devops",
    "file": "11-devops/concepts.md",
    "definition": "Package manager for Kubernetes \u2014 charts template K8s manifests.",
    "why": "Standard way to deploy Airflow, Spark operator, monitoring stacks.",
    "example": "`helm upgrade --install airflow bitnami/airflow -f values.yaml`.",
    "remember": [
      "values.yaml is your config surface",
      "Version charts carefully"
    ],
    "mustKnow": null,
    "snippet": "# infra change via PR + plan/apply",
    "related": [
      "Infrastructure as Code (IaC)",
      "Docker"
    ],
    "symptom": "Environment drift between staging and prod.",
    "tags": [
      "cloud",
      "devops"
    ]
  },
  {
    "name": "Infrastructure as Code (IaC)",
    "category": "DevOps for Data",
    "categorySlug": "11-devops",
    "file": "11-devops/concepts.md",
    "definition": "Define and version infrastructure in code (Terraform, CloudFormation, Pulumi, Bicep).",
    "why": "Repeatable environments; peer-reviewed changes.",
    "example": "Dev/stage/prod lakes created from the same modules with different variables.",
    "remember": [
      "No click-ops for critical paths",
      "Codify networking, IAM, and data stores together"
    ],
    "mustKnow": null,
    "snippet": "terraform plan && terraform apply\n# buckets, IAM, warehouses versioned in Git",
    "related": [
      "Terraform",
      "GitOps",
      "CI/CD"
    ],
    "symptom": "Prod lake config drifts from staging; only one person knows clicks.",
    "tags": [
      "cloud",
      "devops"
    ]
  },
  {
    "name": "Change Data Capture (CDC)",
    "category": "CDC & SCD",
    "categorySlug": "12-cdc-scd",
    "file": "12-cdc-scd/concepts.md",
    "definition": "Continuously capture inserts/updates/deletes from a source system (often via logs) instead of full reloads.",
    "why": "Cheaper, fresher pipelines; enables near-real-time lakes/warehouses.",
    "example": "Debezium reads Postgres WAL \u2192 Kafka topics \u2192 Spark/Flink applies to Delta.",
    "remember": [
      "Prefer log-based CDC over query-based when possible",
      "Handle deletes and schema changes explicitly"
    ],
    "mustKnow": null,
    "snippet": "# Debezium/Postgres example topic\n# orders.Customer \u2192 apply MERGE into lake table by PK",
    "related": [
      "Log-Based CDC",
      "SCD Type 2",
      "Idempotency"
    ],
    "symptom": "Nightly full reloads thrash the source DB and miss deletes.",
    "tags": [
      "cdc",
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "Log-Based CDC",
    "category": "CDC & SCD",
    "categorySlug": "12-cdc-scd",
    "file": "12-cdc-scd/concepts.md",
    "definition": "Read the database transaction log (WAL/binlog/redo) to emit change events.",
    "why": "Low source impact, ordered, includes deletes.",
    "example": "MySQL binlog \u2192 Debezium \u2192 `orders` topic with before/after images.",
    "remember": [
      "Needs log retention + privileges",
      "Clock/ordering still matters across shards"
    ],
    "mustKnow": null,
    "snippet": "# Debezium connector \u2192 Kafka topic with op=c/u/d\n# before/after payload for each change",
    "related": [
      "Change Data Capture (CDC)",
      "Soft Delete vs Hard Delete",
      "Schema Registry"
    ],
    "symptom": "Query-based CDC misses hard deletes and under-loads the warehouse.",
    "tags": [
      "cdc",
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "Query-Based CDC",
    "category": "CDC & SCD",
    "categorySlug": "12-cdc-scd",
    "file": "12-cdc-scd/concepts.md",
    "definition": "Poll source tables using timestamps or version columns to find changes.",
    "why": "Easy to start; misses deletes and can miss updates if clocks/columns are weak.",
    "example": "`SELECT * FROM orders WHERE updated_at > :last_watermark`.",
    "remember": [
      "Soft deletes required to see removals",
      "Overlap windows to avoid gaps"
    ],
    "mustKnow": null,
    "snippet": "SELECT * FROM orders\nWHERE updated_at > :watermark - INTERVAL '5 minutes';",
    "related": [
      "Change Data Capture (CDC)",
      "Log-Based CDC",
      "Checkpointing"
    ],
    "symptom": "Gaps appear when updated_at doesn't move on some updates.",
    "tags": [
      "cdc",
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "Soft Delete vs Hard Delete",
    "category": "CDC & SCD",
    "categorySlug": "12-cdc-scd",
    "file": "12-cdc-scd/concepts.md",
    "definition": "Soft delete marks a row inactive (`is_deleted=true`); hard delete removes the row.",
    "why": "CDC and analytics must treat them differently for correctness.",
    "example": "Source hard-delete \u2192 CDC tombstone \u2192 target marks deleted or removes row.",
    "remember": [
      "Tombstones/delete events are first-class",
      "Downstream BI must filter soft deletes"
    ],
    "mustKnow": null,
    "snippet": "-- soft: UPDATE SET is_deleted=true\n-- CDC hard delete: op='d' tombstone \u2192 target delete/flag",
    "related": [
      "Change Data Capture (CDC)",
      "SCD Type 2",
      "Data Validation"
    ],
    "symptom": "Deleted customers still appear in dashboards.",
    "tags": [
      "cdc",
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "SCD Type 1",
    "category": "CDC & SCD",
    "categorySlug": "12-cdc-scd",
    "file": "12-cdc-scd/concepts.md",
    "definition": "Overwrite attributes in place; no history kept.",
    "why": "Simple corrections (fix typo in email) without dimension bloat.",
    "example": "Customer address updated \u2192 old address gone forever in the dim.",
    "remember": [
      "Fast and small",
      "Cannot answer \u201cwhat was address last year?\u201d"
    ],
    "mustKnow": null,
    "snippet": "MERGE INTO dim_customer t USING staging s ON t.nk=s.nk\nWHEN MATCHED THEN UPDATE SET *;",
    "related": [
      "SCD Type 2",
      "Surrogate Key",
      "Natural Key"
    ],
    "symptom": "You needed yesterday's attribute value but type-1 overwrote it.",
    "tags": [
      "cdc",
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "SCD Type 2",
    "category": "CDC & SCD",
    "categorySlug": "12-cdc-scd",
    "file": "12-cdc-scd/concepts.md",
    "definition": "Keep history by inserting a new dimension row with effective dates / current flag.",
    "why": "Point-in-time correct facts (who was the customer segment at order time).",
    "example": "Segment changes \u2192 close old row (`end_date`), insert new row with new surrogate key.",
    "remember": [
      "Facts store the surrogate key valid at event time",
      "Watch dimension explosion on volatile attributes"
    ],
    "mustKnow": null,
    "snippet": "-- close current row, insert new version\nUPDATE dim SET end_date=CURRENT_DATE, is_current=false WHERE nk=? AND is_current;\nINSERT ... is_current=true;",
    "related": [
      "Effective Dating",
      "Surrogate Key",
      "Late-Arriving Dimension"
    ],
    "symptom": "Historical facts show today's attributes after a customer change.",
    "tags": [
      "cdc",
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "SCD Type 3",
    "category": "CDC & SCD",
    "categorySlug": "12-cdc-scd",
    "file": "12-cdc-scd/concepts.md",
    "definition": "Store limited history in extra columns (e.g., current + previous).",
    "why": "Lightweight \u201cprevious value\u201d without full type-2 history.",
    "example": "`region` and `prior_region` columns on `dim_customer`.",
    "remember": [
      "Only N prior versions",
      "Rare vs type 1/2 in modern lakes"
    ],
    "mustKnow": null,
    "snippet": "UPDATE dim_customer SET prior_segment=segment, segment=:new WHERE customer_nk=:nk;",
    "related": [
      "SCD Type 1",
      "SCD Type 2",
      "Mini Dimension"
    ],
    "symptom": "Need more than one prior value but only type-3 columns exist.",
    "tags": [
      "cdc",
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "Surrogate Key",
    "category": "CDC & SCD",
    "categorySlug": "12-cdc-scd",
    "file": "12-cdc-scd/concepts.md",
    "definition": "Warehouse-generated key (usually integer/UUID) independent of source natural keys.",
    "why": "Stable joins across SCD versions and multi-source masters.",
    "example": "`customer_sk` changes on type-2 version; `customer_id` natural key stays.",
    "remember": [
      "Facts join on surrogate keys",
      "Natural keys still needed for matching"
    ],
    "mustKnow": null,
    "snippet": "-- dim_customer(customer_sk, customer_nk, ...)\n-- fact_orders.customer_sk \u2192 type-2 version at order time",
    "related": [
      "Natural Key",
      "SCD Type 2",
      "Effective Dating"
    ],
    "symptom": "Joining facts on natural keys picks the wrong type-2 version.",
    "tags": [
      "cdc",
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "Natural Key",
    "category": "CDC & SCD",
    "categorySlug": "12-cdc-scd",
    "file": "12-cdc-scd/concepts.md",
    "definition": "Business identifier from the source system (`customer_id`, `sku`).",
    "why": "Matching/CDC identity; not always unique across systems.",
    "example": "Same person has different natural keys in CRM and billing \u2192 MDM/matching.",
    "remember": [
      "Can change or collide across sources",
      "Pair with surrogate keys in dims"
    ],
    "mustKnow": null,
    "snippet": "-- match CDC on source business key\nON target.order_id = source.order_id",
    "related": [
      "Surrogate Key",
      "Change Data Capture (CDC)",
      "Master Data Management (MDM)"
    ],
    "symptom": "Two source systems reuse the same ID for different entities.",
    "tags": [
      "cdc",
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "Effective Dating",
    "category": "CDC & SCD",
    "categorySlug": "12-cdc-scd",
    "file": "12-cdc-scd/concepts.md",
    "definition": "`effective_start` / `effective_end` (or `is_current`) defines when a dim version is valid.",
    "why": "Point-in-time joins between facts and type-2 dimensions.",
    "example": "Join fact `order_date` to dim where `start <= order_date < end`.",
    "remember": [
      "Avoid overlapping ranges",
      "Index/cluster on key + dates"
    ],
    "mustKnow": null,
    "snippet": "JOIN dim d ON f.customer_sk = d.customer_sk\n-- or: d.nk=f.nk AND f.ts >= d.start AND f.ts < d.end",
    "related": [
      "SCD Type 2",
      "Surrogate Key",
      "Grain"
    ],
    "symptom": "Overlapping effective ranges double-count dimension matches.",
    "tags": [
      "cdc",
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "Late-Arriving Dimension",
    "category": "CDC & SCD",
    "categorySlug": "12-cdc-scd",
    "file": "12-cdc-scd/concepts.md",
    "definition": "Fact arrives before its dimension row exists.",
    "why": "Orphan facts break joins and quality checks.",
    "example": "Order for new customer lands before `dim_customer` CDC catches up.",
    "remember": [
      "Use inferred/unknown member placeholders",
      "Re-key when dimension arrives"
    ],
    "mustKnow": null,
    "snippet": "-- insert unknown member sk=-1, then re-key facts when dim arrives",
    "related": [
      "Late-Arriving Fact",
      "SCD Type 2",
      "Surrogate Key"
    ],
    "symptom": "Orders land with NULL customer attributes until CDC catches up.",
    "tags": [
      "cdc",
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "Late-Arriving Fact",
    "category": "CDC & SCD",
    "categorySlug": "12-cdc-scd",
    "file": "12-cdc-scd/concepts.md",
    "definition": "Fact event arrives after its business date / reporting window closed.",
    "why": "Restates historical metrics; needs idempotent merges.",
    "example": "Store posts yesterday\u2019s sale two days late \u2192 merge into prior partition.",
    "remember": [
      "Partition overwrite / MERGE by business key",
      "Communicate restatement SLAs"
    ],
    "mustKnow": null,
    "snippet": "MERGE INTO fct_orders t USING late_arrivals s ON t.order_id=s.order_id\nWHEN MATCHED THEN UPDATE SET * WHEN NOT MATCHED THEN INSERT *;",
    "related": [
      "Idempotency",
      "Late Data",
      "Partition Pruning"
    ],
    "symptom": "Yesterday's partition is wrong after late POS uploads.",
    "tags": [
      "cdc",
      "interview",
      "warehouse"
    ]
  },
  {
    "name": "dbt Model",
    "category": "dbt",
    "categorySlug": "13-dbt",
    "file": "13-dbt/concepts.md",
    "definition": "A versioned SQL (or Python) select statement materialized as a table/view in the warehouse.",
    "why": "Transforms become testable, documented, DAG-aware units.",
    "example": "`models/marts/fct_orders.sql` builds from staging models.",
    "remember": [
      "One model \u2248 one relation",
      "Name by layer: staging / intermediate / marts"
    ],
    "mustKnow": null,
    "snippet": "-- models/marts/fct_orders.sql\nselect * from {{ ref('stg_orders') }}",
    "related": [
      "ref()",
      "Materialization",
      "Tests (dbt)"
    ],
    "symptom": "Copy-pasted SQL across notebooks with no DAG or tests.",
    "tags": [
      "dbt",
      "interview",
      "sql"
    ]
  },
  {
    "name": "Materialization",
    "category": "dbt",
    "categorySlug": "13-dbt",
    "file": "13-dbt/concepts.md",
    "definition": "How dbt builds a model: view, table, incremental, ephemeral.",
    "why": "Cost/performance tradeoff per model.",
    "example": "`{{ config(materialized='incremental', unique_key='order_id') }}`",
    "remember": [
      "Views = cheap to build, pay at query time",
      "Incrementals need a correct strategy"
    ],
    "mustKnow": null,
    "snippet": "{{ config(materialized='table') }}\n-- or view | incremental | ephemeral",
    "related": [
      "Incremental Model",
      "dbt Model",
      "Cost Observability"
    ],
    "symptom": "Huge table rebuilt as a view scanned repeatedly by BI.",
    "tags": [
      "dbt",
      "interview",
      "sql"
    ]
  },
  {
    "name": "Incremental Model",
    "category": "dbt",
    "categorySlug": "13-dbt",
    "file": "13-dbt/concepts.md",
    "definition": "Only process new/changed rows each run instead of full rebuild.",
    "why": "Scales large facts; easy to get wrong (duplicates/misses).",
    "example": "`where updated_at > (select max(updated_at) from {{ this }})`",
    "remember": [
      "Define unique key + late-data strategy",
      "Periodic full-refresh still needed"
    ],
    "mustKnow": null,
    "snippet": "{{ config(materialized='incremental', unique_key='order_id') }}\n{% if is_incremental() %}\nwhere updated_at > (select max(updated_at) from {{ this }})\n{% endif %}",
    "related": [
      "Materialization",
      "Idempotency",
      "Late-Arriving Fact"
    ],
    "symptom": "Incremental run duplicates keys or misses late updates.",
    "tags": [
      "dbt",
      "interview",
      "sql"
    ]
  },
  {
    "name": "Sources",
    "category": "dbt",
    "categorySlug": "13-dbt",
    "file": "13-dbt/concepts.md",
    "definition": "Declared raw upstream tables dbt reads but does not own.",
    "why": "Freshness checks and clear boundary raw \u2192 staging.",
    "example": "`sources.yml` \u2192 `{{ source('stripe', 'payments') }}`",
    "remember": [
      "Sources are ingest landing zones",
      "Test source freshness SLAs"
    ],
    "mustKnow": null,
    "snippet": "select * from {{ source('raw', 'orders') }}",
    "related": [
      "Seeds",
      "Freshness",
      "Data Contracts"
    ],
    "symptom": "Staging reads a renamed raw table nobody declared.",
    "tags": [
      "dbt",
      "interview",
      "sql"
    ]
  },
  {
    "name": "Seeds",
    "category": "dbt",
    "categorySlug": "13-dbt",
    "file": "13-dbt/concepts.md",
    "definition": "CSV files checked into the repo and loaded as tables.",
    "why": "Small mapping/reference data under version control.",
    "example": "`seeds/country_codes.csv` \u2192 `ref('country_codes')`.",
    "remember": [
      "Not for large facts",
      "Great for simple lookups"
    ],
    "mustKnow": null,
    "snippet": "# seeds/country_codes.csv then:\nselect * from {{ ref('country_codes') }}",
    "related": [
      "Sources",
      "dbt Model",
      "Conformed Dimension"
    ],
    "symptom": "Mapping CSV lives only on someone's laptop.",
    "tags": [
      "dbt",
      "interview",
      "sql"
    ]
  },
  {
    "name": "ref()",
    "category": "dbt",
    "categorySlug": "13-dbt",
    "file": "13-dbt/concepts.md",
    "definition": "dbt function linking models so the DAG and schema names resolve correctly.",
    "why": "Dependency graph + environment-aware relation names.",
    "example": "`select * from {{ ref('stg_orders') }}`",
    "remember": [
      "Prefer `ref` over hard-coded table names",
      "Circular refs are errors"
    ],
    "mustKnow": null,
    "snippet": "select * from {{ ref('stg_orders') }} o\njoin {{ ref('dim_customers') }} c using (customer_id)",
    "related": [
      "dbt Model",
      "Sources",
      "Data Lineage"
    ],
    "symptom": "Hard-coded prod schema names break in CI.",
    "tags": [
      "dbt",
      "interview",
      "sql"
    ]
  },
  {
    "name": "Tests (dbt)",
    "category": "dbt",
    "categorySlug": "13-dbt",
    "file": "13-dbt/concepts.md",
    "definition": "Assertions on models/columns: unique, not_null, accepted_values, relationships, custom.",
    "why": "Data quality gates in CI and prod runs.",
    "example": "`unique` + `not_null` on `order_id`; relationship to `dim_customers`.",
    "remember": [
      "Fail builds on critical tests",
      "Custom tests for business rules"
    ],
    "mustKnow": null,
    "snippet": "# schema.yml\n- name: order_id\n  tests: [unique, not_null]",
    "related": [
      "Data Validation",
      "Data Contracts",
      "Exposures"
    ],
    "symptom": "Duplicate order_ids ship to finance before anyone notices.",
    "tags": [
      "dbt",
      "interview",
      "sql"
    ]
  },
  {
    "name": "Exposures",
    "category": "dbt",
    "categorySlug": "13-dbt",
    "file": "13-dbt/concepts.md",
    "definition": "Declared downstream uses (dashboards, ML jobs) that depend on models.",
    "why": "Impact analysis when changing a mart.",
    "example": "Exposure `finance_arr_dashboard` depends on `fct_subscriptions`.",
    "remember": [
      "Document owners and maturity",
      "Ties lineage to consumers"
    ],
    "mustKnow": null,
    "snippet": "exposures:\n  - name: exec_revenue_dash\n    depends_on: [ref('fct_orders')]",
    "related": [
      "Data Lineage",
      "Documentation / Docs Site",
      "Freshness"
    ],
    "symptom": "Changing a mart silently breaks a C-level dashboard.",
    "tags": [
      "dbt",
      "interview",
      "sql"
    ]
  },
  {
    "name": "Documentation / Docs Site",
    "category": "dbt",
    "categorySlug": "13-dbt",
    "file": "13-dbt/concepts.md",
    "definition": "YAML descriptions + `dbt docs generate` catalog of models/columns.",
    "why": "Self-serve understanding of metrics and tables.",
    "example": "Column description for `gross_revenue` definition.",
    "remember": [
      "Docs without ownership rot",
      "Link to data contracts where possible"
    ],
    "mustKnow": null,
    "snippet": "dbt docs generate && dbt docs serve",
    "related": [
      "Exposures",
      "Metadata",
      "Data Catalog"
    ],
    "symptom": "Nobody knows which column is the revenue definition of record.",
    "tags": [
      "dbt",
      "interview",
      "sql"
    ]
  },
  {
    "name": "Slim CI / State Defer",
    "category": "dbt",
    "categorySlug": "13-dbt",
    "file": "13-dbt/concepts.md",
    "definition": "Run/test only modified models (and downstream) using prior manifest state.",
    "why": "Fast PRs on large projects.",
    "example": "`dbt test --select state:modified+` with defer to prod.",
    "remember": [
      "Needs a trusted prod manifest",
      "Still full-refresh periodically"
    ],
    "mustKnow": null,
    "snippet": "dbt test --select state:modified+ --defer --state path/to/prod/manifest",
    "related": [
      "CI/CD",
      "Tests (dbt)",
      "dbt Model"
    ],
    "symptom": "Every PR runs the entire 400-model project.",
    "tags": [
      "dbt",
      "interview",
      "sql"
    ]
  },
  {
    "name": "Topic",
    "category": "Kafka",
    "categorySlug": "14-kafka",
    "file": "14-kafka/concepts.md",
    "definition": "Named stream of records; publishers write, consumers read.",
    "why": "Logical channel for a type of event (`orders`, `clicks`).",
    "example": "Producers write to `payments.completed`; three services consume independently.",
    "remember": [
      "Topics are multi-subscriber",
      "Plan naming + retention up front"
    ],
    "mustKnow": null,
    "snippet": "kafka-topics.sh --create --topic payments.completed --partitions 12 --replication-factor 3",
    "related": [
      "Partition",
      "Consumer Group",
      "Schema Registry"
    ],
    "symptom": "One mega-topic mixes unrelated event types and confuses ownership.",
    "tags": [
      "interview",
      "kafka",
      "streaming"
    ]
  },
  {
    "name": "Partition",
    "category": "Kafka",
    "categorySlug": "14-kafka",
    "file": "14-kafka/concepts.md",
    "definition": "Ordered, append-only log slice of a topic; unit of parallelism.",
    "why": "Throughput and ordering are per partition, not global.",
    "example": "Key by `customer_id` so one customer\u2019s events stay ordered in one partition.",
    "remember": [
      "More partitions \u2192 more parallelism",
      "Rebalancing and key design matter"
    ],
    "mustKnow": null,
    "snippet": "# key by customer_id for per-customer ordering\nproducer.send(topic, key=customer_id, value=event)",
    "related": [
      "Topic",
      "Consumer Group",
      "Offset"
    ],
    "symptom": "Global ordering assumed, but only partitions are ordered.",
    "tags": [
      "interview",
      "kafka",
      "streaming"
    ]
  },
  {
    "name": "Offset",
    "category": "Kafka",
    "categorySlug": "14-kafka",
    "file": "14-kafka/concepts.md",
    "definition": "Position of a consumer within a partition log.",
    "why": "Progress tracking and replay.",
    "example": "Commit offset 1050 after processing; restart resumes at 1051 (depending on config).",
    "remember": [
      "Commit too early \u2192 loss; too late \u2192 duplicates",
      "External stores can hold offsets too"
    ],
    "mustKnow": null,
    "snippet": "consumer.commit_sync()  # after successful sink write",
    "related": [
      "Consumer Group",
      "At Least Once",
      "Checkpointing"
    ],
    "symptom": "Commit-before-write loses records on crash; write-before-commit duplicates.",
    "tags": [
      "interview",
      "kafka",
      "streaming"
    ]
  },
  {
    "name": "Consumer Group",
    "category": "Kafka",
    "categorySlug": "14-kafka",
    "file": "14-kafka/concepts.md",
    "definition": "Set of consumers sharing work on a topic; each partition goes to one member.",
    "why": "Scale reads horizontally without double-processing inside the group.",
    "example": "6 partitions, 3 consumers in group `billing` \u2192 ~2 partitions each.",
    "remember": [
      "Extra consumers beyond partition count sit idle",
      "Rebalance pauses briefly"
    ],
    "mustKnow": null,
    "snippet": "# group.id=billing-service\n# N consumers share topic partitions",
    "related": [
      "Partition",
      "Offset",
      "Dead Letter Topic"
    ],
    "symptom": "Scaling to 20 consumers on 6 partitions leaves 14 idle.",
    "tags": [
      "interview",
      "kafka",
      "streaming"
    ]
  },
  {
    "name": "Producer / Consumer",
    "category": "Kafka",
    "categorySlug": "14-kafka",
    "file": "14-kafka/concepts.md",
    "definition": "Writer vs reader clients of Kafka topics.",
    "why": "Different configs for durability, batching, and commit strategy.",
    "example": "Producer `acks=all`; consumer `enable.auto.commit=false` with manual commits.",
    "remember": [
      "Idempotent producer reduces dupes",
      "Consumer commit policy defines at-least-once behavior"
    ],
    "mustKnow": null,
    "snippet": "acks=all\nenable.idempotence=true\nenable.auto.commit=false",
    "related": [
      "Exactly-Once Semantics (EOS)",
      "Offset",
      "Replication Factor"
    ],
    "symptom": "Default acks=1 + auto-commit causes silent loss/dupes under failure.",
    "tags": [
      "interview",
      "kafka",
      "streaming"
    ]
  },
  {
    "name": "Replication Factor",
    "category": "Kafka",
    "categorySlug": "14-kafka",
    "file": "14-kafka/concepts.md",
    "definition": "How many broker copies of each partition exist.",
    "why": "Durability and availability when brokers die.",
    "example": "RF=3, min ISR=2 for production topics.",
    "remember": [
      "RF=1 risks data loss",
      "Costs disk and network"
    ],
    "mustKnow": null,
    "snippet": "# production topics: replication.factor=3, min.insync.replicas=2",
    "related": [
      "ISR (In-Sync Replicas)",
      "Fault Tolerance",
      "High Availability (HA)"
    ],
    "symptom": "RF=1 topic loses data when a single broker dies.",
    "tags": [
      "interview",
      "kafka",
      "streaming"
    ]
  },
  {
    "name": "ISR (In-Sync Replicas)",
    "category": "Kafka",
    "categorySlug": "14-kafka",
    "file": "14-kafka/concepts.md",
    "definition": "Replicas fully caught up with the leader; eligible for failover.",
    "why": "Durability guarantees with `acks=all`.",
    "example": "Leader waits for ISR acks before acknowledging the produce.",
    "remember": [
      "Under-replicated partitions are a red alert",
      "Tune replica lag thresholds"
    ],
    "mustKnow": null,
    "snippet": "# alert when under-replicated-partitions > 0",
    "related": [
      "Replication Factor",
      "High Availability (HA)",
      "Fault Tolerance"
    ],
    "symptom": "Produces hang or fail when ISR shrinks below min.insync.replicas.",
    "tags": [
      "interview",
      "kafka",
      "streaming"
    ]
  },
  {
    "name": "Exactly-Once Semantics (EOS)",
    "category": "Kafka",
    "categorySlug": "14-kafka",
    "file": "14-kafka/concepts.md",
    "definition": "Kafka transactions / idempotent produce so a read-process-write cycle doesn\u2019t dup effects (within Kafka).",
    "why": "Stronger stream processing guarantees end-to-end still need idempotent sinks.",
    "example": "Kafka Streams transactional write to output topic.",
    "remember": [
      "EOS \u2260 automatic exactly-once into your warehouse",
      "Pair with idempotent sinks outside Kafka"
    ],
    "mustKnow": null,
    "snippet": "# idempotent producer + transactional produce/consume\n# still make external sinks idempotent",
    "related": [
      "Exactly Once",
      "Idempotency",
      "Offset"
    ],
    "symptom": "Assuming Kafka EOS alone dedupes warehouse inserts.",
    "tags": [
      "interview",
      "kafka",
      "streaming"
    ]
  },
  {
    "name": "Compaction (Kafka)",
    "category": "Kafka",
    "categorySlug": "14-kafka",
    "file": "14-kafka/concepts.md",
    "definition": "Log compaction keeps the latest value per key; older keys garbage-collected.",
    "why": "Changelog / compacted topics for state and CDC mirrors.",
    "example": "`customer_profile` compacted topic always has latest profile per `customer_id`.",
    "remember": [
      "Not a substitute for short retention on high-volume events",
      "Tombstones delete keys"
    ],
    "mustKnow": null,
    "snippet": "# cleanup.policy=compact for changelog topics\n# key=customer_id keeps latest profile",
    "related": [
      "Change Data Capture (CDC)",
      "Topic",
      "Soft Delete vs Hard Delete"
    ],
    "symptom": "Using compaction on high-volume click streams deletes needed history.",
    "tags": [
      "interview",
      "kafka",
      "streaming"
    ]
  },
  {
    "name": "Dead Letter Topic",
    "category": "Kafka",
    "categorySlug": "14-kafka",
    "file": "14-kafka/concepts.md",
    "definition": "Topic receiving poison / failed records after retries.",
    "why": "Unblocks consumers; enables quarantine and replay.",
    "example": "Deserialization failures \u2192 `orders.dlq` with headers explaining error.",
    "remember": [
      "Monitor DLQ lag/depth",
      "Have a replay runbook"
    ],
    "mustKnow": null,
    "snippet": "# after retries \u2192 produce to orders.dlq with error headers",
    "related": [
      "Dead Letter Queue (DLQ)",
      "Schema Registry",
      "Retry Strategy"
    ],
    "symptom": "Poison message blocks the whole consumer group forever.",
    "tags": [
      "interview",
      "kafka",
      "streaming"
    ]
  },
  {
    "name": "Schema Registry",
    "category": "Kafka",
    "categorySlug": "14-kafka",
    "file": "14-kafka/concepts.md",
    "definition": "Service storing Avro/Protobuf/JSON schemas for topic payloads with compatibility rules.",
    "why": "Prevents breaking consumers on schema changes.",
    "example": "BACKWARD compatibility for `orders-value` subject.",
    "remember": [
      "Enforce compatibility in CI",
      "Version schemas intentionally"
    ],
    "mustKnow": null,
    "snippet": "# BACKWARD compatibility on orders-value\n# CI fails incompatible schema evolve",
    "related": [
      "Schema Evolution",
      "Data Contracts",
      "Schema Enforcement"
    ],
    "symptom": "Producer ships a breaking Avro change; all consumers crash.",
    "tags": [
      "interview",
      "kafka",
      "streaming"
    ]
  },
  {
    "name": "DAG",
    "category": "Airflow",
    "categorySlug": "15-airflow",
    "file": "15-airflow/concepts.md",
    "definition": "Directed Acyclic Graph of tasks with dependencies \u2014 Airflow\u2019s unit of workflow.",
    "why": "Expresses order, retries, and scheduling of pipeline steps.",
    "example": "`extract >> transform >> test >> publish`.",
    "remember": [
      "No cycles",
      "Keep DAGs idempotent"
    ],
    "mustKnow": null,
    "snippet": "extract >> transform >> test >> publish",
    "related": [
      "Task / Operator",
      "Schedule / Timetable",
      "Backfill"
    ],
    "symptom": "Cron scripts race with no dependency graph.",
    "tags": [
      "airflow",
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "Task / Operator",
    "category": "Airflow",
    "categorySlug": "15-airflow",
    "file": "15-airflow/concepts.md",
    "definition": "A unit of work in a DAG; operators are task templates (Bash, Python, SparkSubmit, etc.).",
    "why": "Standardizes how you call Spark, dbt, SQL, sensors.",
    "example": "`BashOperator` runs `dbt run --select fct_orders`.",
    "remember": [
      "Prefer purpose-built operators/providers",
      "Heavy work should run on workers/clusters, not the scheduler"
    ],
    "mustKnow": null,
    "snippet": "BashOperator(task_id='dbt_run', bash_command='dbt run --select fct_orders')",
    "related": [
      "DAG",
      "Pool / Slot",
      "Sensor"
    ],
    "symptom": "Heavy Spark submit runs on the scheduler host and OOMs it.",
    "tags": [
      "airflow",
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "Sensor",
    "category": "Airflow",
    "categorySlug": "15-airflow",
    "file": "15-airflow/concepts.md",
    "definition": "Task that waits for a condition (file arrives, partition ready, external DAG success).",
    "why": "Data-aware scheduling vs blind cron.",
    "example": "`S3KeySensor` waits for `dt={{ ds }}/_SUCCESS`.",
    "remember": [
      "Mode poke vs reschedule affects worker slots",
      "Timeouts and soft_fail matter"
    ],
    "mustKnow": null,
    "snippet": "S3KeySensor(bucket_key=f'lake/bronze/dt={ds}/_SUCCESS', mode='reschedule')",
    "related": [
      "Dataset Scheduling",
      "Freshness",
      "DAG"
    ],
    "symptom": "Poke sensors hog worker slots all morning waiting for files.",
    "tags": [
      "airflow",
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "Schedule / Timetable",
    "category": "Airflow",
    "categorySlug": "15-airflow",
    "file": "15-airflow/concepts.md",
    "definition": "When DAG runs are created (cron, timetable, dataset-triggered).",
    "why": "Aligns runs with data availability and business calendars.",
    "example": "`@daily` with `data_interval` ending at midnight UTC.",
    "remember": [
      "Understand data interval vs execution date",
      "Catchup can flood the cluster"
    ],
    "mustKnow": null,
    "snippet": "schedule='@daily'  # or dataset-triggered",
    "related": [
      "Execution Date / Data Interval",
      "Catchup",
      "Dataset Scheduling"
    ],
    "symptom": "Job runs before upstream partition exists every day.",
    "tags": [
      "airflow",
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "Execution Date / Data Interval",
    "category": "Airflow",
    "categorySlug": "15-airflow",
    "file": "15-airflow/concepts.md",
    "definition": "Logical time range a run represents (Airflow 2: data interval), not \u201cwhen it ran\u201d.",
    "why": "Partitioning and idempotent writes key off logical date.",
    "example": "Daily DAG for `2024-01-01` processes that day\u2019s partition even if it runs on Jan 2.",
    "remember": [
      "Use `ds` / `data_interval_start` in templates",
      "Don\u2019t confuse with wall-clock start time"
    ],
    "mustKnow": null,
    "snippet": "# write partition for logical date\npath = f'silver/orders/dt={ds}'",
    "related": [
      "Schedule / Timetable",
      "Idempotency",
      "Backfill"
    ],
    "symptom": "Using wall-clock 'today' writes the wrong partition on late runs.",
    "tags": [
      "airflow",
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "XCom",
    "category": "Airflow",
    "categorySlug": "15-airflow",
    "file": "15-airflow/concepts.md",
    "definition": "Cross-communication \u2014 small metadata passed between tasks.",
    "why": "Handy for tiny values; dangerous for large data.",
    "example": "Task A pushes `row_count`; Task B branches if zero.",
    "remember": [
      "Not a data bus \u2014 keep payloads tiny",
      "Prefer object storage paths in XCom"
    ],
    "mustKnow": null,
    "snippet": "ti.xcom_push(key='out_path', value='s3://lake/tmp/xyz')\n# NOT the dataframe itself",
    "related": [
      "Task / Operator",
      "Object Storage",
      "DAG"
    ],
    "symptom": "Giant XCom blows up the metadata DB.",
    "tags": [
      "airflow",
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "Pool / Slot",
    "category": "Airflow",
    "categorySlug": "15-airflow",
    "file": "15-airflow/concepts.md",
    "definition": "Concurrency limits for tasks (pools) and worker parallelism (slots).",
    "why": "Protect warehouses and Spark clusters from stampede.",
    "example": "Pool `snowflake_heavy` with 4 slots for large transforms.",
    "remember": [
      "Separate pools per expensive system",
      "Sensors in poke mode consume slots"
    ],
    "mustKnow": null,
    "snippet": "# pool='snowflake_heavy', slots=4",
    "related": [
      "Task / Operator",
      "Autoscaling / Autosuspend",
      "Cost Observability"
    ],
    "symptom": "50 parallel heavy queries melt the warehouse.",
    "tags": [
      "airflow",
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "Backfill",
    "category": "Airflow",
    "categorySlug": "15-airflow",
    "file": "15-airflow/concepts.md",
    "definition": "Intentionally run a DAG for past logical dates.",
    "why": "Reprocess history after a fix; must be idempotent.",
    "example": "Backfill last 30 days after fixing a currency join.",
    "remember": [
      "Rate-limit backfills",
      "Partition overwrite / MERGE targets"
    ],
    "mustKnow": null,
    "snippet": "airflow dags backfill -s 2024-01-01 -e 2024-01-31 dag_id",
    "related": [
      "Catchup",
      "Idempotency",
      "Execution Date / Data Interval"
    ],
    "symptom": "Backfill appends duplicates into historical partitions.",
    "tags": [
      "airflow",
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "Catchup",
    "category": "Airflow",
    "categorySlug": "15-airflow",
    "file": "15-airflow/concepts.md",
    "definition": "Airflow auto-schedules missed past runs between start_date and now.",
    "why": "Can create a thundering herd of historical runs.",
    "example": "Deploy DAG with old `start_date` and `catchup=True` \u2192 hundreds of runs.",
    "remember": [
      "Default catchup carefully (often False)",
      "Use backfill deliberately instead"
    ],
    "mustKnow": null,
    "snippet": "DAG(..., start_date=..., catchup=False)",
    "related": [
      "Backfill",
      "Schedule / Timetable",
      "Pool / Slot"
    ],
    "symptom": "Deploying with catchup=True schedules hundreds of historical runs.",
    "tags": [
      "airflow",
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "SLA / Callback",
    "category": "Airflow",
    "categorySlug": "15-airflow",
    "file": "15-airflow/concepts.md",
    "definition": "Time expectations and hooks on success/failure/retry (email, Slack, PagerDuty).",
    "why": "Ops signal when data will be late.",
    "example": "SLA miss callback posts to #data-alerts.",
    "remember": [
      "Alert on consumer impact, not every retry",
      "Pair with freshness checks in the warehouse"
    ],
    "mustKnow": null,
    "snippet": "default_args={'sla': timedelta(hours=2), 'on_failure_callback': notify}",
    "related": [
      "Freshness",
      "Observability",
      "Pipeline Orchestration"
    ],
    "symptom": "Team learns data is late from executives, not alerts.",
    "tags": [
      "airflow",
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "Dataset Scheduling",
    "category": "Airflow",
    "categorySlug": "15-airflow",
    "file": "15-airflow/concepts.md",
    "definition": "Trigger downstream DAGs when upstream datasets update (Airflow 2.4+).",
    "why": "Event-ish orchestration without brittle time offsets.",
    "example": "`publish_orders` updates dataset \u2192 `marts_orders` DAG runs.",
    "remember": [
      "Clear dataset naming",
      "Still need quality gates before publish"
    ],
    "mustKnow": null,
    "snippet": "# outlets=[Dataset('s3://lake/silver/orders')]\n# downstream schedule=[Dataset(...)]",
    "related": [
      "Sensor",
      "DAG",
      "Freshness"
    ],
    "symptom": "Fixed cron drifts from actual upstream readiness.",
    "tags": [
      "airflow",
      "interview",
      "pipeline"
    ]
  },
  {
    "name": "Scan Pruning / Partitioning",
    "category": "Cost Optimization",
    "categorySlug": "16-cost-optimization",
    "file": "16-cost-optimization/concepts.md",
    "definition": "Design tables so queries read only needed files/partitions.",
    "why": "Cloud warehouses/lakes bill on bytes scanned and compute time.",
    "example": "Partition by `dt`; filter `WHERE dt = CURRENT_DATE` before joining.",
    "remember": [
      "Partition on common filters",
      "Over-partitioning creates small files / high list costs"
    ],
    "mustKnow": null,
    "snippet": "SELECT ... FROM fact WHERE dt BETWEEN '2024-01-01' AND '2024-01-07'",
    "related": [
      "Partition Pruning",
      "Column Pruning",
      "Predicate Pushdown"
    ],
    "symptom": "Dashboard scans years of data for a week filter.",
    "tags": [
      "cost",
      "interview",
      "performance"
    ]
  },
  {
    "name": "Column Pruning",
    "category": "Cost Optimization",
    "categorySlug": "16-cost-optimization",
    "file": "16-cost-optimization/concepts.md",
    "definition": "Read only selected columns from columnar formats (Parquet/ORC).",
    "why": "Wide tables waste money on `SELECT *`.",
    "example": "`SELECT user_id, amount FROM events` reads two columns, not fifty.",
    "remember": [
      "Ban `SELECT *` in prod marts",
      "Wide nested types still costly"
    ],
    "mustKnow": null,
    "snippet": "df.select('user_id', 'amount')  # not select *",
    "related": [
      "Scan Pruning / Partitioning",
      "Predicate Pushdown",
      "Cost Observability"
    ],
    "symptom": "SELECT * on a 400-column wide table doubles the bill.",
    "tags": [
      "cost",
      "interview",
      "performance"
    ]
  },
  {
    "name": "File Size Tuning",
    "category": "Cost Optimization",
    "categorySlug": "16-cost-optimization",
    "file": "16-cost-optimization/concepts.md",
    "definition": "Target healthy output file sizes (often ~128\u2013512MB) via coalesce/optimize.",
    "why": "Too small \u2192 metadata/task overhead; too big \u2192 poor parallelism.",
    "example": "`coalesce(200)` before write; schedule compaction.",
    "remember": [
      "Streaming ingest needs regular compaction",
      "Measure before/after query cost"
    ],
    "mustKnow": null,
    "snippet": "# target ~128MB-1GB compressed files for batch tables",
    "related": [
      "Delta / Iceberg / Hudi Data Files",
      "Partition Layout (Files on Disk)"
    ],
    "symptom": "Spark driver OOMs listing 5 million small objects before any scan.",
    "tags": [
      "cost",
      "file-format",
      "interview",
      "performance"
    ]
  },
  {
    "name": "Spot / Preemptible Workers",
    "category": "Cost Optimization",
    "categorySlug": "16-cost-optimization",
    "file": "16-cost-optimization/concepts.md",
    "definition": "Use discounted interruptible VMs for fault-tolerant batch compute.",
    "why": "Large Spark/Flink savings when retries are safe.",
    "example": "Spark executors on spot; driver on on-demand.",
    "remember": [
      "Idempotent jobs required",
      "Keep critical coordinators stable"
    ],
    "mustKnow": null,
    "snippet": "# executors on spot; driver on-demand; idempotent job",
    "related": [
      "Fault Tolerance",
      "Idempotency",
      "Autoscaling / Autosuspend"
    ],
    "symptom": "Non-idempotent spot jobs corrupt outputs on preemption.",
    "tags": [
      "cost",
      "interview",
      "performance"
    ]
  },
  {
    "name": "Autoscaling / Autosuspend",
    "category": "Cost Optimization",
    "categorySlug": "16-cost-optimization",
    "file": "16-cost-optimization/concepts.md",
    "definition": "Scale clusters/warehouses to zero or down when idle; up under load.",
    "why": "Idle warehouses dominate waste.",
    "example": "Snowflake auto-suspend 60s; Spark dynamic allocation.",
    "remember": [
      "Warm-up latency vs savings tradeoff",
      "Separate prod vs ad-hoc warehouses"
    ],
    "mustKnow": null,
    "snippet": "# warehouse auto-suspend 60s; Spark dynamic allocation on",
    "related": [
      "Right-Sizing Warehouses",
      "Cost Observability",
      "Pool / Slot"
    ],
    "symptom": "Dev warehouse sits XL 24/7 with no queries.",
    "tags": [
      "cost",
      "interview",
      "performance"
    ]
  },
  {
    "name": "Storage Tiering",
    "category": "Cost Optimization",
    "categorySlug": "16-cost-optimization",
    "file": "16-cost-optimization/concepts.md",
    "definition": "Move cold data to cheaper classes via lifecycle policies.",
    "why": "Lakes grow forever; most bytes are rarely read.",
    "example": "Raw logs \u2192 infrequent access after 30d \u2192 archive after 180d.",
    "remember": [
      "Align with compliance retention",
      "Retrieval can be slow/expensive"
    ],
    "mustKnow": null,
    "snippet": "# lifecycle: Standard \u2192 IA 30d \u2192 Glacier 180d",
    "related": [
      "Lifecycle Policies",
      "Object Storage",
      "Vacuum"
    ],
    "symptom": "Hot tier stores seven years of raw logs nobody reads.",
    "tags": [
      "cost",
      "interview",
      "performance"
    ]
  },
  {
    "name": "Result Caching",
    "category": "Cost Optimization",
    "categorySlug": "16-cost-optimization",
    "file": "16-cost-optimization/concepts.md",
    "definition": "Reuse recent query results when identical SQL hits warm cache.",
    "why": "Dashboards refetching the same query burn credits.",
    "example": "BI extracts cached in warehouse result cache for minutes.",
    "remember": [
      "Great for repetitive dashboards",
      "Don\u2019t confuse with data freshness SLAs"
    ],
    "mustKnow": null,
    "snippet": "-- identical dashboard SQL hits warehouse result cache",
    "related": [
      "Caching",
      "Right-Sizing Warehouses",
      "Cost Observability"
    ],
    "symptom": "Fifty users refresh the same extract every minute.",
    "tags": [
      "cost",
      "interview",
      "performance"
    ]
  },
  {
    "name": "Right-Sizing Warehouses",
    "category": "Cost Optimization",
    "categorySlug": "16-cost-optimization",
    "file": "16-cost-optimization/concepts.md",
    "definition": "Match warehouse/cluster size to workload shape, not peak folklore.",
    "why": "Bigger isn\u2019t always faster enough to justify 2\u00d7 cost.",
    "example": "Drop from 2XL to L after partitioning fixed scan volume.",
    "remember": [
      "Fix data layout before scaling up",
      "Track $/query and $/TB scanned"
    ],
    "mustKnow": null,
    "snippet": "-- fix partitioning first, then step down warehouse size",
    "related": [
      "Scan Pruning / Partitioning",
      "Cost Observability",
      "Autoscaling / Autosuspend"
    ],
    "symptom": "Scaling to 4XL to paper over a missing partition filter.",
    "tags": [
      "cost",
      "interview",
      "performance"
    ]
  },
  {
    "name": "Cost Observability",
    "category": "Cost Optimization",
    "categorySlug": "16-cost-optimization",
    "file": "16-cost-optimization/concepts.md",
    "definition": "Attribute spend to teams, jobs, tables, and queries.",
    "why": "You can\u2019t optimize what you can\u2019t see.",
    "example": "Chargeback tags on Spark jobs; query cost views in Snowflake.",
    "remember": [
      "Tag every production job",
      "Alert on spend anomalies, not only failures"
    ],
    "mustKnow": null,
    "snippet": "# tag jobs: team=payments, dag=orders_daily\n# alert on $/day anomalies",
    "related": [
      "Right-Sizing Warehouses",
      "Observability",
      "IAM"
    ],
    "symptom": "Finance asks who spent $20k on compute; nobody knows.",
    "tags": [
      "cost",
      "interview",
      "performance"
    ]
  },
  {
    "name": "Incremental vs Full Refresh",
    "category": "Cost Optimization",
    "categorySlug": "16-cost-optimization",
    "file": "16-cost-optimization/concepts.md",
    "definition": "Prefer incremental processing; full refresh only when necessary.",
    "why": "Full rebuilds of multi-TB facts are expensive and slow.",
    "example": "Daily MERGE of changed keys vs rebuild entire history.",
    "remember": [
      "Schedule periodic full refresh for drift repair",
      "Measure bytes processed per run"
    ],
    "mustKnow": null,
    "snippet": "MERGE daily deltas; full-refresh weekly for drift repair",
    "related": [
      "Incremental Model",
      "Idempotency",
      "Cost Observability"
    ],
    "symptom": "Multi-TB fact rebuilds every hour 'just to be safe'.",
    "tags": [
      "cost",
      "interview",
      "performance"
    ]
  },
  {
    "name": "Virtual Warehouse",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Dedicated compute cluster (size XS\u20134XL+) that runs queries and DML; billed while running.",
    "why": "Scale compute independently of storage; isolate workloads (ETL vs BI).",
    "example": "`CREATE WAREHOUSE etl_wh WITH WAREHOUSE_SIZE = 'M' AUTO_SUSPEND = 60;`",
    "remember": [
      "Auto-suspend saves credits",
      "Separate warehouses for noisy ETL vs dashboards"
    ],
    "mustKnow": null,
    "snippet": "CREATE WAREHOUSE etl_wh WAREHOUSE_SIZE='M' AUTO_SUSPEND=60 AUTO_RESUME=TRUE;",
    "related": [
      "Separation of Storage and Compute",
      "Result Cache",
      "Query Profile"
    ],
    "symptom": "BI and ETL share one warehouse; dashboards stall during loads.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Separation of Storage and Compute",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Data lives in cloud storage; warehouses attach compute on demand.",
    "why": "Many teams/warehouses can query the same data without copying.",
    "example": "Finance XL warehouse and Data Eng M warehouse both query `PROD.ANALYTICS` tables.",
    "remember": [
      "Storage \u2248 cheap continuous cost; compute \u2248 bursty credits",
      "No classic \u201cresize the cluster that holds the data\u201d"
    ],
    "mustKnow": null,
    "snippet": "-- same tables, different warehouses\nUSE WAREHOUSE bi_wh; SELECT ...;\nUSE WAREHOUSE etl_wh; MERGE ...;",
    "related": [
      "Virtual Warehouse",
      "Cost Observability",
      "Right-Sizing Warehouses"
    ],
    "symptom": "Team thinks they must duplicate data to isolate compute.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Micro-partitions",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Immutable ~16MB columnar storage units Snowflake manages automatically (with metadata min/max).",
    "why": "Enable pruning; clustering improves locality for filters.",
    "example": "Filter `WHERE order_date = '2024-01-01'` skips micro-partitions outside that range.",
    "remember": [
      "You don\u2019t create micro-partitions manually",
      "Clustering keys guide co-location as data grows"
    ],
    "mustKnow": null,
    "snippet": "-- check pruning in Query Profile: partitions scanned vs total",
    "related": [
      "Clustering Key",
      "Query Profile",
      "Scan Pruning / Partitioning"
    ],
    "symptom": "Filter on unhelpful column scans almost all partitions.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Clustering Key",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Optional key(s) that guide how related rows are co-located across micro-partitions.",
    "why": "Better pruning on large tables filtered by those columns.",
    "example": "`ALTER TABLE events CLUSTER BY (event_date, customer_id);`",
    "remember": [
      "Useful on multi-TB tables with repeated filter patterns",
      "Over-clustering wastes recluster credits"
    ],
    "mustKnow": null,
    "snippet": "ALTER TABLE events CLUSTER BY (event_date, customer_id);\n-- monitor SYSTEM$CLUSTERING_INFORMATION",
    "related": [
      "Micro-partitions",
      "Query Profile",
      "Cost Observability"
    ],
    "symptom": "Huge table filtered by date still reads most micro-partitions.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Time Travel (Snowflake)",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Query or restore table data as of a past timestamp/statement within retention (typically 1\u201390 days).",
    "why": "Undo bad loads; audit historical state.",
    "example": "`SELECT * FROM orders AT (TIMESTAMP => '2024-06-01 12:00:00');`",
    "remember": [
      "Longer retention \u2192 more storage cost",
      "Different from Fail-safe"
    ],
    "mustKnow": null,
    "snippet": "SELECT * FROM orders AT (TIMESTAMP => '2024-06-01 12:00:00'::TIMESTAMP_TZ);\nUNDROP TABLE orders;",
    "related": [
      "Fail-safe",
      "Zero-Copy Clone",
      "Time Travel"
    ],
    "symptom": "Bad MERGE discovered hours later with no way to see prior state.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Fail-safe",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "7-day Snowflake-managed recovery window after Time Travel ends (not for user queries).",
    "why": "Disaster recovery last resort via Snowflake Support.",
    "example": "Accidental drop discovered after Time Travel expired \u2192 Fail-safe recovery request.",
    "remember": [
      "Not a substitute for Time Travel / backups you control",
      "Historical data access only through Snowflake"
    ],
    "mustKnow": null,
    "snippet": "-- 7-day Snowflake-managed recovery AFTER time travel\n-- request via Snowflake Support (not SQL)",
    "related": [
      "Time Travel (Snowflake)",
      "Fault Tolerance",
      "Zero-Copy Clone"
    ],
    "symptom": "Expecting Fail-safe to be self-serve like AT(TIMESTAMP).",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Zero-Copy Clone",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Instant copy of database/schema/table that shares storage until data diverges.",
    "why": "Fast DEV/QA copies without duplicating bytes upfront.",
    "example": "`CREATE TABLE orders_dev CLONE prod.analytics.orders;`",
    "remember": [
      "Cheap until writes cause divergence",
      "Great for experiments and backfill tests"
    ],
    "mustKnow": null,
    "snippet": "CREATE SCHEMA analytics_dev CLONE analytics_prod;",
    "related": [
      "Time Travel (Snowflake)",
      "Virtual Warehouse",
      "Backfill"
    ],
    "symptom": "Nightly full COPY of prod into DEV burns storage and time.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Stage",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Named location for files (internal Snowflake stage or external S3/Azure/GCS) used by COPY/Snowpipe.",
    "why": "Standard landing zone for bulk loads.",
    "example": "`COPY INTO target FROM @my_ext_stage/path/ FILE_FORMAT = (TYPE=PARQUET);`",
    "remember": [
      "External stages need storage integration / credentials",
      "Prefer patterned paths by date"
    ],
    "mustKnow": null,
    "snippet": "CREATE STAGE s3_raw URL='s3://bucket/raw/' STORAGE_INTEGRATION=si;\nLIST @s3_raw;",
    "related": [
      "COPY INTO",
      "Snowpipe",
      "Object Storage"
    ],
    "symptom": "Ad-hoc credentialed COPY from laptops into prod.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "COPY INTO",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Bulk load (or unload) command between stages and tables.",
    "why": "High-throughput ingestion workhorse.",
    "example": "`COPY INTO raw.events FROM @stage/dt=2024-01-01/ PATTERN='.*[.]parquet';`",
    "remember": [
      "Idempotent-ish with force/purge options \u2014 design carefully",
      "Validate file formats and error options"
    ],
    "mustKnow": null,
    "snippet": "COPY INTO raw.events\nFROM @s3_raw/dt=2024-01-01/\nFILE_FORMAT=(TYPE=PARQUET);",
    "related": [
      "Stage",
      "Snowpipe",
      "Idempotency"
    ],
    "symptom": "Re-running COPY duplicates rows without a truncate/MERGE strategy.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Snowpipe",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Continuous/auto-ingest service that loads files from stages as they arrive (often via event notifications).",
    "why": "Near-real-time landing without scheduling heavy COPY jobs.",
    "example": "S3 event \u2192 Snowpipe \u2192 `raw.events` within minutes.",
    "remember": [
      "Great for many small files; watch pipe errors",
      "Downstream transforms still need orchestration"
    ],
    "mustKnow": null,
    "snippet": "CREATE PIPE events_pipe AUTO_INGEST=TRUE AS\nCOPY INTO raw.events FROM @s3_raw FILE_FORMAT=(TYPE=JSON);",
    "related": [
      "Stage",
      "COPY INTO",
      "Stream (Snowflake)"
    ],
    "symptom": "Pipe error queue grows; files land but table stays empty.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Stream (Snowflake)",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Change-tracking object on a table (CDC-style inserts/updates/deletes) for incremental processing.",
    "why": "Build incremental ETL without re-scanning full tables.",
    "example": "`CREATE STREAM orders_stream ON TABLE orders; SELECT * FROM orders_stream WHERE METADATA$ACTION = 'INSERT';`",
    "remember": [
      "Consuming a stream advances offsets \u2014 design transactions carefully",
      "Pair with Tasks for scheduled apply"
    ],
    "mustKnow": null,
    "snippet": "CREATE STREAM stg_orders_stream ON TABLE stg_orders;\nSELECT * FROM stg_orders_stream WHERE METADATA$ACTION='INSERT';",
    "related": [
      "Task",
      "Change Data Capture (CDC)",
      "Incremental Model"
    ],
    "symptom": "Full-table rebuild every hour instead of reading changes.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Task",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Scheduled SQL/procedure job in Snowflake (can chain into task trees).",
    "why": "Lightweight in-platform orchestration for SQL transforms.",
    "example": "Hourly task merges stream changes into a curated table.",
    "remember": [
      "Not a full Airflow replacement for complex multi-system DAGs",
      "Monitor failed task history"
    ],
    "mustKnow": null,
    "snippet": "CREATE TASK merge_orders WAREHOUSE=etl_wh SCHEDULE='60 MINUTE'\nAS MERGE INTO cur.orders t USING stg.orders_stream s ...;",
    "related": [
      "Stream (Snowflake)",
      "DAG",
      "Pipeline Orchestration"
    ],
    "symptom": "Complex multi-cloud DAG forced entirely into Snowflake Tasks.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "VARIANT",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Semi-structured column type for JSON/Avro/XML-like nested data.",
    "why": "Land flexible payloads then flatten into relational models.",
    "example": "`SELECT payload:user.id::STRING AS user_id FROM raw_events;`",
    "remember": [
      "Queryable without predefined schema",
      "Flatten hot paths into typed columns for BI performance"
    ],
    "mustKnow": null,
    "snippet": "SELECT payload:user.id::STRING AS user_id,\n       payload:events[0].type::STRING AS first_event\nFROM raw_json;",
    "related": [
      "Schema Evolution",
      "Data Lake",
      "dbt Model"
    ],
    "symptom": "BI tools choke on deep VARIANT paths with no flattened model.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Result Cache",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Cached query results reused when identical SQL hits unchanged data (24h, no warehouse needed).",
    "why": "Repeated dashboard SQL can be nearly free/instant.",
    "example": "Same BI extract refreshed by many users within minutes \u2192 cache hits.",
    "remember": [
      "Any underlying data change invalidates",
      "Different from local warehouse disk cache"
    ],
    "mustKnow": null,
    "snippet": "-- identical SQL + unchanged data => result cache (no WH credits)",
    "related": [
      "Virtual Warehouse",
      "Result Caching",
      "Query Profile"
    ],
    "symptom": "Assuming all fast queries mean the warehouse size is perfect.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Query Profile",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Visual breakdown of a query\u2019s operators, pruning, spillage, and time.",
    "why": "Primary Snowflake performance debugging tool.",
    "example": "Profile shows TableScan reading most micro-partitions \u2192 add filter/clustering.",
    "remember": [
      "Look for partition scan %, spilling, exploding joins",
      "Fix data layout before upsizing warehouse"
    ],
    "mustKnow": null,
    "snippet": "-- History \u2192 Query ID \u2192 Profile\n-- check partitions scanned, spilling, join explosion",
    "related": [
      "Micro-partitions",
      "Clustering Key",
      "Virtual Warehouse"
    ],
    "symptom": "Upsizing warehouse repeatedly without reading the profile.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Secure Data Sharing",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Share live database objects with other Snowflake accounts without copying data.",
    "why": "Provider/consumer data products with governance.",
    "example": "Share `analytics.sales_mart` to a partner reader account.",
    "remember": [
      "Consumers query shared objects in their account",
      "Revoke access instantly vs sending flat files"
    ],
    "mustKnow": null,
    "snippet": "CREATE SHARE partner_share;\nGRANT USAGE ON DATABASE analytics TO SHARE partner_share;\nGRANT SELECT ON VIEW analytics.public.sales_mart TO SHARE partner_share;",
    "related": [
      "Roles & RBAC",
      "Data Contracts",
      "Object Storage"
    ],
    "symptom": "Emailing weekly CSV extracts instead of a live share.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Roles & RBAC",
    "category": "Snowflake",
    "categorySlug": "17-snowflake",
    "file": "17-snowflake/concepts.md",
    "definition": "Access control via roles granted privileges on warehouses, databases, schemas, tables.",
    "why": "Least-privilege for ETL roles vs analyst roles.",
    "example": "`GRANT USAGE ON WAREHOUSE bi_wh TO ROLE analyst; GRANT SELECT ON ALL TABLES IN SCHEMA marts TO ROLE analyst;`",
    "remember": [
      "Users activate a primary role (plus secondary roles if enabled)",
      "Own objects carefully \u2014 avoid personal ownership in prod"
    ],
    "mustKnow": null,
    "snippet": "GRANT USAGE ON WAREHOUSE bi_wh TO ROLE analyst;\nGRANT SELECT ON ALL TABLES IN SCHEMA marts TO ROLE analyst;",
    "related": [
      "IAM",
      "Secure Data Sharing",
      "Data Governance"
    ],
    "symptom": "Everyone uses ACCOUNTADMIN for daily queries.",
    "tags": [
      "interview",
      "snowflake",
      "warehouse"
    ]
  },
  {
    "name": "Medication Order",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "A clinician\u2019s request for a drug (what should be given) \u2014 often before dispense or administration.",
    "why": "Orders \u2260 what the patient actually received; joining order\u2192admin without care doubles or misses doses.",
    "example": "Order: \u201cAmoxicillin 500 mg PO TID \u00d7 7 days.\u201d May never be dispensed if patient leaves AMA.",
    "remember": [
      "Grain is usually one row per order (or order line)",
      "Status matters: draft / active / completed / cancelled"
    ],
    "mustKnow": null,
    "snippet": "-- grain: 1 row per medication order\nSELECT patient_id, order_id, drug_name, status, ordered_at\nFROM med_order WHERE status IN ('active','completed');",
    "related": [
      "Medication Dispense",
      "Medication Administration (MAR)",
      "Order \u2192 Dispense \u2192 Administration Chain"
    ],
    "symptom": "Sepsis antibiotic metric uses order time; nursing says dose was hours later.",
    "tags": [
      "clinical",
      "healthcare",
      "medication"
    ]
  },
  {
    "name": "Medication Dispense",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Pharmacy fills/gives out a supply of medication (quantity, NDC, days supply).",
    "why": "Claims and refill analytics often live at dispense grain, not administration.",
    "example": "One order for 30 tablets \u2192 one dispense of 30; a refill is another dispense.",
    "remember": [
      "Dispense has quantity + days_supply",
      "Same drug can appear as different NDCs over time"
    ],
    "mustKnow": null,
    "snippet": "SELECT patient_id, ndc, quantity, days_supply, dispensed_at\nFROM med_dispense WHERE dispensed_at >= DATE '2024-01-01';",
    "related": [
      "NDC (National Drug Code)",
      "RxNorm",
      "Medication Order"
    ],
    "symptom": "Refill counts explode because each package NDC is treated as a new drug.",
    "tags": [
      "clinical",
      "healthcare",
      "medication"
    ]
  },
  {
    "name": "Medication Administration (MAR)",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Record that a dose was given (or intentionally not given) to the patient \u2014 Medication Administration Record.",
    "why": "Inpatient adherence, dosing safety, and \u201cdid they get the antibiotic?\u201d live here.",
    "example": "Nurse documents ceftriaxone 1 g IV given at 2024-01-02 08:12.",
    "remember": [
      "Grain: usually one row per dose event",
      "\u201cNot given\u201d reasons are first-class data"
    ],
    "mustKnow": null,
    "snippet": "-- grain: 1 row per dose event\nSELECT encounter_id, rxnorm, dose_value, dose_unit, route, administered_at, given_flag\nFROM med_admin;",
    "related": [
      "Order \u2192 Dispense \u2192 Administration Chain",
      "Sig / Dose / Route / Frequency",
      "Clinical Event Time vs System Time"
    ],
    "symptom": "Inpatient adherence looks perfect because cancelled/not-given rows were dropped.",
    "tags": [
      "clinical",
      "healthcare",
      "medication"
    ]
  },
  {
    "name": "Order \u2192 Dispense \u2192 Administration Chain",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "The typical inpatient med pipeline: ordered \u2192 dispensed \u2192 administered (ambulatory often stops at dispense).",
    "why": "Analytics fail when you pick the wrong stage for the question.",
    "example": "\u201cAntibiotics within 1h of sepsis\u201d needs **administration** time, not order time.",
    "remember": [
      "State which stage your metric uses",
      "Left joins across stages create fan-out"
    ],
    "mustKnow": null,
    "snippet": "-- pick the stage that matches the question\n-- time-to-antibiotic \u2192 administered_at\n-- refill behavior \u2192 dispensed_at\n-- intent to treat \u2192 ordered_at",
    "related": [
      "Medication Order",
      "Medication Dispense",
      "Medication Administration (MAR)"
    ],
    "symptom": "Joining order to admin without keys fans out to 5\u00d7 dose rows.",
    "tags": [
      "clinical",
      "healthcare",
      "medication"
    ]
  },
  {
    "name": "Active Medication List",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Medications believed current for the patient (home meds + active inpatient orders), not the full history.",
    "why": "Reconciliation and \u201con blood thinner?\u201d checks use the active list, not every past order.",
    "example": "Active: metformin, lisinopril. Historical: finished amoxicillin course last month.",
    "remember": [
      "Needs start/stop (or end) logic",
      "Stale \u201cactive\u201d flags are a common EHR quality issue"
    ],
    "mustKnow": null,
    "snippet": "SELECT patient_id, rxnorm, start_date, end_date\nFROM med_list WHERE end_date IS NULL OR end_date > CURRENT_DATE;",
    "related": [
      "Medication Order",
      "RxNorm"
    ],
    "symptom": "Patient marked on warfarin years after it was stopped.",
    "tags": [
      "clinical",
      "healthcare",
      "medication"
    ]
  },
  {
    "name": "RxNorm",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "NIH normalized naming system for clinical drugs (ingredients, strength, dose form).",
    "why": "Maps messy local drug names to a shared code for analytics and interoperability.",
    "example": "Map \u201cAmox 500mg Cap\u201d and \u201camoxicillin 500 mg capsule\u201d \u2192 same RxNorm CUI/SCD.",
    "remember": [
      "Prefer RxNorm over free-text drug names for joins",
      "Strength + form are part of the concept"
    ],
    "mustKnow": null,
    "snippet": "-- map local name \u2192 RxNorm for rollups\nSELECT local_drug_name, rxnorm_cui, tty\nFROM drug_map WHERE tty IN ('SCD','SBD','IN');",
    "related": [
      "NDC (National Drug Code)",
      "Medication Order"
    ],
    "symptom": "Drug utilization splits one antibiotic across 40 string spellings.",
    "tags": [
      "clinical",
      "healthcare",
      "medication"
    ]
  },
  {
    "name": "NDC (National Drug Code)",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "FDA product identifier for how a drug is packaged (labeler + product + package).",
    "why": "Billing, inventory, and dispense feeds key off NDC; same clinical drug has many NDCs.",
    "example": "Two manufacturers\u2019 bottles of sertraline 50 mg \u2192 different NDCs, same RxNorm ingredient/strength.",
    "remember": [
      "NDC changes with package/manufacturer",
      "Map NDC \u2192 RxNorm for clinical rollups"
    ],
    "mustKnow": null,
    "snippet": "-- NDC is package-level; map to RxNorm for clinical sameness\nSELECT ndc, rxnorm_cui FROM ndc_to_rxnorm WHERE ndc = '00093-3107-01';",
    "related": [
      "RxNorm",
      "Medication Dispense"
    ],
    "symptom": "Switching suppliers doubles 'distinct drugs' overnight.",
    "tags": [
      "clinical",
      "healthcare",
      "medication"
    ]
  },
  {
    "name": "Sig / Dose / Route / Frequency",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Structured (or semi-structured) instructions: how much (dose), how given (route), how often (frequency), plus free-text sig.",
    "why": "Free-text sig breaks analytics; structured fields enable dose and adherence measures.",
    "example": "Dose `500`, unit `mg`, route `PO`, frequency `TID` vs sig `\"one capsule by mouth three times daily\"`.",
    "remember": [
      "Prefer structured columns; treat sig as secondary",
      "Unit mismatches cause 10\u00d7 dosing errors in data"
    ],
    "mustKnow": null,
    "snippet": "SELECT dose_value, dose_unit, route, frequency, sig_text\nFROM med_order\nWHERE dose_unit IS NULL;  -- quality: unstructured only",
    "related": [
      "Medication Administration (MAR)",
      "Units of Measure (UCUM)"
    ],
    "symptom": "Average dose is nonsense because mg and mcg were mixed.",
    "tags": [
      "clinical",
      "healthcare",
      "medication"
    ]
  },
  {
    "name": "Lab Order vs Lab Result",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Order = request for a test; result = the reported value(s) for analyte(s).",
    "why": "Pending orders aren\u2019t results; cancelled orders still clutter feeds.",
    "example": "Order CBC \u2192 results for WBC, RBC, hemoglobin, platelets (multiple result rows).",
    "remember": [
      "One order can fan out to many result rows (panel)",
      "Always filter on result status (final vs preliminary)"
    ],
    "mustKnow": null,
    "snippet": "-- 1 order \u2192 many result rows (panel)\nSELECT o.order_id, r.loinc, r.value_num, r.value_text, r.result_status\nFROM lab_order o\nJOIN lab_result r ON r.order_id = o.order_id\nWHERE r.result_status = 'final';",
    "related": [
      "Panel vs Analyte",
      "LOINC"
    ],
    "symptom": "Lab volume metrics count orders once and results 12\u00d7 for the same CBC.",
    "tags": [
      "clinical",
      "healthcare",
      "lab"
    ]
  },
  {
    "name": "LOINC",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Universal codes for lab observations and clinical measures (what was measured).",
    "why": "Local test names differ by lab; LOINC lets you compare \u201cthe same test\u201d across sites.",
    "example": "Serum creatinine assays from Lab A and Lab B \u2192 same LOINC `2160-0`.",
    "remember": [
      "Join/analytics on LOINC, not display name",
      "Same analyte can have multiple LOINCs (method/specimen)"
    ],
    "mustKnow": null,
    "snippet": "SELECT loinc, component, system, property\nFROM loinc WHERE loinc = '2160-0';  -- creatinine",
    "related": [
      "Lab Order vs Lab Result",
      "Specimen & Collection Time"
    ],
    "symptom": "Creatinine trends break across hospitals because names differ and LOINC wasn't mapped.",
    "tags": [
      "clinical",
      "healthcare",
      "lab"
    ]
  },
  {
    "name": "Reference Range",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Lab\u2019s expected normal low/high (or qualitative normal set) for a result, often age/sex/method specific.",
    "why": "\u201cAbnormal\u201d flags and clinical decision support depend on the range in force at result time.",
    "example": "K+ result `5.8` with range `3.5\u20135.1` \u2192 high; same number might be normal for a different assay.",
    "remember": [
      "Store range with the result (don\u2019t assume today\u2019s range)",
      "Ranges differ by specimen and method"
    ],
    "mustKnow": null,
    "snippet": "SELECT value_num, ref_low, ref_high, abnormal_flag\nFROM lab_result\nWHERE loinc = '2823-3';  -- potassium",
    "related": [
      "Abnormal / Critical Flags",
      "Units of Measure (UCUM)"
    ],
    "symptom": "Historical labs look newly abnormal after a method/range change.",
    "tags": [
      "clinical",
      "healthcare",
      "lab"
    ]
  },
  {
    "name": "Units of Measure (UCUM)",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Standard representation of units (e.g. `mg/dL`, `mmol/L`) so values are comparable.",
    "why": "Mixing `mg/dL` and `mmol/L` without conversion silently corrupts trends.",
    "example": "Glucose `100 mg/dL` \u2248 `5.6 mmol/L` \u2014 not the same number.",
    "remember": [
      "Never chart mixed units without conversion",
      "Prefer UCUM codes alongside display units"
    ],
    "mustKnow": null,
    "snippet": "-- never mix units in one chart series\nSELECT loinc, value_num, unit_ucum\nFROM lab_result\nWHERE loinc = '2345-7' AND unit_ucum IN ('mg/dL','mmol/L');",
    "related": [
      "Quantitative vs Qualitative Results",
      "Reference Range"
    ],
    "symptom": "Glucose trend line jumps because mmol/L rows were plotted as mg/dL.",
    "tags": [
      "clinical",
      "healthcare",
      "lab"
    ]
  },
  {
    "name": "Specimen & Collection Time",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "What was collected (blood, urine, \u2026) and **when** it was collected \u2014 distinct from result-reported time.",
    "why": "Clinical timelines use collection time; pipelines often only get result timestamp.",
    "example": "Blood drawn 06:00, resulted 09:40 \u2014 sepsis bundle timing uses 06:00.",
    "remember": [
      "Prefer `collected_at` over `resulted_at` for physiology",
      "Wrong specimen type \u2192 wrong LOINC interpretation"
    ],
    "mustKnow": null,
    "snippet": "SELECT specimen_type, collected_at, resulted_at, value_num\nFROM lab_result\nWHERE collected_at IS NOT NULL;",
    "related": [
      "Clinical Event Time vs System Time",
      "LOINC"
    ],
    "symptom": "Bundle compliance fails clocks using resulted_at instead of draw time.",
    "tags": [
      "clinical",
      "healthcare",
      "lab"
    ]
  },
  {
    "name": "Quantitative vs Qualitative Results",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Quantitative = numeric value; qualitative = categorical (Positive/Negative, Detected, \u2026).",
    "why": "Averaging \u201cPositive\u201d or casting text to float fails; schemas must allow both.",
    "example": "Troponin `0.04 ng/mL` (quant) vs COVID PCR `Detected` (qual).",
    "remember": [
      "Keep `value_num` and `value_text` (or typed columns)",
      "Don\u2019t force all labs into one numeric column"
    ],
    "mustKnow": null,
    "snippet": "SELECT\n  loinc,\n  value_num,\n  value_text,\n  CASE WHEN value_num IS NOT NULL THEN 'quant' ELSE 'qual' END AS kind\nFROM lab_result;",
    "related": [
      "Lab Order vs Lab Result",
      "Units of Measure (UCUM)"
    ],
    "symptom": "ETL crash: cannot cast 'Detected' to decimal for all lab values.",
    "tags": [
      "clinical",
      "healthcare",
      "lab"
    ]
  },
  {
    "name": "Abnormal / Critical Flags",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Lab- or EHR-assigned markers that a result is abnormal, high/low, or critically out of range.",
    "why": "Alerts and quality metrics often key off flags, not only raw values.",
    "example": "Flag `HH` (critically high) on potassium triggers rapid notification workflows.",
    "remember": [
      "Flags are not a substitute for storing the value + range",
      "Flag vocabularies differ by vendor"
    ],
    "mustKnow": null,
    "snippet": "SELECT loinc, value_num, abnormal_flag\nFROM lab_result\nWHERE abnormal_flag IN ('H','HH','L','LL','AA');",
    "related": [
      "Reference Range",
      "PHI in Meds & Labs"
    ],
    "symptom": "Critical-lab dashboard empty because vendor uses 'critical' not 'HH'.",
    "tags": [
      "clinical",
      "healthcare",
      "lab"
    ]
  },
  {
    "name": "Panel vs Analyte",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Panel = ordered battery (CMP, CBC); analyte = individual measurable component.",
    "why": "Facts are usually at analyte-result grain; panel codes alone don\u2019t give the numbers.",
    "example": "CMP order \u2192 separate rows for sodium, potassium, creatinine, glucose, \u2026",
    "remember": [
      "Model results at analyte grain",
      "Panel code is useful lineage, not the measure"
    ],
    "mustKnow": null,
    "snippet": "-- store analyte results; keep panel code as lineage\nSELECT panel_code, loinc AS analyte, value_num\nFROM lab_result WHERE panel_code = 'CBC';",
    "related": [
      "Lab Order vs Lab Result",
      "LOINC"
    ],
    "symptom": "Fact table at panel grain cannot answer 'what was the hemoglobin?'.",
    "tags": [
      "clinical",
      "healthcare",
      "lab"
    ]
  },
  {
    "name": "Encounter / Visit Context",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "The visit (inpatient encounter, ED visit, outpatient appointment) tying meds and labs to a care episode.",
    "why": "\u201cLabs during admission\u201d and \u201cmeds given this encounter\u201d need encounter keys.",
    "example": "Join `lab_result.encounter_id` to inpatient stay for length-of-stay cohorts.",
    "remember": [
      "Some labs are outpatient with weak encounter links",
      "Declare whether grain is patient-day, encounter, or event"
    ],
    "mustKnow": null,
    "snippet": "SELECT e.encounter_id, e.encounter_type, r.loinc, r.collected_at\nFROM encounter e\nJOIN lab_result r ON r.encounter_id = e.encounter_id\nWHERE e.encounter_type = 'inpatient';",
    "related": [
      "Medication Administration (MAR)",
      "Lab Order vs Lab Result"
    ],
    "symptom": "Admission lab cohort drops half the draws with null encounter_id.",
    "tags": [
      "clinical",
      "healthcare"
    ]
  },
  {
    "name": "Clinical Event Time vs System Time",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Event time = when it happened clinically (collected, administered); system time = when the EHR recorded/updated the row.",
    "why": "Late charting makes \u201csystem time\u201d look like delayed care.",
    "example": "Dose given at 08:00, documented at 11:30 \u2014 administration event time is 08:00.",
    "remember": [
      "Prefer clinical event timestamps for outcomes",
      "Keep both; use system time for pipeline freshness"
    ],
    "mustKnow": null,
    "snippet": "SELECT administered_at AS event_time, charted_at AS system_time,\n       TIMESTAMPDIFF(MINUTE, administered_at, charted_at) AS chart_lag_min\nFROM med_admin;",
    "related": [
      "Specimen & Collection Time",
      "Medication Administration (MAR)"
    ],
    "symptom": "Care looks delayed 3 hours; nurses charted after the rush.",
    "tags": [
      "clinical",
      "healthcare"
    ]
  },
  {
    "name": "PHI in Meds & Labs",
    "category": "Clinical Data \u2014 Meds & Labs",
    "categorySlug": "18-clinical-data",
    "file": "18-clinical-data/concepts.md",
    "definition": "Medication and lab rows are Protected Health Information when identifiable (patient id + clinical facts).",
    "why": "Exports, logs, and lower environments need de-identification and access controls.",
    "example": "A table of patient_id + HIV viral load is highly sensitive PHI.",
    "remember": [
      "Minimize columns in analytic extracts",
      "Audit access; never log full result payloads casually"
    ],
    "mustKnow": null,
    "snippet": "-- minimize identifiable columns in lower envs\n-- hash patient_id; drop free-text notes; gate HIV/genetic LOINCs",
    "related": [
      "Lab Order vs Lab Result",
      "Active Medication List"
    ],
    "symptom": "Full lab extract with names landed in an unrestricted S3 bucket.",
    "tags": [
      "clinical",
      "healthcare",
      "quality"
    ]
  },
  {
    "name": "REST API",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "HTTP API that manipulates resources with methods like GET, POST, PUT/PATCH, DELETE and status codes.",
    "why": "Most SaaS extracts and internal services speak REST; pipelines are HTTP clients first.",
    "example": "`GET /v1/orders?updated_since=2024-01-01` \u2192 JSON list of orders.",
    "remember": [
      "Idempotent methods: GET, PUT, DELETE (when designed correctly)",
      "2xx success, 4xx your fault, 5xx retry carefully"
    ],
    "mustKnow": null,
    "snippet": "GET /v1/orders?updated_since=2024-01-01 HTTP/1.1\nHost: api.example.com\nAuthorization: Bearer <token>",
    "related": [
      "Endpoint / Resource",
      "Status Codes",
      "Pagination"
    ],
    "symptom": "Pipeline treats every non-200 as fatal and never retries 503s.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Endpoint / Resource",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "A URL path that represents a collection or item (`/customers`, `/customers/{id}`).",
    "why": "Your extract grain and join keys usually follow resource design.",
    "example": "`/patients/{id}/labs` returns lab results for one patient.",
    "remember": [
      "Path params identify; query params filter/sort/page",
      "Stable resource IDs beat fuzzy name matches"
    ],
    "mustKnow": null,
    "snippet": "GET /customers/{id}\nGET /customers/{id}/orders?limit=100",
    "related": [
      "REST API",
      "Pagination"
    ],
    "symptom": "Join keys missing because extract used display names instead of resource ids.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Request / Response",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Client sends a request (method, URL, headers, body); server returns a response (status, headers, body).",
    "why": "Logging only the body loses status and rate-limit headers you need to debug.",
    "example": "Request `Authorization: Bearer \u2026` \u2192 Response `200` + JSON payload.",
    "remember": [
      "Capture status, latency, and `Retry-After`",
      "Never log secrets (tokens, API keys)"
    ],
    "mustKnow": null,
    "snippet": "# log status + headers, not only body\nstatus, retry_after = resp.status_code, resp.headers.get(\"Retry-After\")",
    "related": [
      "Status Codes",
      "Rate Limiting"
    ],
    "symptom": "On-call can't tell timeout from 401 because only payloads were saved.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "JSON / Payload Schema",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Structured body format (usually JSON) with an agreed shape for fields and types.",
    "why": "Schema drift breaks pipelines; optional fields become required without notice.",
    "example": "`{ \"order_id\": \"O-1\", \"total_cents\": 1999, \"currency\": \"USD\" }`",
    "remember": [
      "Version fields or use a schema registry/contract",
      "Prefer explicit nulls over omitting keys inconsistently"
    ],
    "mustKnow": null,
    "snippet": "{\n  \"order_id\": \"O-1\",\n  \"total_cents\": 1999,\n  \"currency\": \"USD\"\n}",
    "related": [
      "API Contract / OpenAPI",
      "Versioning"
    ],
    "symptom": "Field renamed from total to total_cents; warehouse silently nulls revenue.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Status Codes",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Numeric HTTP outcomes: 2xx OK, 3xx redirect, 4xx client error, 5xx server error.",
    "why": "Retry logic must distinguish \u201cbad request\u201d from \u201ctry again.\u201d",
    "example": "`429 Too Many Requests` \u2192 back off; `401` \u2192 refresh auth, don\u2019t hammer.",
    "remember": [
      "Don\u2019t retry 400/401/403/404 blindly",
      "409 conflict often needs upsert/merge semantics"
    ],
    "mustKnow": null,
    "snippet": "# retry policy sketch\nif status in (429, 500, 502, 503, 504): backoff()\nelif status in (400, 401, 403, 404): fail_no_retry()",
    "related": [
      "Timeout / Retry / Backoff",
      "Rate Limiting"
    ],
    "symptom": "Job retries 400 Bad Request 50 times and burns the quota.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Authentication vs Authorization",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Authentication = who you are; authorization = what you\u2019re allowed to do.",
    "why": "A valid token can still lack scope to read the dataset you need.",
    "example": "Service authenticates with OAuth client credentials but isn\u2019t authorized for `/admin/exports`.",
    "remember": [
      "Check scopes/roles when extracts return empty",
      "Rotate credentials; store in a secret manager"
    ],
    "mustKnow": null,
    "snippet": "# 401 \u2192 auth; 403 \u2192 authenticated but not allowed\nif resp.status_code == 401: refresh_token()\nelif resp.status_code == 403: raise PermissionError(scope)",
    "related": [
      "OAuth 2.0 / Bearer Token",
      "API Key"
    ],
    "symptom": "Token works in Postman personal sandbox but prod service principal lacks scope.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "API Key",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Static secret sent in a header or query param to identify the calling app.",
    "why": "Simple but easy to leak in logs, notebooks, and git.",
    "example": "Header `X-API-Key: sk_live_\u2026`",
    "remember": [
      "Prefer header over query string (URLs get logged)",
      "Treat like a password; never commit"
    ],
    "mustKnow": null,
    "snippet": "headers = {\"X-API-Key\": os.environ[\"VENDOR_API_KEY\"]}\n# never: ?api_key=... in committed notebooks",
    "related": [
      "Authentication vs Authorization",
      "OAuth 2.0 / Bearer Token"
    ],
    "symptom": "Key leaked in CI logs via verbose HTTP debug.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "OAuth 2.0 / Bearer Token",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Delegated auth where the client gets a short-lived access token (often refreshable).",
    "why": "Most modern SaaS APIs require token refresh in long-running pipelines.",
    "example": "Client credentials \u2192 `access_token` \u2192 `Authorization: Bearer <token>`.",
    "remember": [
      "Refresh before expiry; handle 401 with one retry",
      "Cache tokens; don\u2019t request a new one per page"
    ],
    "mustKnow": null,
    "snippet": "POST /oauth/token\ngrant_type=client_credentials&client_id=...&client_secret=...\n# then: Authorization: Bearer <access_token>",
    "related": [
      "Authentication vs Authorization",
      "Timeout / Retry / Backoff"
    ],
    "symptom": "Long backfill dies mid-way when access token expires and isn't refreshed.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Pagination",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Splitting large result sets across pages (offset/limit, cursor/token, or page numbers).",
    "why": "Full extracts die without pagination; naive offset pages drift under concurrent writes.",
    "example": "`GET /events?cursor=eyJpZCI6MTAwfQ&limit=500`",
    "remember": [
      "Prefer cursor pagination for stable extracts",
      "Stop when next cursor is null/empty"
    ],
    "mustKnow": null,
    "snippet": "cursor = None\nwhile True:\n    page = get(\"/events\", params={\"cursor\": cursor, \"limit\": 500})\n    write(page[\"data\"])\n    cursor = page.get(\"next_cursor\")\n    if not cursor: break",
    "related": [
      "Rate Limiting",
      "Change Data via API (Incremental Extract)"
    ],
    "symptom": "Offset pages skip/dupe rows while source data is actively updating.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Rate Limiting",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Server caps how many requests you can make per window (per key, IP, or tenant).",
    "why": "Ignoring limits causes 429s, bans, and incomplete loads.",
    "example": "100 req/min; response headers `X-RateLimit-Remaining: 0`, `Retry-After: 30`.",
    "remember": [
      "Respect `Retry-After`",
      "Parallelism \u00d7 pages can explode QPS"
    ],
    "mustKnow": null,
    "snippet": "if resp.status_code == 429:\n    sleep(int(resp.headers.get(\"Retry-After\", 60)))\n    retry()",
    "related": [
      "Timeout / Retry / Backoff",
      "SLA / Quotas"
    ],
    "symptom": "10 parallel workers \u00d7 50 pages/sec \u2192 IP banned for 24h.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Idempotency Key",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Client-supplied key so retries of the same write are applied once.",
    "why": "Network timeouts cause duplicate POSTs without idempotency.",
    "example": "Header `Idempotency-Key: load-2024-01-01-batch-7` on `POST /charges`.",
    "remember": [
      "Critical for payment and webhook-driven writes",
      "Keys should be deterministic per business event"
    ],
    "mustKnow": null,
    "snippet": "headers = {\n  \"Idempotency-Key\": f\"{pipeline}-{run_id}-{record_id}\"\n}\nPOST /v1/charges",
    "related": [
      "Timeout / Retry / Backoff",
      "Webhook"
    ],
    "symptom": "Timeout on POST creates duplicate charges after retry.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Webhook",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Server pushes an HTTP callback to your URL when an event happens (vs you polling).",
    "why": "Near-real-time ingest without constant polling cost.",
    "example": "Stripe sends `POST /hooks/stripe` with `invoice.paid` payload.",
    "remember": [
      "Verify signatures; reject unsigned calls",
      "Make handlers idempotent \u2014 deliveries retry"
    ],
    "mustKnow": null,
    "snippet": "# verify signature, then upsert by event id\nassert hmac_ok(headers, body, secret)\nupsert(event_id=payload[\"id\"], payload=payload)",
    "related": [
      "Idempotency Key",
      "Polling vs Event-Driven"
    ],
    "symptom": "Provider retries webhooks; mirror table doubles without event_id dedupe.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Polling vs Event-Driven",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Polling periodically GETs for changes; event-driven consumes webhooks/streams as events arrive.",
    "why": "Polling is simple but laggy/expensive; events need reliable endpoints.",
    "example": "Poll `updated_since` every 5 minutes vs webhook on `order.updated`.",
    "remember": [
      "Polling needs a high-water mark",
      "Combine: webhook + nightly reconcile poll"
    ],
    "mustKnow": null,
    "snippet": "# hybrid\n# 1) webhook \u2192 near real time\n# 2) nightly poll updated_since = watermark - 1h",
    "related": [
      "Webhook",
      "Change Data via API (Incremental Extract)"
    ],
    "symptom": "Only webhooks \u2192 missed events during endpoint downtime forever.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "API Contract / OpenAPI",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Documented agreement on paths, params, schemas, and errors (often OpenAPI/Swagger).",
    "why": "Contracts let you generate clients and catch breaking changes in CI.",
    "example": "OpenAPI spec declares `Order.total_cents` as integer required.",
    "remember": [
      "Consumer-driven checks on staging",
      "Breaking change = version bump"
    ],
    "mustKnow": null,
    "snippet": "# openapi.yaml excerpt\nOrder:\n  required: [order_id, total_cents]\n  properties:\n    total_cents: { type: integer }",
    "related": [
      "JSON / Payload Schema",
      "Versioning"
    ],
    "symptom": "Producer ships breaking JSON; 12 Airflow DAGs fail at 3am.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Versioning",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Evolving an API without breaking clients (`/v1`, `/v2`, or header versions).",
    "why": "Unversioned breaking changes silently empty or corrupt warehouses.",
    "example": "`/v1/orders` keeps old shape while `/v2/orders` renames fields.",
    "remember": [
      "Pin version in pipeline config",
      "Run dual-read during migrations"
    ],
    "mustKnow": null,
    "snippet": "BASE = \"https://api.example.com/v1\"  # pin in config\n# dual run v1+v2 during migration",
    "related": [
      "API Contract / OpenAPI",
      "JSON / Payload Schema"
    ],
    "symptom": "Unpinned client floated to v2 and renamed fields overnight.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Timeout / Retry / Backoff",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Client gives up after N ms; retries failed calls with increasing delay (and jitter).",
    "why": "Aggressive retries amplify outages; no retries drop data.",
    "example": "Timeout 30s; retry 5xx/429 with exponential backoff + jitter.",
    "remember": [
      "Cap max retries and total time",
      "Idempotent retries only for safe/write-keyed calls"
    ],
    "mustKnow": null,
    "snippet": "for attempt in range(5):\n    try:\n        return call(timeout=30)\n    except Transient:\n        sleep(min(2**attempt, 60) + jitter())",
    "related": [
      "Rate Limiting",
      "Idempotency Key"
    ],
    "symptom": "Immediate retries from 100 workers turn a blip into an outage.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Synchronous vs Asynchronous API",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Sync returns the final result in one response; async accepts work and returns a job id to poll or webhook later.",
    "why": "Big exports are often async \u2014 treating them as sync causes timeouts.",
    "example": "`POST /exports` \u2192 `202` + `job_id` \u2192 poll `/exports/{job_id}` until `download_url`.",
    "remember": [
      "Store job ids for recovery",
      "Don\u2019t block an Airflow worker for hours without deferral"
    ],
    "mustKnow": null,
    "snippet": "job = POST(\"/exports\", {\"from\": date})\nwhile job[\"status\"] != \"done\":\n    job = GET(f\"/exports/{job['id']}\")\ndownload(job[\"url\"])",
    "related": [
      "Timeout / Retry / Backoff",
      "Polling vs Event-Driven"
    ],
    "symptom": "Airflow task times out waiting for a 2-hour export in one HTTP call.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "GraphQL (Basics)",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Query language API where the client asks for exact fields in one request (vs many REST round-trips).",
    "why": "Flexible reads; easy to over-fetch or hit cost/complexity limits.",
    "example": "Query `{ order(id:\"O-1\") { id total items { sku qty } } }`",
    "remember": [
      "Paginate connections explicitly",
      "Watch rate/cost limits on nested queries"
    ],
    "mustKnow": null,
    "snippet": "query {\n  order(id: \"O-1\") {\n    id total\n    items(first: 50) { sku qty }\n  }\n}",
    "related": [
      "Pagination",
      "Rate Limiting"
    ],
    "symptom": "Nested GraphQL extract hits complexity limit and returns partial data.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "gRPC / Protobuf (Basics)",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Binary RPC framework using Protocol Buffers \u2014 common for internal low-latency services.",
    "why": "Faster than JSON REST inside the datacenter; schemas are strongly typed.",
    "example": "`GetOrder(OrderRequest) returns (Order)` over HTTP/2.",
    "remember": [
      "Need stub generation and proto versioning",
      "Harder to curl; invest in tooling"
    ],
    "mustKnow": null,
    "snippet": "// order.proto\nrpc GetOrder(OrderRequest) returns (Order);",
    "related": [
      "API Contract / OpenAPI",
      "Versioning"
    ],
    "symptom": "Proto field number reused; old consumers read garbage values.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Change Data via API (Incremental Extract)",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Pull only new/changed rows using cursors, `updated_at` filters, or event APIs.",
    "why": "Full dumps don\u2019t scale; incremental needs trustworthy watermarks.",
    "example": "`GET /objects?updated_after={watermark}&sort=updated_at`",
    "remember": [
      "Overlap windows to avoid missing late updates",
      "Periodic full reconcile catches API gaps"
    ],
    "mustKnow": null,
    "snippet": "watermark = state.get(\"updated_at\")\npage = GET(\"/objects\", {\"updated_after\": watermark - overlap})\nstate[\"updated_at\"] = max(row.updated_at)",
    "related": [
      "Pagination",
      "Polling vs Event-Driven"
    ],
    "symptom": "Watermark advances past late-updated rows; incremental forever incomplete.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "SLA / Quotas",
    "category": "APIs for Data Engineers",
    "categorySlug": "19-api",
    "file": "19-api/concepts.md",
    "definition": "Provider promises (latency, uptime) and hard caps (rows/day, concurrent calls).",
    "why": "Pipeline design must fit quotas or fail mid-load.",
    "example": "Free tier 10k calls/day; enterprise 100 concurrent connections.",
    "remember": [
      "Budget calls per DAG run",
      "Alert before quota exhaustion"
    ],
    "mustKnow": null,
    "snippet": "# budget: 50k calls/day \u2192 ~30 calls/min sustained\nmax_workers = min(config.workers, quota.safe_concurrency)",
    "related": [
      "Rate Limiting",
      "REST API"
    ],
    "symptom": "Monday backfill exhausts monthly quota by noon.",
    "tags": [
      "api",
      "pipeline"
    ]
  },
  {
    "name": "Parquet",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Columnar binary file format with per-column compression, stats (min/max), and nested types (often via Arrow/Spark).",
    "why": "Default for analytics lakes \u2014 scans read only needed columns and prune with stats.",
    "example": "`df.write.mode(\"overwrite\").parquet(\"s3://lake/events/\")`",
    "remember": [
      "Great for OLAP; not ideal for single-row point lookups",
      "Schema is embedded in the file footer"
    ],
    "mustKnow": null,
    "snippet": "df.write.mode(\"overwrite\").option(\"compression\", \"zstd\").parquet(\"s3://lake/events/dt=2024-01-01\")",
    "related": [
      "Columnar vs Row-Oriented",
      "Parquet Footer / Footer Statistics",
      "File Size Tuning"
    ],
    "symptom": "Wide SELECT * on Parquet is almost as slow as scanning a row format.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Columnar vs Row-Oriented",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Columnar stores values by column across rows; row-oriented stores whole rows together (CSV, JSON lines, Avro records).",
    "why": "Analytics queries touch few columns \u2014 columnar cuts I/O dramatically.",
    "example": "SELECT `user_id, amount` from a 200-column Parquet table reads ~2 columns, not all 200.",
    "remember": [
      "Lakes/warehouses \u2192 columnar",
      "Event logs / streaming payloads \u2192 often row (Avro/JSON)"
    ],
    "mustKnow": null,
    "snippet": "-- columnar reads only needed columns\nSELECT user_id, amount FROM events_parquet WHERE dt = '2024-01-01';",
    "related": [
      "Parquet",
      "ORC",
      "Avro"
    ],
    "symptom": "Lake bills soar because every query rewrites/reads full-row JSON dumps.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Compression Codec",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Algorithm used inside a format (Snappy, ZSTD, Gzip, LZ4, Brottli, \u2026).",
    "why": "Codec tradeoff is CPU vs size vs splittability for parallel reads.",
    "example": "Parquet + Snappy for fast ETL; ZSTD for colder, denser storage.",
    "remember": [
      "Prefer splittable codecs for large files in Spark",
      "Don\u2019t double-compress (Gzip whole file *and* inner Parquet)"
    ],
    "mustKnow": null,
    "snippet": "spark.conf.set(\"spark.sql.parquet.compression.codec\", \"zstd\")",
    "related": [
      "Snappy / ZSTD / Gzip",
      "Parquet"
    ],
    "symptom": "CPU-bound jobs after switching all datasets to max-level Gzip.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet",
      "performance"
    ]
  },
  {
    "name": "Snappy / ZSTD / Gzip",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Common codecs \u2014 Snappy (fast), ZSTD (strong ratio, tunable), Gzip (ubiquitous, often slower / less friendly for splits).",
    "why": "Wrong codec slows jobs or bloats S3 bills.",
    "example": "`parquet.compression=zstd` in Spark writer options.",
    "remember": [
      "Snappy \u2248 speed; ZSTD \u2248 size",
      "Gzip\u2019d CSV is common but painful at scale"
    ],
    "mustKnow": null,
    "snippet": "# prefer Parquet+Snappy/ZSTD over .csv.gz for big scans",
    "related": [
      "Compression Codec",
      "Splittable File"
    ],
    "symptom": "One 20GB .csv.gz file \u2192 single Spark task forever.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Row Group / Stripe",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Parquet row group (or ORC stripe) \u2014 a horizontal chunk of rows with its own column chunks and stats.",
    "why": "Predicate pushdown and parallelism operate at this granularity.",
    "example": "128MB row groups; filter `dt='2024-01-01'` skips row groups whose max(dt) is older.",
    "remember": [
      "Too-small row groups \u2192 many footers / files overhead",
      "Too-large \u2192 weaker prune + heavy memory"
    ],
    "mustKnow": null,
    "snippet": "# parquet.block.size / row group size affects prune + memory\nspark.conf.set(\"parquet.block.size\", 128 * 1024 * 1024)",
    "related": [
      "Parquet Footer / Footer Statistics",
      "Predicate Pushdown (Files)"
    ],
    "symptom": "Filters never skip data because each file is one huge row group of mixed dates.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Page / Column Chunk",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Inside a row group, each column is stored as chunks/pages (data + optional dictionary page).",
    "why": "Dictionary encoding and page stats speed decoding and filters.",
    "example": "Low-cardinality `country` column stores a dictionary once per chunk.",
    "remember": [
      "Dictionary encoding shines on low cardinality",
      "High-cardinality strings may skip dictionaries"
    ],
    "mustKnow": null,
    "snippet": "# low-cardinality columns benefit from dictionary pages",
    "related": [
      "Encoding (Dictionary / RLE / Plain)",
      "Parquet"
    ],
    "symptom": "String column of UUIDs bloats files \u2014 dictionaries don't help.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Parquet Footer / Footer Statistics",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Trailing metadata in a Parquet file: schema, row-group offsets, and min/max/null counts per column chunk.",
    "why": "Engines read the footer first to prune row groups without scanning data.",
    "example": "Query `WHERE amount > 1000` skips row groups with `max(amount) <= 1000`.",
    "remember": [
      "Corrupt/truncated footers make files unreadable",
      "Stats enable skip; they don\u2019t replace partition layout"
    ],
    "mustKnow": null,
    "snippet": "# engines read footer first for min/max prune\n# corrupt footer \u2192 file unreadable even if bytes remain",
    "related": [
      "Predicate Pushdown (Files)",
      "Row Group / Stripe"
    ],
    "symptom": "Query scans every file despite selective filters \u2014 stats missing or types mismatched.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Predicate Pushdown (Files)",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Pushing filters into the scan so the reader skips row groups/pages using stats (and partitions).",
    "why": "\u201cFilter early\u201d only works if the format + layout expose prune-able metadata.",
    "example": "Spark reads Parquet with `filter(\"event_date = '2024-01-01'\")` \u2192 partition + row-group prune.",
    "remember": [
      "Works best with Parquet/ORC + partition columns",
      "Casts/functions on columns can disable prune"
    ],
    "mustKnow": null,
    "snippet": "spark.read.parquet(path).filter(\"dt = '2024-01-01' AND amount > 100\")",
    "related": [
      "Partition Layout (Files on Disk)",
      "Parquet Footer / Footer Statistics"
    ],
    "symptom": "CAST(dt AS string) in filter disables partition/stat pruning.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet",
      "performance"
    ]
  },
  {
    "name": "Schema Evolution (Files)",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Adding/renaming/dropping fields over time while old files remain readable.",
    "why": "Lakes accumulate files written under different schemas.",
    "example": "New column `device_type` added; old Parquet files return null for that field.",
    "remember": [
      "Prefer additive changes; renames break readers",
      "Table formats (Iceberg/Delta) track schema history better than raw files alone"
    ],
    "mustKnow": null,
    "snippet": "# additive column: old files \u2192 null for new field\nspark.read.parquet(path)  # merge schemas carefully",
    "related": [
      "Parquet",
      "Delta / Iceberg / Hudi Data Files"
    ],
    "symptom": "Renamed column creates two half-null fields across history.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Avro",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Row-oriented binary format with a JSON schema; common in Kafka and streaming.",
    "why": "Compact, schema\u2019d events; evolved with Schema Registry.",
    "example": "Produce `OrderCreated` Avro to Kafka; Spark Structured Streaming reads with from_avro.",
    "remember": [
      "Better for write-heavy / record-at-a-time than wide analytics scans",
      "Pair with schema registry for compatibility"
    ],
    "mustKnow": null,
    "snippet": "# Kafka + Schema Registry\n# value.serializer = KafkaAvroSerializer",
    "related": [
      "Columnar vs Row-Oriented",
      "JSON / JSON Lines (NDJSON)"
    ],
    "symptom": "Analytics team scans raw Avro topics like a warehouse \u2014 I/O explodes.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet",
      "streaming"
    ]
  },
  {
    "name": "ORC",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Columnar format popular in Hive ecosystems (stripes, indexes, ACID tables historically).",
    "why": "Still common in Hadoop/Hive estates alongside Parquet.",
    "example": "Hive table `STORED AS ORC` with ZLIB compression.",
    "remember": [
      "Similar goals to Parquet (columnar + stats)",
      "Prefer one columnar standard per lake when possible"
    ],
    "mustKnow": null,
    "snippet": "CREATE TABLE t (...) STORED AS ORC TBLPROPERTIES (\"orc.compress\"=\"ZLIB\");",
    "related": [
      "Parquet",
      "Columnar vs Row-Oriented"
    ],
    "symptom": "Mixed Parquet+ORC lake confuses tooling and doubles reader codepaths.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Arrow (In-Memory)",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Columnar in-memory representation (and IPC/flight) used by pandas, Spark, DuckDB, etc.",
    "why": "Zero-copy interchange between engines; Parquet \u2194 Arrow is a common path.",
    "example": "`pyarrow.parquet.read_table(...)` \u2192 Arrow table \u2192 pandas.",
    "remember": [
      "Arrow \u2260 on-disk lake format (though IPC files exist)",
      "Speeds Python/Spark handoffs"
    ],
    "mustKnow": null,
    "snippet": "import pyarrow.parquet as pq\ntable = pq.read_table(\"events.parquet\", columns=[\"user_id\", \"amount\"])",
    "related": [
      "Parquet",
      "Columnar vs Row-Oriented"
    ],
    "symptom": "Pandas read_csv of a huge extract OOMs; Arrow/Parquet path would prune columns.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "CSV",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Delimited text rows; no embedded types or compression standards.",
    "why": "Universal exchange format; terrible default for large analytics.",
    "example": "`user_id,event_time,amount`",
    "remember": [
      "Ambiguous types, escaping, encodings (UTF-8 vs Latin-1)",
      "Convert to Parquet early in the lake"
    ],
    "mustKnow": null,
    "snippet": "# ok for exchange; convert early\n# sep, quote, escape, encoding must be explicit",
    "related": [
      "JSON / JSON Lines (NDJSON)",
      "Parquet"
    ],
    "symptom": "Commas inside quoted fields shift columns; types inferred wrong every run.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "JSON / JSON Lines (NDJSON)",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "JSON objects \u2014 as an array file or one object per line (JSONL/NDJSON).",
    "why": "APIs and logs emit JSON; nested fields map unevenly to tables.",
    "example": "`{\"id\":1,\"items\":[{\"sku\":\"A\"}]}` per line in `.jsonl`.",
    "remember": [
      "JSONL streams better than one giant array",
      "Inferring schema from JSON is fragile \u2014 enforce a contract"
    ],
    "mustKnow": null,
    "snippet": "{\"id\":1,\"ts\":\"2024-01-01T00:00:00Z\"}\n{\"id\":2,\"ts\":\"2024-01-01T00:01:00Z\"}",
    "related": [
      "Avro",
      "CSV"
    ],
    "symptom": "One giant JSON array file can't stream; memory spikes on read.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Delta / Iceberg / Hudi Data Files",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Open table formats that store data mainly as Parquet (sometimes ORC) plus transaction metadata.",
    "why": "You still tune Parquet layout; the table layer adds ACID, time travel, and compaction.",
    "example": "Delta `OPTIMIZE` rewrites small Parquet files into larger ones.",
    "remember": [
      "Table format \u2260 replacing Parquet \u2014 it orchestrates Parquet files",
      "Small-file problems are still Parquet layout problems"
    ],
    "mustKnow": null,
    "snippet": "# data still Parquet underneath\nOPTIMIZE lake.events;  -- compaction rewrites small files",
    "related": [
      "Parquet",
      "File Size Tuning",
      "Manifest / File List"
    ],
    "symptom": "Blaming Delta for slowness when the real issue is 2M tiny Parquet files.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "File Size Tuning",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Targeting healthy on-disk sizes (often ~128MB\u20131GB compressed) per file for engine parallelism.",
    "why": "Tiny files kill listing/planning; huge files hurt split balance and recovery.",
    "example": "Compact 10,000 \u00d7 2MB Parquet files \u2192 ~200 \u00d7 100MB files.",
    "remember": [
      "Streaming micro-batches create small files \u2014 schedule compaction",
      "Aim for fewer, fatter files in hot tables"
    ],
    "mustKnow": null,
    "snippet": "# target ~128MB-1GB compressed files for batch tables",
    "related": [
      "Delta / Iceberg / Hudi Data Files",
      "Partition Layout (Files on Disk)"
    ],
    "symptom": "Spark driver OOMs listing 5 million small objects before any scan.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet",
      "performance"
    ]
  },
  {
    "name": "Splittable File",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "A file that workers can read in byte-range splits in parallel (e.g. uncompressed or block-compressed Parquet).",
    "why": "Non-splittable Gzip CSV forces one task per file.",
    "example": "8GB Gzip CSV \u2192 one giant task; 8GB Parquet \u2192 many parallel tasks.",
    "remember": [
      "Parquet/ORC are designed for parallel reads",
      "Avoid Gzip-on-the-outside for big batch inputs"
    ],
    "mustKnow": null,
    "snippet": "# good: Parquet/ORC\n# bad: single huge .gz CSV for parallel ETL",
    "related": [
      "Snappy / ZSTD / Gzip",
      "Parquet"
    ],
    "symptom": "Cluster idle except one executor reading a non-splittable blob.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet",
      "performance"
    ]
  },
  {
    "name": "Partition Layout (Files on Disk)",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Directory layout like `dt=2024-01-01/country=US/*.parquet` that engines prune by path.",
    "why": "Partition columns are the coarsest, cheapest filter.",
    "example": "`s3://lake/events/dt=2024-01-01/*.parquet`",
    "remember": [
      "Don\u2019t over-partition (e.g. by user_id \u2192 millions of folders)",
      "Partition on selective, commonly filtered columns (often date)"
    ],
    "mustKnow": null,
    "snippet": "s3://lake/events/dt=2024-01-01/part-000.parquet",
    "related": [
      "Predicate Pushdown (Files)",
      "File Size Tuning"
    ],
    "symptom": "Partitioned by user_id \u2192 empty folders and unusable planning times.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Manifest / File List",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Inventory of data files belonging to a table version (Iceberg/Delta manifests; Hive directory listing).",
    "why": "Planning queries means knowing which files to read \u2014 listing millions of objects is slow.",
    "example": "Iceberg manifest lists Parquet paths for snapshot `N`.",
    "remember": [
      "Table formats avoid repeated S3 LIST storms",
      "Raw Hive-style dirs scale poorly with file count"
    ],
    "mustKnow": null,
    "snippet": "# Iceberg/Delta track files in manifests instead of LIST /*",
    "related": [
      "Delta / Iceberg / Hudi Data Files",
      "File Size Tuning"
    ],
    "symptom": "Hive-style dir listing dominates query time on large tables.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Bloom Filter (Parquet)",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "Optional probabilistic structure to skip row groups that definitely don\u2019t contain a value.",
    "why": "Helps selective point-ish filters on high-cardinality columns when stats alone aren\u2019t enough.",
    "example": "Enable bloom filters on `order_id` for \u201cfetch these ids\u201d style scans.",
    "remember": [
      "Not a replacement for an index/DBMS",
      "Extra write CPU and metadata size"
    ],
    "mustKnow": null,
    "snippet": "# enable bloom filter on selective id columns (engine-specific writer flags)",
    "related": [
      "Parquet Footer / Footer Statistics",
      "Predicate Pushdown (Files)"
    ],
    "symptom": "Point-lookup style filters still read most row groups \u2014 no bloom/stats help.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Encoding (Dictionary / RLE / Plain)",
    "category": "File Formats",
    "categorySlug": "20-file-formats",
    "file": "20-file-formats/concepts.md",
    "definition": "How column values are packed \u2014 dictionary, run-length, delta, plain bytes, etc.",
    "why": "Right encoding shrinks files and speeds decode for repetitive data.",
    "example": "Booleans and repeated enums compress extremely well with RLE/dictionary.",
    "remember": [
      "Engines usually choose encodings automatically",
      "Cardinality drives dictionary effectiveness"
    ],
    "mustKnow": null,
    "snippet": "# low cardinality \u2192 dictionary + RLE\n# unique IDs \u2192 plain / delta often better",
    "related": [
      "Page / Column Chunk",
      "Parquet"
    ],
    "symptom": "Unexpectedly large Parquet for enum-like columns written as plain strings.",
    "tags": [
      "file-format",
      "lakehouse",
      "parquet"
    ]
  },
  {
    "name": "Dimensional Modeling (Kimball)",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "Analytics design that centers on **facts** (measures at a declared grain) and **dimensions** (descriptive context), usually published as star or snowflake marts.",
    "why": "Matches how BI users think (\u201csales by product by day\u201d) and keeps metric definitions tied to grain.",
    "example": "`fact_order_line` + `dim_product` + `dim_customer` + `dim_date` for revenue reporting.",
    "remember": [
      "Declare grain in one sentence before keys and measures",
      "Conformed dimensions unlock drill-across across marts"
    ],
    "mustKnow": null,
    "snippet": "-- grain: one row per order line\n-- fact_order_line + dim_product + dim_customer + dim_date",
    "related": [
      "Star Schema",
      "Grain",
      "Fact Table",
      "Modeling Pattern Choice"
    ],
    "symptom": "BI metrics disagree or cannot drill by the attributes users expect.",
    "tags": [
      "interview",
      "modeling",
      "warehouse"
    ]
  },
  {
    "name": "Star Schema",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "Fact table in the center with denormalized dimension tables around it (few joins, wide dims).",
    "why": "Simple for BI tools and analysts; usually the default serving layer for dashboards.",
    "example": "`fact_sales` \u2192 `dim_store`, `dim_product`, `dim_date` (product attributes live on `dim_product`, not nested tables).",
    "remember": [
      "Prefer star for marts / gold serving",
      "Dimensions are often denormalized on purpose"
    ],
    "mustKnow": null,
    "snippet": "fact_* --FK--> dim_* (denormalized attributes on dims)",
    "related": [
      "Snowflake Schema",
      "Dimensional Modeling (Kimball)",
      "One Big Table (OBT)"
    ],
    "symptom": "Analysts drown in joins or dashboards are slow \u2014 consider star (or OBT) for serving.",
    "tags": [
      "interview",
      "modeling",
      "warehouse"
    ]
  },
  {
    "name": "Snowflake Schema",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "Dimensions are normalized into related sub-tables (product \u2192 brand \u2192 category), so the model \u201csnowflakes\u201d outward.",
    "why": "Saves storage and avoids repeating attributes, but adds joins and complexity for BI.",
    "example": "`dim_product` \u2192 `dim_brand` \u2192 `dim_category` instead of repeating brand/category on every product row.",
    "remember": [
      "Use sparingly; most teams stay star at the mart",
      "Fine in warehouse storage layer if tools hide the joins"
    ],
    "mustKnow": null,
    "snippet": "dim_product -> dim_brand -> dim_category (normalized dims)",
    "related": [
      "Star Schema",
      "Dimensional Modeling (Kimball)"
    ],
    "symptom": "Too many dimension joins for a simple dashboard filter.",
    "tags": [
      "interview",
      "modeling",
      "warehouse"
    ]
  },
  {
    "name": "Fact Table",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "Table of events or measurements at a fixed grain; foreign keys to dimensions plus numeric measures.",
    "why": "Facts are where KPIs live; wrong grain or missing keys break every report built on top.",
    "example": "One row per order line: `order_line_key`, `date_key`, `product_key`, `qty`, `amount`.",
    "remember": [
      "Keys + measures; grain first",
      "Transaction, periodic snapshot, accumulating snapshot are common patterns"
    ],
    "mustKnow": null,
    "snippet": "fact: keys + measures at one declared grain",
    "related": [
      "Dimension Table",
      "Grain",
      "Additive Facts"
    ],
    "symptom": "Counts and sums double or disappear after a join \u2014 grain/keys wrong.",
    "tags": [
      "interview",
      "modeling",
      "warehouse"
    ]
  },
  {
    "name": "Dimension Table",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "Descriptive attributes used to filter, group, and label facts (who, what, where, when).",
    "why": "Stable, conformed dims make metrics comparable across domains.",
    "example": "`dim_customer` with name, segment, region; facts store `customer_key`.",
    "remember": [
      "Surrogate keys + natural/business keys",
      "Slowly changing dims (SCD) when history matters"
    ],
    "mustKnow": null,
    "snippet": "dim: surrogate_key + business_key + attributes (+ SCD cols)",
    "related": [
      "Fact Table",
      "Conformed Dimension",
      "SCD Type 2"
    ],
    "symptom": "Same customer looks different across two marts.",
    "tags": [
      "interview",
      "modeling",
      "warehouse"
    ]
  },
  {
    "name": "One Big Table (OBT)",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "A single wide, denormalized table with facts and dimension attributes flattened into columns (often the final \u201cgold\u201d or ML feature table).",
    "why": "Fast and simple for one use case (dashboard, notebook, model training); painful when many consumers need different grains or shared definitions.",
    "example": "`orders_obt` with `order_id`, `amount`, `customer_name`, `customer_segment`, `product_name`, `category` all on one row.",
    "remember": [
      "Great for a single product/team slice; weak as the enterprise source of truth",
      "Duplication and SCD history explode unless carefully designed",
      "Common pattern: Vault or 3NF \u2192 star \u2192 optional OBT for a consumer"
    ],
    "mustKnow": null,
    "snippet": "-- wide denormalized row for one consumer\n-- order_id, amount, customer_*, product_* all in one table",
    "related": [
      "Star Schema",
      "Data Vault",
      "Modeling Pattern Choice"
    ],
    "symptom": "One team wants a single flat table; enterprise shared metrics keep breaking.",
    "tags": [
      "interview",
      "modeling",
      "warehouse"
    ]
  },
  {
    "name": "Data Vault",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "Modeling method for the **enterprise integration** layer: Hubs (business keys), Links (relationships), Satellites (descriptive/history) \u2014 built for auditability and multi-source change.",
    "why": "Absorbs source change and history without rewriting marts every time; separates \u201cwhat happened\u201d storage from \u201chow we present it.\u201d",
    "example": "Hub Customer, Hub Product, Link Order, Satellites for customer attributes from CRM and ERP.",
    "remember": [
      "Vault is usually raw/integration (silver), not the BI star",
      "Build dimensional or OBT marts **on top** of Vault",
      "Insert-mostly / historized by design"
    ],
    "mustKnow": null,
    "snippet": "Hub (BK) + Link (relationship) + Satellite (attrs/history)",
    "related": [
      "Hub (Data Vault)",
      "Link (Data Vault)",
      "Satellite (Data Vault)",
      "Modeling Pattern Choice"
    ],
    "symptom": "Sources keep changing and rebuilding stars from scratch is too slow.",
    "tags": [
      "interview",
      "modeling",
      "warehouse"
    ]
  },
  {
    "name": "Hub (Data Vault)",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "Table that stores a unique **business key** (and load metadata) for a core business entity \u2014 no descriptive attributes.",
    "why": "Stable identity across sources; hubs don\u2019t churn when attributes change.",
    "example": "`hub_customer(customer_hk, customer_bk, load_dts, record_source)`.",
    "remember": [
      "Business key only (+ hash key, load timestamp, source)",
      "Attributes go in satellites"
    ],
    "mustKnow": null,
    "snippet": "hub_customer(hk, business_key, load_dts, record_source)",
    "related": [
      "Satellite (Data Vault)",
      "Link (Data Vault)",
      "Data Vault"
    ],
    "symptom": "Need a stable identity across CRM + ERP without mixing attributes.",
    "tags": [
      "interview",
      "modeling",
      "warehouse"
    ]
  },
  {
    "name": "Link (Data Vault)",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "Table that records a **relationship** (or transaction) between hubs \u2014 the association, not the descriptions.",
    "why": "Many-to-many and multi-source relationships stay explicit and historized.",
    "example": "`link_order(order_hk, customer_hk, product_hk, load_dts, record_source)` for \u201ccustomer bought product.\u201d",
    "remember": [
      "Links connect hubs; satellites hang off hubs or links",
      "Same relationship from two sources can share structure carefully"
    ],
    "mustKnow": null,
    "snippet": "link_order(order_hk, customer_hk, product_hk, load_dts, source)",
    "related": [
      "Hub (Data Vault)",
      "Satellite (Data Vault)",
      "Data Vault"
    ],
    "symptom": "Relationships (who bought what) need history independent of attribute changes.",
    "tags": [
      "interview",
      "modeling",
      "warehouse"
    ]
  },
  {
    "name": "Satellite (Data Vault)",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "Descriptive and historized attributes for a hub or link \u2014 where SCD-like change lives in Vault.",
    "why": "Source systems change fields independently; satellites isolate that churn.",
    "example": "`sat_customer_crm` (name, email, segment) and `sat_customer_erp` (credit_limit) on the same hub.",
    "remember": [
      "One satellite per rate-of-change / source family is common",
      "Load timestamp + hashdiff for change detection"
    ],
    "mustKnow": null,
    "snippet": "sat_* (parent_hk, load_dts, hashdiff, attrs...) \u2014 historized",
    "related": [
      "Hub (Data Vault)",
      "SCD Type 2",
      "Data Vault"
    ],
    "symptom": "Attribute history from one source must not overwrite another source's fields.",
    "tags": [
      "cdc",
      "interview",
      "modeling",
      "warehouse"
    ]
  },
  {
    "name": "Raw Vault vs Business Vault",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "**Raw Vault** mirrors source-driven hubs/links/sats with minimal business rules; **Business Vault** adds soft rules, computed satellites, and cleaned keys closer to enterprise meaning.",
    "why": "Keeps auditable raw history separate from interpreted business logic.",
    "example": "Raw: CRM customer as loaded. Business: standardized phone, golden customer match key, derived \u201cactive_flag.\u201d",
    "remember": [
      "Don\u2019t bury heavy BI metrics in Raw Vault",
      "Marts still sit above Business Vault / information marts"
    ],
    "mustKnow": null,
    "snippet": "Raw = source-shaped hubs/links/sats\nBusiness = rules, cleansed keys, derived sats",
    "related": [
      "Data Vault",
      "Data Contracts",
      "Modeling Pattern Choice"
    ],
    "symptom": "Business rules buried in the auditable raw layer \u2014 hard to explain lineage.",
    "tags": [
      "interview",
      "modeling",
      "warehouse"
    ]
  },
  {
    "name": "Inmon (CIF / EDW)",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "Enterprise warehouse approach: normalized integration model (often 3NF) as the system of record, then departmental dimensional marts for consumption.",
    "why": "Strong for enterprise consistency; heavier to build and change than pure Kimball marts.",
    "example": "Normalized EDW tables for customer/order \u2192 sales mart star for Finance.",
    "remember": [
      "Inmon \u2248 integrate normalized first; Kimball \u2248 marts first",
      "Modern lakes often mix: Vault/3NF silver + Kimball/OBT gold"
    ],
    "mustKnow": null,
    "snippet": "normalized EDW (integrate) \u2192 dimensional marts (serve)",
    "related": [
      "Dimensional Modeling (Kimball)",
      "Data Vault",
      "Modeling Pattern Choice"
    ],
    "symptom": "Need one enterprise integrated model before many departmental marts.",
    "tags": [
      "interview",
      "modeling",
      "warehouse"
    ]
  },
  {
    "name": "Modeling Pattern Choice",
    "category": "Data Modeling Patterns",
    "categorySlug": "21-data-modeling",
    "file": "21-data-modeling/concepts.md",
    "definition": "Picking Vault / dimensional / OBT (or a layered mix) based on change rate, consumers, and who owns metrics.",
    "why": "Wrong pattern \u2192 either endless remapping (too rigid) or metric chaos (too flat).",
    "example": "Multi-source core \u2192 Data Vault; Finance KPIs \u2192 star mart; one ML feature set \u2192 OBT built from the mart/Vault.",
    "remember": [
      "Integration layer: Vault or normalized EDW",
      "Serving layer: star (shared metrics) or OBT (single consumer)",
      "Don\u2019t make OBT your only enterprise model"
    ],
    "mustKnow": null,
    "snippet": "integrate: Vault/3NF\nserve shared KPIs: star\nserve one consumer: OBT",
    "related": [
      "Data Vault",
      "Star Schema",
      "One Big Table (OBT)",
      "Dimensional Modeling (Kimball)"
    ],
    "symptom": "Team debates Vault vs star vs OBT \u2014 pick by layer and consumer, not fashion.",
    "tags": [
      "interview",
      "modeling",
      "warehouse"
    ]
  }
];
window.DE_SCENARIOS = [
  {
    "id": "dup-orders-retry",
    "title": "Double-counted orders after retry",
    "prompt": "A daily orders job failed halfway, was rerun, and revenue doubled for that day. Which 3 concepts best explain the failure and the fix?",
    "answers": [
      "Idempotency",
      "At Least Once",
      "Checkpointing"
    ],
    "explain": "Retries replay work. Without idempotent writes (overwrite/MERGE) and clear checkpoints/offsets, at-least-once delivery duplicates rows."
  },
  {
    "id": "straggler-join",
    "title": "One task runs forever on a join",
    "prompt": "Spark UI shows 199 tasks done in 2 minutes and 1 task still running after 40 minutes on a big join. Which concepts apply?",
    "answers": [
      "Data Skew",
      "Shuffle",
      "Salting"
    ],
    "explain": "A hot key balloons one shuffle partition. Skew detection/salting/AQE skew join spreads the hot key."
  },
  {
    "id": "stale-green",
    "title": "Job is green but dashboard is wrong",
    "prompt": "Airflow DAG succeeded, yet executives say numbers are from yesterday. Which concepts matter?",
    "answers": [
      "Freshness",
      "Observability",
      "SLA / Callback"
    ],
    "explain": "Pipeline success \u2260 data freshness. Track max(updated_at)/partition arrival and alert on SLAs, not only task status."
  },
  {
    "id": "small-files-slow",
    "title": "Queries got slower every week",
    "prompt": "A streaming job writes continuously to a lake table. Listing and queries crawl. Which concepts?",
    "answers": [
      "Small File Problem",
      "Compaction",
      "File Size Tuning"
    ],
    "explain": "Micro-batches create tiny files. Compact/optimize and target healthy file sizes."
  },
  {
    "id": "late-events",
    "title": "Hourly metrics keep changing",
    "prompt": "An hourly KPI changes when you refresh three hours later. Events are mobile clicks. Which concepts?",
    "answers": [
      "Event Time",
      "Watermarks",
      "Late Data"
    ],
    "explain": "Processing-time windows move with arrival. Event-time + watermarks define when windows close and how late data is handled."
  },
  {
    "id": "grain-blowup",
    "title": "Revenue explodes after a join",
    "prompt": "Joining orders to order_items makes total revenue 3\u00d7 finance. Which concepts?",
    "answers": [
      "Grain",
      "Additive Facts",
      "Bridge Table"
    ],
    "explain": "Mismatched grain multiplies measures. Declare grain and aggregate to the reporting grain before summing."
  },
  {
    "id": "schema-break",
    "title": "Overnight breakage after a producer change",
    "prompt": "A producer renamed a field; 12 Spark jobs failed at 3am. Which concepts prevent this?",
    "answers": [
      "Data Contracts",
      "Schema Enforcement",
      "Schema Registry"
    ],
    "explain": "Contracts + enforce-on-write + registry compatibility catch breaking changes before fan-out."
  },
  {
    "id": "scd-point-in-time",
    "title": "Customer segment wrong on old orders",
    "prompt": "Historical orders show today\u2019s segment, not the segment at purchase time. Which concepts?",
    "answers": [
      "SCD Type 2",
      "Surrogate Key",
      "Effective Dating"
    ],
    "explain": "Type-2 history with surrogate keys and effective dates enables point-in-time joins."
  },
  {
    "id": "kafka-dup-consumer",
    "title": "Consumer reprocesses a chunk after rebalance",
    "prompt": "After a deploy, a consumer group rebalances and some events are processed twice into the warehouse. Which concepts?",
    "answers": [
      "At Least Once",
      "Offset",
      "Idempotency"
    ],
    "explain": "Kafka consumers are typically at-least-once. Commit policy + idempotent sinks prevent duplicate effects."
  },
  {
    "id": "cost-spike",
    "title": "Warehouse bill doubled",
    "prompt": "Same dashboards, 2\u00d7 spend. Query history shows SELECT * on a wide Parquet fact and full scans. Which concepts?",
    "answers": [
      "Column Pruning",
      "Scan Pruning / Partitioning",
      "Cost Observability"
    ],
    "explain": "Bytes scanned drive cost. Project columns, prune partitions, and attribute spend to queries/teams."
  }
];
window.DE_PLAYBOOK = [
  {
    "id": "wrong-kpi",
    "see": "KPI / revenue suddenly wrong after a join or backfill",
    "do": "Declare grain on both sides. Aggregate to the reporting grain before summing. Prefer overwrite/MERGE for backfills.",
    "concepts": [
      "Grain",
      "Idempotency",
      "Additive Facts"
    ],
    "habit": "Never sum a measure until you know what one row means."
  },
  {
    "id": "dup-retry",
    "see": "Job failed, was rerun, now rows are duplicated",
    "do": "Make the write rerunnable: partition overwrite, MERGE on business keys, or deterministic upserts.",
    "concepts": [
      "Idempotency",
      "At Least Once",
      "Checkpointing"
    ],
    "habit": "Design every pipeline as if it will be retried tomorrow."
  },
  {
    "id": "skew-task",
    "see": "One Spark task runs forever; the rest finished",
    "do": "Open Spark UI \u2192 slow stage \u2192 shuffle read. Check for a hot key; salt / AQE skew join / pre-aggregate.",
    "concepts": [
      "Data Skew",
      "Shuffle",
      "Salting"
    ],
    "habit": "Debug stages and bytes, not just wall-clock time."
  },
  {
    "id": "full-scan",
    "see": "Query reads years of data for one day",
    "do": "Filter on partition columns (often date). Push filters into the scan. Avoid transforming before filtering.",
    "concepts": [
      "Partition Pruning",
      "Predicate Pushdown",
      "Scan Pruning / Partitioning"
    ],
    "habit": "Filter early. Always."
  },
  {
    "id": "stale-green",
    "see": "Pipeline is green but dashboards look stale",
    "do": "Track freshness (max event time / partition arrival). Alert on SLA, not only task success.",
    "concepts": [
      "Freshness",
      "Observability",
      "SLA / Callback"
    ],
    "habit": "Success \u2260 useful data."
  },
  {
    "id": "slow-lake",
    "see": "Lake / table queries get slower every week",
    "do": "Check file counts and sizes. Compact small files; target healthy file sizes.",
    "concepts": [
      "Small File Problem",
      "Compaction",
      "File Size Tuning"
    ],
    "habit": "Ingest shape becomes query cost."
  },
  {
    "id": "cost-spike",
    "see": "Warehouse / cloud bill spiked with same dashboards",
    "do": "Find heavy queries: SELECT *, missing partition filters, huge shuffles. Attribute cost to owners.",
    "concepts": [
      "Column Pruning",
      "Scan Pruning / Partitioning",
      "Cost Observability"
    ],
    "habit": "Bytes scanned are the bill."
  },
  {
    "id": "late-metrics",
    "see": "Hourly metrics keep changing hours later",
    "do": "Use event time + watermarks. Document how late data is handled (update vs ignore).",
    "concepts": [
      "Event Time",
      "Watermarks",
      "Late Data"
    ],
    "habit": "Processing time lies; event time is the business clock."
  },
  {
    "id": "schema-break",
    "see": "Producer changed a field; overnight jobs failed",
    "do": "Schema registry compatibility + enforce-on-write + data contracts before fan-out.",
    "concepts": [
      "Data Contracts",
      "Schema Enforcement",
      "Schema Registry"
    ],
    "habit": "Prevent breaking changes at the boundary."
  },
  {
    "id": "point-in-time",
    "see": "Historical report shows today\u2019s attributes, not past ones",
    "do": "SCD Type 2 (or equivalent) with effective dates; join on the version valid at event time.",
    "concepts": [
      "SCD Type 2",
      "Surrogate Key",
      "Effective Dating"
    ],
    "habit": "Dimensions change; facts must remember the past."
  },
  {
    "id": "cardinality-plan",
    "see": "Join or group-by much slower than expected",
    "do": "Check distinct counts (cardinality). Wrong NDV \u2192 bad join strategy. Broadcast small side when safe.",
    "concepts": [
      "Cardinality",
      "Shuffle",
      "Broadcast Join"
    ],
    "habit": "Know which side is small before you join."
  },
  {
    "id": "lazy-surprise",
    "see": "Spark job \u2018does nothing\u2019 then suddenly runs forever at write/count",
    "do": "Remember lazy evaluation: actions trigger jobs. Inspect the plan before the expensive action.",
    "concepts": [
      "Lazy Evaluation",
      "Job \u2192 Stage \u2192 Task",
      "Catalyst Optimizer"
    ],
    "habit": "Read the plan; don\u2019t wait for the surprise."
  },
  {
    "id": "med-stage-wrong",
    "see": "Antibiotic timing / adherence metrics look wrong vs nursing charts",
    "do": "Confirm whether the metric should use order, dispense, or administration time. Prefer MAR administered_at for inpatient dosing.",
    "concepts": [
      "Order \u2192 Dispense \u2192 Administration Chain",
      "Medication Administration (MAR)",
      "Clinical Event Time vs System Time"
    ],
    "habit": "Name the medication stage before you write the SQL."
  },
  {
    "id": "lab-units-mix",
    "see": "Lab trends jump or ETL fails casting result values",
    "do": "Split numeric vs text results; group by LOINC + unit (UCUM); convert before charting. Use collection time for physiology.",
    "concepts": [
      "Units of Measure (UCUM)",
      "Quantitative vs Qualitative Results",
      "LOINC"
    ],
    "habit": "Same LOINC with mixed units is not one series."
  },
  {
    "id": "api-429-storm",
    "see": "SaaS extract fails with 429s or incomplete pages",
    "do": "Respect Retry-After, lower concurrency, use cursor pagination, and budget calls against quotas.",
    "concepts": [
      "Rate Limiting",
      "Pagination",
      "Timeout / Retry / Backoff"
    ],
    "habit": "Throughput is limited by the API, not your cluster size."
  },
  {
    "id": "webhook-dupes",
    "see": "Webhook-fed table has duplicate events after provider retries",
    "do": "Upsert on event id, verify signatures, and run a nightly reconcile poll.",
    "concepts": [
      "Webhook",
      "Idempotency Key",
      "Polling vs Event-Driven"
    ],
    "habit": "At-least-once delivery needs idempotent handlers."
  },
  {
    "id": "tiny-parquet",
    "see": "Lake queries slow; driver struggles listing files",
    "do": "Check file counts/sizes. Compact small Parquet files; target ~128MB\u20131GB. Fix streaming small-file write pattern.",
    "concepts": [
      "File Size Tuning",
      "Parquet",
      "Delta / Iceberg / Hudi Data Files"
    ],
    "habit": "Table formats manage files \u2014 you still owe them healthy Parquet sizes."
  },
  {
    "id": "gzip-csv-wall",
    "see": "One executor works; the rest of the cluster is idle on ingest",
    "do": "Avoid non-splittable huge .gz CSV. Convert to Parquet/ORC; use splittable compression.",
    "concepts": [
      "Splittable File",
      "Snappy / ZSTD / Gzip",
      "Parquet"
    ],
    "habit": "If it isn't splittable, you don't have parallelism."
  },
  {
    "id": "model-choice",
    "see": "Team debates Data Vault vs star schema vs one big table",
    "do": "Separate layers: integrate multi-source history in Vault/3NF; serve shared KPIs as star marts; flatten to OBT only for a single consumer (BI extract or ML features).",
    "concepts": [
      "Modeling Pattern Choice",
      "Data Vault",
      "Star Schema",
      "One Big Table (OBT)"
    ],
    "habit": "Pattern follows the layer and the consumer \u2014 not a single company-wide religion."
  },
  {
    "id": "cron-overlap",
    "see": "Hourly cron still running when the next hour fires; duplicates or locks",
    "do": "Prevent overlap (flock / max_active_runs=1), make writes idempotent, or move to an orchestrator with sensors.",
    "concepts": [
      "Overlapping Cron Runs",
      "Idempotency",
      "Cron vs Orchestrator"
    ],
    "habit": "A schedule is a start signal \u2014 not a guarantee the previous run finished."
  },
  {
    "id": "cron-tz",
    "see": "Partition dates off by one day after a DST change or UTC vs local schedule",
    "do": "Run schedules in UTC; derive business-local dates in code; document the clock the cron uses.",
    "concepts": [
      "Cron Timezone Pitfalls",
      "Cron Expression",
      "Execution Date / Data Interval"
    ],
    "habit": "Never assume the server\u2019s local midnight is the business day."
  }
];
